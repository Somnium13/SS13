	...
	class Obj_Effect_ProcHolder_Changeling_Sting_Transformation : Obj_Effect_ProcHolder_Changeling_Sting {
		...
		public override dynamic sting_action( Mob user = null, dynamic target = null ) {
			...
			if ( target is Mob_Living_Carbon ) {
				...
				if ( ( C.status_flags & 2 ) != 0 ) {
					((Mob_Living)C).do_jitter_animation( 500 );
					((Mob_Living)C).take_organ_damage( 20, 0 );
				}
				((Ent_Static)target).visible_message( "<span class='danger'>" + target + " begins to violenty convulse!</span>", "<span class='userdanger'>You feel a tiny prick and a begin to uncontrollably convulse!</span>" );
