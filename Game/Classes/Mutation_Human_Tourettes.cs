	...
	class Mutation_Human_Tourettes : Mutation_Human {
		...
		public override void on_life( dynamic owner = null ) {
			...
			if ( Rand13.PercentChance( 10 ) && owner.paralysis <= 1 ) {
				...
				if ( 2<=_a&&_a<=3 ) {
					((Ent_Dynamic)owner).say( "" + ( Rand13.PercentChance( 50 ) ? ";" : "" ) + Rand13.Pick(new object [] { "SHIT", "PISS", "FUCK", "CUNT", "COCKSUCKER", "MOTHERFUCKER", "TITS" }) );
				} else if ( _a==1 ) {
					((Mob)owner).emote( "twitch" );
				}
				x_offset_old = Convert.ToInt32( owner.pixel_x );
				y_offset_old = Convert.ToInt32( owner.pixel_y );
				x_offset = Convert.ToInt32( owner.pixel_x + Rand13.Int( -2, 2 ) );
				y_offset = Convert.ToInt32( owner.pixel_y + Rand13.Int( -1, 1 ) );
