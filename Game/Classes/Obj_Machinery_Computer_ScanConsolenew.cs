...
namespace Somnium.Game {
	class Obj_Machinery_Computer_ScanConsolenew : Obj_Machinery_Computer {
		...
		public void ShowInterface( dynamic user = null, string last_change = null ) {
			...
			if ( Lang13.Bool( this.connected ) ) {
				buttons += new Txt( " <a href='?src=" ).Ref( this ).str( ";task=toggleopen;'>" ).item( ( this.connected.state_open ? "Close" : "Open" ) ).str( " Scanner</a> " ).ToString();
				...
				case "buffer":
					...
					if ( this.buffer is ByTable ) {
						...
						while (( i ??0) <= this.buffer.len) {
							...
							if ( !Lang13.Bool( buffer_slot ) || !Lang13.Bool( buffer_slot.len ) || !Lang13.Bool( buffer_slot["name"] ) || !( Lang13.Bool( buffer_slot["UI"] ) && Lang13.Bool( buffer_slot["UE"] ) || Lang13.Bool( buffer_slot["SE"] ) ) ) {
