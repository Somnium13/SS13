...
namespace Somnium.Game {
	class Obj_Item_Device_CameraBug : Obj_Item_Device {
			...
			if ( href_list.Contains( "view" ) ) {
				...
				if ( C3 is Obj_Machinery_Camera ) {
					...
					Task13.Schedule( 6, (Task13.Closure)(() => {
						...
						if ( this.check_eye( Task13.User ) == true ) {
							Task13.User.reset_view( C3 );
							this.interact();
						} else {
							Task13.User.unset_machine();
							Task13.User.reset_view( null );
		...
		public override bool? check_eye( Mob user = null ) {
			...
			if ( user.stat != 0 || this.loc != user || !user.canmove || user.eye_blind != 0 || !Lang13.Bool( this.current ) ) {
