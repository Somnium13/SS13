...
namespace Somnium.Game {
	class Obj_Item_ConveyorConstruct : Obj_Item {
		...
		public override bool afterattack( dynamic target = null, dynamic user = null, bool? proximity_flag = null, string click_parameters = null ) {
			double? cdir = null;
			Obj_Machinery_Conveyor CB = null;
			...
			if ( !GlobalVars.cardinal.Contains( cdir ) || target == user.loc ) {
				return false;
			}

			foreach (dynamic _a in Lang13.Enumerate( target, typeof(Obj_Machinery_Conveyor) )) {
				CB = _a;
				

				if ( CB.dir == cdir || CB.dir == Num13.Rotate( cdir, 180 ) ) {
					return false;
				}
				cdir = ((int)( cdir )) | ( CB.dir );
				GlobalFuncs.qdel( CB );
			}
