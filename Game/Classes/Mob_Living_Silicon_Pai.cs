...
namespace Somnium.Game {
	class Mob_Living_Silicon_Pai : Mob_Living_Silicon {
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hsrc = null ) {
			...
			dynamic _c = soft; // Was a switch-case, sorry for the mess.
			if ( _c=="buy" ) {
				...
				if ( Lang13.Bool( href_list["freq"] ) ) {
					new_frequency = ( this.sradio.frequency ??0) + ( String13.ParseNumber( href_list["freq"] ) ??0);

					if ( ( new_frequency ??0) < 1200 || ( new_frequency ??0) > 1600 ) {
				...
				if ( Lang13.Bool( href_list["cable"] ) ) {
					...
					foreach (dynamic _b in Lang13.Enumerate( Map13.FetchViewers( null, T ) )) {
						M2 = _b;
						
						M2.show_message( "<span class='warning'>A port on " + this + " opens to reveal " + this.cable + ", which promptly falls to the floor.</span>", 3, "<span class='italics'>You hear the soft click of something light and hard falling to the ground.</span>", 2 );
					}
				}
			}
		...
		public string softwareAtmo(  ) {
			...
			if ( T == null ) {
				...
				if ( total_moles != 0 ) {
					o2_level = ( environment.oxygen ??0) / total_moles;
					n2_level = environment.nitrogen / total_moles;
					co2_level = ( environment.carbon_dioxide ??0) / total_moles;
					plasma_level = ( environment.toxins ??0) / total_moles;
					unknown_level = 1 - ( o2_level + n2_level + co2_level + plasma_level );
					dat += "Nitrogen: " + Num13.Floor( n2_level * 100 ) + "%<br>";
					dat += "Oxygen: " + Num13.Floor( o2_level * 100 ) + "%<br>";
					dat += "Carbon Dioxide: " + Num13.Floor( co2_level * 100 ) + "%<br>";
					dat += "Plasma: " + Num13.Floor( plasma_level * 100 ) + "%<br>";
