...
namespace Somnium.Game {
	class Reagents : Game_Data {
		...
		// Function from file: holder.dm
		public dynamic copy_data( Reagent current_reagent = null ) {
			dynamic trans_data = null;
			dynamic v = null;

			
			if ( !( current_reagent != null ) || !Lang13.Bool( current_reagent.data ) ) {
				return null;
			}
		...
		// Function from file: holder.dm
		public dynamic get_data( string reagent_id = null ) {
			Reagent D = null;
		...
		public bool add_reagent( dynamic reagent = null, dynamic amount = null, dynamic data = null, dynamic reagtemp = null, bool? no_react = null ) {
			...
			if ( D != null ) {
				...
				GlobalFuncs.warning( "" + ( "" + this.my_atom + " attempted to add a reagent called ' " + reagent + " ' which doesn't exist. (" + Task13.User + ")" ) + " in " + "code/modules/reagents/chemistry/holder.dm" + " at line " + 494 + " src: " + this + " usr: " + Task13.User + "." );
			}
		...
		public void reaction( dynamic A = null, int? method = null, double? volume_modifier = null, bool? show_message = null ) {
			...
			if ( A is Mob_Living ) {
				...
				foreach (dynamic _a in Lang13.Enumerate( this.reagent_list, typeof(Reagent) )) {
					...
					R.reaction_mob( L, method, R.volume * ( volume_modifier ??0), show_message, touch_protection );
				}
			} else if ( A is Tile ) {
				...
				foreach (dynamic _b in Lang13.Enumerate( this.reagent_list, typeof(Reagent) )) {
					...
					R2.reaction_turf( A, R2.volume * ( volume_modifier ??0) );
				}
			} else if ( A is Obj ) {
				
				foreach (dynamic _c in Lang13.Enumerate( this.reagent_list, typeof(Reagent) )) {
					R3 = _c;
					
					R3.reaction_obj( A, R3.volume * ( volume_modifier ??0) );
				}
			}
			return;
		}
		...
		public bool handle_reactions(  ) {
			...
			do {
				...
				foreach (dynamic _g in Lang13.Enumerate( this.reagent_list, typeof(Reagent) )) {
					R = _g;
					...
					if ( !((Mob)M).reagent_check( R ) ) {
						...
						if ( can_overdose == true ) {
							...
							if ( GlobalFuncs.is_type_in_list( R, this.addiction_list ) ) {
								...
								foreach (dynamic _a in Lang13.Enumerate( this.addiction_list, typeof(Reagent) )) {
									addicted_reagent = _a;
									

									if ( Lang13.Bool( ((dynamic)addicted_reagent).IsInstanceOfType( R ) ) ) {
		...
		public dynamic trans_to( dynamic target = null, dynamic amount = null, double? multiplier = null, bool? preserve_data = null ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( this.reagent_list, typeof(Reagent) )) {
				...
				if ( current_reagent.id == "blood" && target is Mob_Living_Carbon_Human ) {
					H = target;
					((Mob_Living_Carbon)H).inject_blood( this.my_atom, amount );
					continue;
				}
				current_reagent_transfer = current_reagent.volume * Convert.ToDouble( part );

				if ( preserve_data == true ) {
					trans_data = this.copy_data( current_reagent );
				}
				((Reagents)R).add_reagent( current_reagent.id, current_reagent_transfer * ( multiplier ??0), trans_data, this.chem_temp, true );
