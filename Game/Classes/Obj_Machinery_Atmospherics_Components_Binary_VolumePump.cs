...
namespace Somnium.Game {
	class Obj_Machinery_Atmospherics_Components_Binary_VolumePump : Obj_Machinery_Atmospherics_Components_Binary {
		...
		public Obj_Machinery_Atmospherics_Components_Binary_VolumePump ( dynamic loc = null ) : base( (object)(loc) ) {
		...
		public override bool ui_act( dynamic action = null, ByTable _params = null ) {
			...
			if ( _b=="power" ) {
				...
				dynamic _a = _params["rate"]; // Was a switch-case, sorry for the mess.
				if ( _a=="max" ) {
					this.transfer_rate = 200;
				} else if ( _a=="custom" ) {
					this.transfer_rate = Num13.MaxInt( 0, Num13.MinInt( 200, Convert.ToInt32( this.safe_input( "Pressure control", "Enter new transfer rate (0-" + 200 + " L/s)", this.transfer_rate ) ) ) );
				}
		...
		public override ByTable get_ui_data( dynamic user = null ) {
			...
			data = new ByTable();
			data["on"] = this.on;
			data["transfer_rate"] = Num13.Floor( this.transfer_rate );
		...
		public override int process_atmos(  ) {
			...
			transfer_ratio = Num13.MaxInt( 1, ((int)( this.transfer_rate / air1.volume )) );
