			...
			if ( target2.mind != null ) {
				...
				if ( target2.say_log.len > 8 ) {
					...
					foreach (dynamic _b in Lang13.Enumerate( target2.say_log )) {
						...
						if ( Convert.ToDouble( recent_speech.len ) >= 8 ) {
							break;
						}
				...
				if ( Lang13.Bool( recent_speech.len ) ) {
				...
				if ( target2.mind.changeling != null ) {
					...
					changeling.absorbedcount += target2.mind.changeling.absorbedcount ??0;
