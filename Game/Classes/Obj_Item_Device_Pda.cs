	...
	class Obj_Item_Device_Pda : Obj_Item_Device {
			...
			switch ((int)( this.scanmode )) {
				case 3:
					...
					if ( !( target.reagents == null ) ) {
						...
						if ( Convert.ToDouble( target.reagents.reagent_list.len ) > 0 ) {
							reagents_length = Convert.ToInt32( target.reagents.reagent_list.len );
		...
		public override bool attack( dynamic M = null, dynamic user = null, bool? def_zone = null ) {
			...
			if ( M is Mob_Living_Carbon ) {
				...
				switch ((int)( this.scanmode )) {
					case 1:
						((Ent_Static)user).visible_message( "<span class='alert'>" + user + " has analyzed " + M + "'s vitals!</span>" );
		...
		public Obj_Machinery_MessageServer can_send( Obj_Item_Device_Pda P = null ) {
			...
			if ( !( P != null ) || GlobalFuncs.qdeleted( P ) || P.toff ) {
				return null;
			}
			useMS = null;
		...
		public void show_to_ghosts( Mob user = null, DataPdaMsg msg = null, bool? multiple = null ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( GlobalVars.player_list )) {
				...
				if ( M is Mob_Dead_Observer && Lang13.Bool( M.client ) && ( M.client.prefs.chat_toggles & 256 ) != 0 ) {
					M.show_message( new Txt( "<a href='?src=" ).Ref( M ).str( ";follow=" ).Ref( user ).str( "'>(F)</a><span class='name'> " ).item( msg.sender ).str( " </span><span class='game say'>PDA Message</span> --> <span class='name'>" ).item( ( multiple == true ? "Everyone" : msg.recipient ) ).str( "</span>: <span class='message'>" ).item( msg.message ).item( msg.get_photo_ref() ).str( "</span></span>" ).ToString() );
				}
			}
			return;
		}
