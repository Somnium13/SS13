...
namespace Somnium.Game {
	class Obj_Machinery_Computer_Holodeck : Obj_Machinery_Computer {
		...
		public override dynamic attack_hand( dynamic a = null, bool? b = null, bool? c = null ) {
			...
			if ( this.emagged != 0 && this.emag_programs.len != 0 ) {
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hsrc = null ) {
			...
			if ( this.Adjacent( Task13.User ) || Task13.User is Mob_Living_Silicon ) {
				...
				if ( Lang13.Bool( href_list["loadarea"] ) ) {
					typepath = Lang13.FindClass( href_list["loadarea"] );

					if ( Lang13.Bool( ((dynamic)typepath).IsSubclassOf( typeof(Zone) ) ) ) {
						target = Lang13.FindObj( typepath );

						if ( target is Zone ) {
							this.load_program( target );
						} else {
							Game13.log.WriteMsg( "area not ready: " + typepath );
						}
					} else {
						Game13.log.WriteMsg( "bad area argument: " + href_list["loadarea"] );
					}
				} else if ( href_list.Contains( "safety" ) ) {
					oe = this.emagged;
					this.emagged = !Lang13.Bool( String13.ParseNumber( href_list["safety"] ) ) ?1:0;

					if ( oe == this.emagged ) {
						return null;
					}

					if ( Lang13.Bool( this.program ) && !( this.stat != 0 ) ) {
						
						if ( !( this.emagged != 0 ) && this.program.restricted ) {
		...
		public void derez( dynamic obj = null, bool? silent = null, bool? forced = null ) {
			...
			if ( Lang13.Bool( obj ) && this.emagged != 0 && !( this.stat != 0 ) && !( forced == true ) ) {
		...
		public void load_program( dynamic A = null, bool? force = null, bool? delay = null ) {
			...
			this.program = A;
			this.spawned = ((Zone)A).copy_contents_to( this.linked, true, !( this.emagged != 0 ) );
		...
		// Function from file: computer.dm
		public void nerf( bool active = false ) {
			Obj_Item I = null;
			Obj_Effect_HolodeckEffect HE = null;
			...
			foreach (dynamic _a in Lang13.Enumerate( this.spawned, typeof(Obj_Item) )) {
				...
				I.damtype = ( active ? ((dynamic)( "stamina" )) : Lang13.Initial( I, "damtype" ) );
			}
