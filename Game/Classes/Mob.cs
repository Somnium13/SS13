...
namespace Somnium.Game {
	class Mob : Base_Mob {
		...
		public dynamic actions = new ByTable();
		public int stat = 0;
		public Obj_Screen flash = null;
		public Obj_Screen blind = null;
		public Obj_Screen_Robot_Module hands = null;
		public Obj_Screen_Pull pullin = null;
		public Obj internals = null;
		public dynamic i_select = null;
		public dynamic m_select = null;
		public Obj_Screen healths = null;
		public Obj_Screen throw_icon = null;
		public Obj_Screen damageoverlay = null;
		public Obj_Screen_ZoneSel zone_sel = null;
		public Obj_Screen healthdoll = null;
		...
		public Ent_Static machine = null;
		...
		public double eye_blind = 0;
		public double eye_blurry = 0;
		...
		public Obj_Item_Weapon_Storage_Bag_Sheetsnatcher s_active = null;
		...
		public override int Process_Spacemove( dynamic movement_dir = null ) {
			...
			Ent_Static dense_object_backup = null;
			Ent_Static A = null;
			Ent_Static turf = null;
			Ent_Static AM = null;
			...
			foreach (dynamic _a in Lang13.Enumerate( Map13.FetchInRangeExcludeThis( GlobalFuncs.get_turf( this ), 1 ), typeof(Ent_Static) )) {
				A = _a;
				...
				if ( A is Zone ) {
					...
					if ( !turf.density && !this.mob_negates_gravity() ) {
						continue;
					}
					return 1;
				} else {
			...
			if ( Lang13.Bool( movement_dir ) && dense_object_backup != null ) {
				
				if ( Lang13.Bool( ((dynamic)dense_object_backup).newtonian_move( Num13.Rotate( movement_dir, 180 ) ) ) ) {
					this.WriteMsg( "<span class='info'>You push off of " + dense_object_backup + " to propel yourself.</span>" );
				}
				return 1;
			}
			return 0;
		...
		public override dynamic Stat(  ) {
			...
			if ( this.client != null && this.client.holder != null ) {
				...
				if ( Interface13.IsStatPanelActive( "MC" ) ) {
					...
					if ( GlobalVars.Master != null ) {
						Interface13.Stat( "Subsystems:", "" + Num13.Round( GlobalVars.Master.subsystem_cost, 0.001 ) + "ds" );
						Interface13.Stat( null, null );
		...
		public override void visible_message( dynamic message = null, dynamic self_message = null, string blind_message = null ) {
			...
			mob_viewers.Or( this );
			mob_viewers.Or( Map13.FetchViewers( null, this ) );
			heard = GlobalFuncs.get_hear( 7, this );

			foreach (dynamic _a in Lang13.Enumerate( heard, typeof(Ent_Dynamic) )) {
				A = _a;
				
				possible_viewers.Or( GlobalFuncs.recursive_hear_check( A ) );
			}

			foreach (dynamic _b in Lang13.Enumerate( possible_viewers )) {
				B = _b;
				...
				if ( Map13.FetchInView( null, T ).Contains( this ) ) {
					mob_viewers.Or( B );
				}
			}

			foreach (dynamic _c in Lang13.Enumerate( mob_viewers )) {
				M = _c;
			...
			if ( Lang13.Bool( blind_message ) ) {
				...
				foreach (dynamic _d in Lang13.Enumerate( GlobalFuncs.get_hearers_in_view( 7, this ) )) {
					C = _d;
				...
				foreach (dynamic _e in Lang13.Enumerate( mob_hearers )) {
					MOB = _e;
...
Any-Mode: (hotkey doesn't need to be on)
	...
	END = throw
		...
		// Function from file: update_icons.dm
		public virtual bool update_inv_handcuffed(  ) {
			return false;
		}
		...
		// Function from file: update_icons.dm
		public virtual bool update_hud(  ) {
			return false;
		}
		...
		public virtual dynamic Animalize(  ) {
			...
			((Ent_Static)new_mob).update_pipe_vision();
			_default = new_mob;
		...
		// Function from file: mob_movement.dm
		public virtual bool slip( double s_amount = 0, dynamic w_amount = null, dynamic O = null, dynamic lube = null ) {
			return false;
		}
		...
		// Function from file: mob_movement.dm
		public virtual bool mob_negates_gravity(  ) {
			return false;
		}
		...
		public void AdjustParalysis( int amount = 0 ) {
			...
			if ( ( this.status_flags & 4 ) != 0 ) {
				this.paralysis = Num13.MaxInt( ((int)( this.paralysis + amount )), 0 );
				this.update_canmove();
			}
			return;
		}
		...
		public void Paralyse( dynamic amount = null ) {
			...
			if ( ( this.status_flags & 4 ) != 0 ) {
				this.paralysis = Num13.MaxInt( Num13.MaxInt( ((int)( this.paralysis )), Convert.ToInt32( amount ) ), 0 );
				this.update_canmove();
			}
			return;
		}
		...
		// Function from file: mob.dm
		public void Weaken( dynamic amount = null, bool? ignore_canweaken = null ) {
		...
		public void SetStunned( int amount = 0 ) {
			...
			if ( ( this.status_flags & 1 ) != 0 ) {
				this.stunned = Num13.MaxInt( amount, 0 );
				this.update_canmove();
			}
			return;
		}
		...
		public virtual void start_pulling( dynamic AM = null ) {
			...
			if ( !Lang13.Bool( AM ) || !( this != null ) || this == AM || !( AM.loc is Tile ) ) {
				return;
			}
		...
		// Function from file: mob.dm
		public virtual void show_inv( Ent_Static user = null ) {
			return;
		}
		...
		// Function from file: mob.dm
		public bool put_in_any_hand_if_possible( dynamic W = null, bool? qdel_on_fail = null, bool? disable_warning = null, bool? redraw_mob = null ) {
			qdel_on_fail = qdel_on_fail ?? false;
			disable_warning = disable_warning ?? true;
			redraw_mob = redraw_mob ?? true;

			
			if ( this.equip_to_slot_if_possible( W, 4, qdel_on_fail, disable_warning, redraw_mob ) ) {
				return true;
			} else if ( this.equip_to_slot_if_possible( W, 5, qdel_on_fail, disable_warning, redraw_mob ) ) {
				return true;
			}
			return false;
		}
		...
		public void show_message( dynamic msg = null, int? type = null, string alt = null, int? alt_type = null ) {
			...
			if ( Lang13.Bool( type ) ) {
				...
				if ( ( ( type ??0) & 1 ) != 0 && ( Lang13.Bool( this.disabilities & 1 ) || this.paralysis != 0 ) ) {
				...
				if ( ( ( type ??0) & 2 ) != 0 && Lang13.Bool( this.ear_deaf ) ) {
					...
					if ( !Lang13.Bool( alt ) ) {
						...
						if ( ( ( type ??0) & 1 ) != 0 && Lang13.Bool( this.disabilities & 1 ) ) {
							return;
						}
					}
				}
			...
			if ( this.stat == 1 || this.sleeping > 0 ) {
				this.WriteMsg( "<I>... You can almost hear someone talking ...</I>" );
			} else {
				this.WriteMsg( msg );
			}
			return;
		...
		public virtual void update_normal_mode(  ) {
			Interface13.WindowSet( this, null, "mainwindow.macro=macro hotkey_toggle.is-checked=false input.focus=true input.background-color=#D3B5B5" );
			return;
		}

		// Function from file: login.dm
		public virtual void update_hotkey_mode(  ) {
			Interface13.WindowSet( this, null, "mainwindow.macro=hotkeymode hotkey_toggle.is-checked=true mapwindow.map.focus=true input.background-color=#F0F0F0" );
			return;
		}
		...
		public void update_interface(  ) {
			...
			if ( this.client != null ) {
				...
				if ( Interface13.WindowGet( this, "mainwindow.hotkey_toggle", "is-checked" ) == "true" ) {
		...
		// Function from file: inventory.dm
		public virtual bool unEquip( dynamic I = null, bool? force = null ) {
			
			if ( !Lang13.Bool( I ) ) {
				return true;
			}

			if ( Lang13.Bool( I.flags & 2 ) && !( force == true ) ) {
				return false;
			}
		...
		public void set_machine( Ent_Static O = null ) {
		...
		public dynamic throw_alert( string category = null, Type type = null, int? severity = null, dynamic new_master = null ) {
			...
			if ( Lang13.Bool( this.alerts[category] ) ) {
				...
				if ( Lang13.Bool( new_master ) && new_master != alert.master ) {
					GlobalFuncs.warning( "" + ( "" + this + " threw alert " + category + " with new_master " + new_master + " while already having that alert with master " + alert.master ) + " in " + "code/_onclick/hud/alert.dm" + " at line " + 27 + " src: " + this + " usr: " + Task13.User + "." );
					this.clear_alert( category );
					return this.throw_alert();
				} else if ( alert.type == type && ( !Lang13.Bool( severity ) || severity == Lang13.IntNullable( alert.severity ) ) ) {
		...
		public virtual void face_atom( dynamic A = null ) {
			...
			if ( this.buckled != null || this.stat != 0 || !Lang13.Bool( A ) || !( this.x != 0 ) || !( this.y != 0 ) || !Lang13.Bool( A.x ) || !Lang13.Bool( A.y ) ) {
				return;
			}
			dx = Convert.ToDouble( A.x - this.x );
			dy = Convert.ToDouble( A.y - this.y );

			if ( !( dx != 0 ) && !( dy != 0 ) ) {
				
				if ( Convert.ToDouble( A.pixel_y ) > 16 ) {
					this.dir = ((int)( GlobalVars.NORTH ));
				} else if ( Convert.ToDouble( A.pixel_y ) < -16 ) {
					this.dir = ((int)( GlobalVars.SOUTH ));
				} else if ( Convert.ToDouble( A.pixel_x ) > 16 ) {
		...
		public virtual void CtrlShiftClickOn( dynamic A = null ) {
			((Ent_Static)A).CtrlShiftClick( this );
			return;
		}
		...
		// Function from file: click.dm
		public virtual bool RestrainedClickOn( dynamic A = null ) {
			return false;
		}

		// Function from file: click.dm
		public virtual void RangedAttack( dynamic A = null, string _params = null ) {
			return;
		}

		// Function from file: click.dm
		public virtual void UnarmedAttack( dynamic A = null, bool? proximity_flag = null ) {
		...
		// Function from file: click.dm
		public virtual void ClickOn( dynamic A = null, string _params = null ) {
			...
			if ( !( A is Tile ) && A == this.loc || this.contents.Contains( A ) || this.contents.Contains( A.loc ) || A.loc != null && this.contents.Contains( A.loc.loc ) ) {
				...
				if ( Lang13.Bool( W ) ) {
					resolved = ((Ent_Static)A).attackby( W, this );

					if ( !Lang13.Bool( resolved ) && Lang13.Bool( A ) && Lang13.Bool( W ) ) {
			...
			if ( A is Tile || A.loc is Tile || A.loc != null && A.loc.loc is Tile ) {
				...
				if ( ((Ent_Static)A).Adjacent( this ) ) {
					...
					if ( Lang13.Bool( W ) ) {
						resolved2 = ((Ent_Static)A).attackby( W, this, _params );

						if ( !Lang13.Bool( resolved2 ) && Lang13.Bool( A ) && Lang13.Bool( W ) ) {
							W.afterattack( A, this, 1, _params );
						}
					} else {
		...
		public bool fully_replace_character_name( dynamic oldname = null, dynamic newname = null ) {
			...
			if ( this is Mob_Living_Carbon ) {
				...
				if ( Lang13.Bool( ((dynamic)C).dna ) ) {
					((dynamic)C).dna.real_name = this.real_name;
				}
			}
			...
			if ( this is Mob_Living_Silicon_Ai ) {
				...
				if ( oldname != this.real_name ) {
					...
					if ( Lang13.Bool( ((dynamic)AI).eyeobj ) ) {
						((dynamic)AI).eyeobj.name = "" + newname + " (AI Eye)";
					}

					if ( Lang13.Bool( ((dynamic)AI).aiPDA ) ) {
						((dynamic)AI).aiPDA.owner = newname;
						((dynamic)AI).aiPDA.name = newname + " (" + ((dynamic)AI).aiPDA.ownjob + ")";
					}

					foreach (dynamic _a in Lang13.Enumerate( ((dynamic)AI).connected_robots, typeof(Mob_Living_Silicon_Robot) )) {
						Slave = _a;
			...
			if ( this is Mob_Living_Silicon_Robot ) {
				...
				if ( oldname != this.real_name ) {
					((dynamic)R).notify_ai( 3, oldname, newname );
				}

				if ( Lang13.Bool( ((dynamic)R).camera ) ) {
					((dynamic)R).camera.c_tag = this.real_name;
				}
			}

			if ( Lang13.Bool( oldname ) ) {
				
				foreach (dynamic _b in Lang13.Enumerate( new ByTable(new object [] { GlobalVars.data_core.general, GlobalVars.data_core.medical, GlobalVars.data_core.security, GlobalVars.data_core.locked }), typeof(ByTable) )) {
					L = _b;
					
					R2 = GlobalFuncs.find_record( "name", oldname, L );

					if ( R2 != null ) {
						R2.fields["name"] = newname;
					}
				}
				...
				foreach (dynamic _c in Lang13.Enumerate( searching )) {
					A = _c;
				...
				foreach (dynamic _e in Lang13.Enumerate( GlobalVars.ticker.minds, typeof(Mind) )) {
					...
					foreach (dynamic _d in Lang13.Enumerate( T.objectives, typeof(Objective) )) {
						obj = _d;
		...
		[VerbArg( 1, InputType.Mob | InputType.Obj | InputType.Tile )]
		public void examinate( Ent_Static A = null ) {
		...
		public void button_pressed_F12(  ) {
			...
			if ( this.hud_used != null && this.client != null ) {
				...
				if ( this is Mob_Living_Carbon_Human ) {
					...
					Task13.User.WriteMsg( "<span class ='warning'>Inventory hiding is currently only supported for human mobs, sorry.</span>" );
				}
			} else {
