...
namespace Somnium.Game {
	class Obj_Machinery_Atmospherics_Components_Unary_VentPump : Obj_Machinery_Atmospherics_Components_Unary {
		...
		public Obj_Machinery_Atmospherics_Components_Unary_VentPump ( dynamic loc = null ) : base( (object)(loc) ) {
		...
		public bool broadcast_status(  ) {
			...
			if ( !Lang13.Bool( this.initial_loc.air_vent_names[this.id_tag] ) ) {
				new_name = new Txt().improper().item( this.initial_loc.name ).str( " vent pump #" ).item( this.initial_loc.air_vent_names.len + 1 ).ToString();
				this.initial_loc.air_vent_names[this.id_tag] = new_name;
				this.name = new_name;
			}
			this.initial_loc.air_vent_info[this.id_tag] = signal.data;
			...
			if ( ( ((int)( this.pump_direction ??0 )) & 1 ) != 0 ) {
				...
				if ( pressure_delta > 0 ) {
					...
					if ( Convert.ToDouble( air_contents.temperature ) > 0 ) {
						transfer_moles = pressure_delta * environment.volume / Convert.ToDouble( air_contents.temperature * 8.31 );
		...
		public override dynamic Destroy(  ) {
			...
			if ( Lang13.Bool( this.initial_loc ) ) {
				this.initial_loc.air_vent_info.Remove( this.id_tag );
				this.initial_loc.air_vent_names.Remove( this.id_tag );
