...
namespace Somnium.Game {
	class Obj_Machinery_Atmospherics_Components_Unary_HeatReservoir_Heater : Obj_Machinery_Atmospherics_Components_Unary_HeatReservoir {

		public bool max_temperature = false;
		...
		public Obj_Machinery_Atmospherics_Components_Unary_HeatReservoir_Heater ( dynamic loc = null ) : base( (object)(loc) ) {
			Obj_Item_Weapon_Circuitboard_Thermomachine H = null;

			// Warning: Super call was HERE! If anything above HERE is needed by the super call, it might break!;
			this.initialize_directions = this.dir;
			H = new Obj_Item_Weapon_Circuitboard_Thermomachine( null );
			H.build_path = typeof(Obj_Machinery_Atmospherics_Components_Unary_HeatReservoir_Heater);
			H.name = "circuit board (Heater)";
			this.component_parts = new ByTable();
			this.component_parts.Add( H );
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hsrc = null ) {
			...
			if ( Lang13.Bool( href_list["temp"] ) ) {
				...
				if ( ( amount ??0) > 0 ) {
					this.current_temperature = Num13.MinInt( this.max_temperature ?1:0, ((int)( this.current_temperature + ( amount ??0) )) );
				} else {
					this.current_temperature = Num13.MaxInt( ((int)( 293.41 )), ((int)( this.current_temperature + ( amount ??0) )) );
				}
				this.active_power_usage = this.current_heat_capacity * ( this.current_temperature - 293.41 ) / 100 + this.idle_power_usage;
			}
			this.updateUsrDialog();
		...
		public override dynamic interact( dynamic user = null, bool? flag1 = null ) {
			...
			if ( Convert.ToDouble( air_contents.temperature ) < 373.41 ) {
				temp_text = "<span class='good'>" + air_contents.temperature + "</span>";
			} else if ( Convert.ToDouble( air_contents.temperature ) > 373.41 && Convert.ToDouble( air_contents.temperature ) < 473.41 ) {
			...
			popup = new Browser( user, "freezer", "Pyro Gas Heating System", 400, 240 );
		...
		public override bool? update_icon( dynamic new_state = null, dynamic new_icon = null, int? new_px = null, int? new_py = null ) {
			...
			if ( this.panel_open == true ) {
				this.icon_state = "heater-o";
			} else if ( this.on ) {
				this.icon_state = "heater_1";
			} else {
				this.icon_state = "heater";
			}
			return null;
		...
		public override dynamic attackby( dynamic A = null, dynamic user = null, string _params = null, bool? silent = null, bool? replace_spent = null ) {
			...
			if ( this.default_deconstruction_screwdriver( user, "heater-o", "heater", A ) ) {
				this.on = false;
				this.update_icon();
				return null;
			}
		...
		public override void RefreshParts(  ) {
			...
			this.max_temperature = Lang13.Bool( T * 140 + 293.41 );
