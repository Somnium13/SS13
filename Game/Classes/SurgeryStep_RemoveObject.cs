	...
	class SurgeryStep_RemoveObject : SurgeryStep {
		...
		public override bool success( dynamic user = null, Mob target = null, string target_zone = null, dynamic tool = null, Surgery surgery = null ) {
			...
			if ( Lang13.Bool( this.L ) ) {
				...
				user.WriteMsg( "<span class='warning'>You can't find " + target + "'s " + GlobalFuncs.parse_zone( user.zone_sel.selecting ) + ", let alone any objects embedded in it!</span>" );
			}
			return true;
		}
		...
		public override int preop( dynamic user = null, Mob target = null, string target_zone = null, dynamic tool = null, Surgery surgery = null ) {
			...
			if ( Lang13.Bool( this.L ) ) {
				((Ent_Static)user).visible_message( "" + user + " looks for objects embedded in " + target + "'s " + GlobalFuncs.parse_zone( user.zone_sel.selecting ) + ".", "<span class='notice'>You look for objects embedded in " + target + "'s " + GlobalFuncs.parse_zone( user.zone_sel.selecting ) + "...</span>" );
			} else {
