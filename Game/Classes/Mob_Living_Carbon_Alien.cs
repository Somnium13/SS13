...
namespace Somnium.Game {
	class Mob_Living_Carbon_Alien : Mob_Living_Carbon {
		...
		public override bool say( dynamic message = null, string bubble_type = null ) {
			...
			_default = base.say( (object)(message), "A" );
		...
		public override bool check_breath( dynamic breath = null ) {
			...
			Toxins_pp = Convert.ToDouble( breath.toxins / ((GasMixture)breath).total_moles() * breath_pressure );

			if ( Toxins_pp != 0 ) {
				this.adjustPlasma( breath.toxins * 250 );
				this.throw_alert( "alien_tox", typeof(Obj_Screen_Alert_AlienTox) );
				toxins_used = Convert.ToDouble( breath.toxins );
			} else {
		...
		public override bool adjustFireLoss( dynamic amount = null ) {
			...
			if ( Convert.ToDouble( amount ) > 0 ) {
				base.adjustFireLoss( (object)(amount * 2) );
			} else {
				base.adjustFireLoss( (object)(amount) );
			}
			return false;
		}
		...
		public override void UnarmedAttack( dynamic A = null, bool? proximity_flag = null ) {
			((Ent_Static)A).attack_alien( this );
			return;
		}
