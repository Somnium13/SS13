...
namespace Somnium.Game {
	class RoundEvent : Game_Data {
		...
		public override int? process( dynamic seconds = null ) {
			...
			if ( ( this.startWhen ??0) < ( this.activeFor ??0) && ( this.activeFor ??0) < ( this.endWhen ??0) ) {
				this.tick();
			}
			...
			if ( ( this.activeFor ??0) >= ( this.endWhen ??0) && ( this.activeFor ??0) >= ( this.announceWhen ??0) && ( this.activeFor ??0) >= ( this.startWhen ??0) ) {
				this.kill();
			}
			this.activeFor++;
			return null;
