...
namespace Somnium.Game {
	class Tile : Base_Tile {
		...
		public int atmos_adjacent_turfs = 0;
		public int? atmos_adjacent_turfs_amount = 0;
		...
		public override dynamic remove_air( dynamic amount = null ) {
			...
			GM = new GasMixture();
			sum = this.oxygen + ( this.carbon_dioxide ?1:0) + this.nitrogen + ( this.toxins ?1:0);

			if ( sum > 0 ) {
				GM.oxygen = this.oxygen / sum * Convert.ToDouble( amount );
				GM.carbon_dioxide = ( this.carbon_dioxide ?1:0) / sum * Convert.ToDouble( amount );
				GM.nitrogen = this.nitrogen / sum * Convert.ToDouble( amount );
				GM.toxins = ( this.toxins ?1:0) / sum * Convert.ToDouble( amount );
			}
		...
		public override GasMixture return_air(  ) {
			...
			GM.carbon_dioxide = this.carbon_dioxide ?1:0;
			GM.nitrogen = this.nitrogen;
			GM.toxins = this.toxins ?1:0;
			GM.temperature = this.temperature;
			return GM;
		}
		...
		public override dynamic Entered( Ent_Dynamic Obj = null, Ent_Static oldloc = null ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( Map13.FetchInRange( null, 1 ), typeof(Ent_Static) )) {
				A = _a;
		...
		public bool LinkBlockedWithAccess( Tile T = null, dynamic ID = null ) {
			int adir = 0;
			int rdir = 0;
			Obj O = null;
			...
			if ( GlobalFuncs.DirBlockedWithAccess( this, adir, ID ) ) {
				return true;
			}

			if ( GlobalFuncs.DirBlockedWithAccess( T, rdir, ID ) ) {
				return true;
			}
		...
		public ByTable reachableAdjacentAtmosTurfs(  ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( GlobalVars.cardinal )) {
				...
				if ( Lang13.Bool( dir & this.atmos_adjacent_turfs ) ) {
		...
		// Function from file: AStar.dm
		public ByTable reachableAdjacentTurfs( dynamic atom = null, dynamic ID = null, dynamic simulated_only = null ) {
			ByTable L = null;
			Tile T = null;
			dynamic dir = null;
			...
			foreach (dynamic _a in Lang13.Enumerate( GlobalVars.cardinal )) {
				...
				if ( !T.density && !this.LinkBlockedWithAccess( T, ID ) ) {
					L.Add( T );
				}
			}
			return L;
		...
		// Function from file: LINDA_turf_tile.dm
		public void consider_pressure_difference( Tile T = null, dynamic difference = null ) {
		...
		// Function from file: LINDA_system.dm
		public ByTable GetAtmosAdjacentTurfs( bool? alldir = null ) {
			alldir = alldir ?? false;
			...
			foreach (dynamic _a in Lang13.Enumerate( GlobalVars.cardinal )) {
				...
				if ( !( ( curloc.atmos_adjacent_turfs & Convert.ToInt32( direction ) ) != 0 ) ) {
					continue;
				}
			...
			foreach (dynamic _c in Lang13.Enumerate( GlobalVars.diagonals )) {
				direction2 = _c;
				
				matchingDirections = 0;
				S2 = Map13.GetStep( curloc, Convert.ToInt32( direction2 ) );

				foreach (dynamic _b in Lang13.Enumerate( GlobalVars.cardinal )) {
					checkDirection = _b;
					

					if ( !( ( S2.atmos_adjacent_turfs & Convert.ToInt32( checkDirection ) ) != 0 ) ) {
						continue;
					}
					checkTurf = Map13.GetStep( S2, Convert.ToInt32( checkDirection ) );
		...
		public void CalculateAdjacentTurfs(  ) {
			...
			this.atmos_adjacent_turfs_amount = 0;
			...
			foreach (dynamic _a in Lang13.Enumerate( GlobalVars.cardinal )) {
				...
				counterdir = Map13.GetDistance( T, this );

				if ( this.CanAtmosPass( T ) ) {
					this.atmos_adjacent_turfs_amount += 1;
					this.atmos_adjacent_turfs |= Convert.ToInt32( direction );

					if ( !( ( T.atmos_adjacent_turfs & counterdir ) != 0 ) ) {
		...
		public int can_lay_cable(  ) {
			return this.can_have_cabling() & ( !this.intact ?1:0);
		}

		// Function from file: turf.dm
		public virtual int can_have_cabling(  ) {
			return 1;
		}
