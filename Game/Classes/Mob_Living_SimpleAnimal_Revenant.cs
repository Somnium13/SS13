	...
	class Mob_Living_SimpleAnimal_Revenant : Mob_Living_SimpleAnimal {
		...
		public override dynamic attackby( dynamic A = null, dynamic user = null, string _params = null, bool? silent = null, bool? replace_spent = null ) {
			...
			if ( A is Obj_Item_Weapon_Nullrod ) {
				this.visible_message( "<span class='warning'>" + this + " violently flinches!</span>", "<span class='revendanger'>As the null rod passes through you, you feel your essence draining away!</span>" );
		...
		// Function from file: revenant.dm
		public bool change_essence_amount( int essence_amt = 0, bool? silent = null, dynamic source = null ) {
			silent = silent ?? false;
		...
		// Function from file: revenant.dm
		public void Harvest( dynamic target = null ) {
			...
			if ( !Lang13.Bool( target.stat ) ) {
				...
				if ( Rand13.PercentChance( 10 ) ) {
					target.WriteMsg( "You feel as if you are being watched." );
				}
				return;
			}
			this.draining = true;
				...
				if ( GlobalFuncs.do_after( this, 20, false, target ) ) {
					...
					if ( GlobalFuncs.do_after( this, 20, false, target ) ) {
						...
						if ( !Lang13.Bool( target.stat ) ) {
							...
							target.WriteMsg( "<span class='boldannounce'>You feel something tugging across your body before subsiding.</span>" );
							this.draining = false;
						...
						if ( Convert.ToInt32( target.stat ) != 2 ) {
							target.WriteMsg( "<span class='warning'>You feel a horribly unpleasant draining sensation as your grip on life weakens...</span>" );
						}
						this.reveal( 46 );
						this.stun( 46 );
						((Ent_Static)target).visible_message( "<span class='warning'>" + target + " suddenly rises slightly into the air, their skin turning an ashy gray.</span>" );
						...
						if ( GlobalFuncs.do_after( this, 50, false, target ) ) {
							...
							if ( this.essence_drained <= 90 && Convert.ToInt32( target.stat ) != 2 ) {
							...
							((Ent_Static)target).visible_message( "<span class='warning'>" + target + " slumps onto the ground.</span>", "<span class='revenwarning'>Violets lights, dancing in your vision, getting clo--</span>" );
							this.drained_mobs.Add( target );
							((Mob)target).death( false );
						} else {
							this.WriteMsg( "<span class='revenwarning'>" + ( Lang13.Bool( target ) ? "" + target + " has" : "They have" ) + " been drawn out of your grasp. The link has been broken.</span>" );
							this.draining = false;
							this.essence_drained = 0;

							if ( Lang13.Bool( target ) ) {
								((Ent_Static)target).visible_message( "<span class='warning'>" + target + " slumps onto the ground.</span>", "<span class='revenwarning'>Violets lights, dancing in your vision, receding--</span>" );
							}
							return;
						}
					} else {
						this.WriteMsg( "<span class='revenwarning'>You are not close enough to siphon " + ( Lang13.Bool( target ) ? "" + target + "'s" : "their" ) + " soul. The link has been broken.</span>" );
						this.draining = false;
						this.essence_drained = 0;
						return;
					}
