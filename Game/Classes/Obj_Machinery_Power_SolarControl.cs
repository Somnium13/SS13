	...
	class Obj_Machinery_Power_SolarControl : Obj_Machinery_Power {
		...
		public override bool ui_act( dynamic action = null, ByTable _params = null ) {
			...
			if ( _a=="control" ) {
				...
				if ( Lang13.Bool( _params["cdir"] ) ) {
					this.cdir = GlobalFuncs.dd_range( 0, 359, ( ( this.cdir ??0) + ( String13.ParseNumber( _params["cdir"] ) ??0) + 360 ) % 360 );
					this.targetdir = this.cdir;
				...
				if ( Lang13.Bool( _params["tdir"] ) ) {
					this.trackrate = GlobalFuncs.dd_range( -7200, 7200, this.trackrate + ( String13.ParseNumber( _params["tdir"] ) ??0) );
				...
				this.track = String13.ParseNumber( _params["mode"] );
		...
		public override ByTable get_ui_data( dynamic user = null ) {
			...
			data["angle"] = this.cdir;
			data["direction"] = GlobalFuncs.angle2text( this.cdir );
		...
		public override int ui_interact( dynamic user = null, string ui_key = null, Tgui ui = null, bool? force_open = null, Tgui master_ui = null, UiState state = null ) {
			...
			if ( !( ui != null ) ) {
				ui = new Tgui( user, this, ui_key, "solar_control", this.name, 465, 400, master_ui, state );
				ui.open();
			}
			return 0;
		}
