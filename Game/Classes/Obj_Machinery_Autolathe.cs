...
namespace Somnium.Game {
	class Obj_Machinery_Autolathe : Obj_Machinery {
		...
		public Wires_Autolathe wires = null;
		...
		public ByTable categories = new ByTable(new object [] { "Tools", "Electronics", "Construction", "T-Comm", "Security", "Machinery", "Medical", "Misc" });
		...
		// Function from file: autolathe.dm
		public void adjust_hacked( bool hack = false ) {
			Design D = null;
			Design D2 = null;
			...
			if ( hack ) {
				...
				foreach (dynamic _a in Lang13.Enumerate( this.files.possible_designs, typeof(Design) )) {
					...
					if ( ( D.build_type & 4 ) != 0 && D.category.Contains( "hacked" ) ) {
						this.files.known_designs.Add( D );
					}
				}
			} else {
				...
				foreach (dynamic _b in Lang13.Enumerate( this.files.known_designs, typeof(Design) )) {
					...
					if ( D2.category.Contains( "hacked" ) ) {
						this.files.known_designs.Remove( D2 );
					}
				}
			}
			return;
		...
		// Function from file: autolathe.dm
		public dynamic get_design_cost( Design D = null ) {
			double coeff = 0;
			dynamic dat = null;
		...
		// Function from file: autolathe.dm
		public bool can_build( Design D = null ) {
			double coeff = 0;
		...
		public string search_win( dynamic user = null ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( this.matching_designs, typeof(Design) )) {
				D = _a;
				

				if ( this.disabled || !this.can_build( D ) ) {
					...
					if ( ( this.materials.amount( "$metal" ) ?1:0) >= Convert.ToDouble( metal_cost * multiplier / coeff ) && ( this.materials.amount( "$glass" ) ?1:0) >= Convert.ToDouble( glass_cost * multiplier / coeff ) ) {
						...
						Task13.Schedule( ((int)( 32 / coeff )), (Task13.Closure)(() => {
							...
							if ( Lang13.Bool( is_stack ) ) {
								...
								foreach (dynamic _a in Lang13.Enumerate( T, typeof(Obj_Item_Stack) )) {
									S = _a;
									...
									if ( ( S.amount ??0) >= ( S.max_amount ??0) ) {
										continue;
									}
									to_transfer = ( S.max_amount ??0) - ( S.amount ??0);

									if ( ( to_transfer ??0) < ( multiplier ??0) ) {
										S.amount += to_transfer ??0;
										multiplier -= to_transfer ??0;
										S.update_icon();
										continue;
									} else {
										S.amount += multiplier ??0;
										multiplier = 0;
										S.update_icon();
										break;
									}
								...
								if ( Lang13.Bool( multiplier ) ) {
									N = Lang13.Call( this.being_built.build_path, T );
									N.amount = multiplier;
									N.update_icon();
		...
		public override dynamic attack_hand( dynamic a = null, bool? b = null, bool? c = null ) {
			...
			if ( Lang13.Bool( base.attack_hand( (object)(a), false, c ) ) ) {
				return null;
			}
			this.interact( a );
			return null;
		...
		public override dynamic interact( dynamic user = null, bool? flag1 = null ) {
			...
			if ( this.shocked && !( ( this.stat & 2 ) != 0 ) ) {
				this.shock( user, 50 );
			}

			if ( this.panel_open == true ) {
				dat = this.wires.GetInteractWindow();
			} else {
