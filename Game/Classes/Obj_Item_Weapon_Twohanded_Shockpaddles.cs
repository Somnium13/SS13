	...
	class Obj_Item_Weapon_Twohanded_Shockpaddles : Obj_Item_Weapon_Twohanded {
		...
		public override bool attack( dynamic M = null, dynamic user = null, bool? def_zone = null ) {
			...
			if ( !( M is Mob_Living_Carbon_Human ) ) {
				...
				if ( user.zone_sel != null && user.zone_sel.selecting == "chest" ) {
					...
					if ( GlobalFuncs.do_after( user, 30, null, M ) ) {
						...
						if ( GlobalFuncs.do_after( user, 20, null, M ) ) {
							...
							if ( Convert.ToInt32( H.stat ) == 2 ) {
								...
								if ( H.suiciding || Lang13.Bool( H.disabilities & 128 ) ) {
								...
								} else if ( ((Mob)H).get_ghost() != null || !Lang13.Bool( ((Mob)H).getorgan( typeof(Obj_Item_Organ_Internal_Brain) ) ) ) {
								...
								if ( Lang13.Bool( failed ) ) {
									...
									if ( Convert.ToDouble( H.health ) > halfwaycritdeath ) {
										((Mob_Living)H).adjustOxyLoss( H.health - halfwaycritdeath );
									} else {
										overall_damage = total_brute + total_burn + ((Mob_Living)H).getToxLoss() + ((Mob_Living)H).getOxyLoss();
										mobhealth = H.health;
										((Mob_Living)H).adjustOxyLoss( ( mobhealth - halfwaycritdeath ) * ( ((Mob_Living)H).getOxyLoss() / overall_damage ) );
										((Mob_Living)H).adjustToxLoss( ( mobhealth - halfwaycritdeath ) * ( ((Mob_Living)H).getToxLoss() / overall_damage ) );
										((Mob_Living)H).adjustFireLoss( ( mobhealth - halfwaycritdeath ) * ( total_burn / overall_damage ) );
										((Mob_Living)H).adjustBruteLoss( ( mobhealth - halfwaycritdeath ) * ( total_brute / overall_damage ) );
									}
		...
		// Function from file: defib.dm
		public bool check_defib_exists( dynamic mainunit = null, Obj M = null, Ent_Static O = null ) {
