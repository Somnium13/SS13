	...
	class Obj_Item_Weapon_Cartridge : Obj_Item_Weapon {
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hsrc = null ) {
			...
			if ( _c=="Medical Records" ) {
				...
				S2 = this.radio;
				new_frequency = GlobalFuncs.sanitize_frequency( ( S2.frequency ??0) + ( String13.ParseNumber( href_list["sfreq"] ) ??0) );
			...
			if ( Lang13.Bool( href_list["op"] ) ) {
				...
				if ( _d=="control" ) {
					...
					((Mob_Living_SimpleAnimal_Bot)this.active_bot).bot_control( href_list["op"] );
				}
			}
		...
		public string bot_control(  ) {
			...
			if ( Lang13.Bool( this.active_bot ) ) {
				...
				if ( Convert.ToInt32( this.active_bot.bot_type ) == 2 ) {
					...
					this.menu += "Home: " + ( !Lang13.Bool( MULE.home_destination ) ? "<i>none</i>" : MULE.home_destination ) + "<BR>";
				...
				foreach (dynamic _a in Lang13.Enumerate( GlobalVars.living_mob_list, typeof(Mob_Living_SimpleAnimal_Bot) )) {
					...
					if ( !Bot.on || Bot.z != zlevel || Bot.remote_disabled || !( ( this.bot_access_flags & Bot.bot_type ) != 0 ) ) {
						continue;
					}
			...
			switch ((double)( this.mode )) {
				...
				case 433:
					...
					if ( !Lang13.Bool( this.powmonitor ) ) {
						...
						foreach (dynamic _d in Lang13.Enumerate( this.powmonitor.powernet.nodes, typeof(Obj_Machinery_Power_Terminal) )) {
							term = _d;
					...
					if ( Lang13.Bool( cl ) ) {
						...
						foreach (dynamic _o in Lang13.Enumerate( GlobalVars.living_mob_list, typeof(Mob_Living_SimpleAnimal_Bot_Cleanbot) )) {
							...
							if ( Lang13.Bool( bl2 ) ) {
								...
								ldat += "Cleanbot - <b>[" + bl2.x + "," + bl2.y + " (" + String13.ToUpper( GlobalFuncs.dir2text( direction3 ) ) + ")]</b> - " + ( B2.on ? "Online" : "Offline" ) + "<br>";
							}
						}
