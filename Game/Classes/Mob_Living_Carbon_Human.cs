...
namespace Somnium.Game {
	class Mob_Living_Carbon_Human : Mob_Living_Carbon {
		...
		public Mob_Living_Carbon_Human ( dynamic loc = null ) : base( (object)(loc) ) {
			...
			M = this;
			this.faction |= new Txt().Ref( M ).ToString();
			return;
		}
		...
		public override int shared_ui_interaction( Game_Data src_object = null ) {
			...
			if ( this.dna.check_mutation( "Telekinesis" ) != 0 ) {
				
				if ( GlobalFuncs.tkMaxRangeCheck( this, src_object ) ) {
					return 2;
				}
			}
		...
		public override bool update_hud(  ) {
		...
		// Function from file: update_icons.dm
		public override dynamic update_inv_back(  ) {
			dynamic B = null;

			B = base.update_inv_back();

			if ( Lang13.Bool( B ) ) {
				B.screen_loc = "CENTER-2:14,SOUTH:5";
		...
		// Function from file: update_icons.dm
		public override dynamic update_inv_wear_mask(  ) {
			dynamic M = null;

			M = base.update_inv_wear_mask();
			...
			if ( Lang13.Bool( M ) ) {
				...
				if ( this.client != null && this.hud_used != null && this.hud_used.hud_shown ) {
					...
					if ( this.hud_used.inventory_shown ) {
						M.screen_loc = "WEST+1:8,SOUTH+2:9";
					}
					this.client.screen.Add( M );
				}
			}
		...
		// Function from file: update_icons.dm
		public override dynamic update_inv_head(  ) {
			dynamic H = null;

			H = base.update_inv_head();
			...
			if ( Lang13.Bool( H ) ) {
				...
				if ( this.client != null && this.hud_used != null && this.hud_used.hud_shown ) {
					...
					if ( this.hud_used.inventory_shown ) {
						H.screen_loc = "WEST+1:8,SOUTH+3:11";
					}
					this.client.screen.Add( H );
				}
			}
			this.update_mutant_bodyparts();
		...
		// Function from file: life.dm
		public override bool handle_hud_icons(  ) {
			this.dna.species.handle_hud_icons( this );
			return false;
		}

		// Function from file: life.dm
		public override void handle_vision(  ) {
			...
			} else if ( !this.remote_view && !this.client.adminobs ) {
				this.reset_view( null );
			}
			this.dna.species.handle_vision( this );
			return;
		}
		...
		public override void handle_disabilities(  ) {
			base.handle_disabilities();

			if ( !Lang13.Bool( this.disabilities & 1 ) ) {
				
				if ( this.tinttotal >= 3 ) {
					this.eye_blurry = Num13.MaxInt( ((int)( this.eye_blurry - 2 )), 0 );
				}
			}
		...
		// Function from file: inventory.dm
		public override void wear_mask_update( dynamic I = null, bool? unequip = null ) {
			unequip = unequip ?? true;

			
			if ( Lang13.Bool( I.flags & 32768 ) ) {
				this.update_hair();
			}
			...
			if ( unequip == true && Lang13.Bool( this.v_internal ) ) {
				...
				if ( this.internals != null ) {
					this.internals.icon_state = "internal0";
				}
				this.v_internal = null;
			}
			this.sec_hud_set_security_status();
			base.wear_mask_update( (object)(I), unequip );
			return;
		}
		...
		public override bool unEquip( dynamic I = null, bool? force = null ) {
			...
			if ( I == this.wear_suit ) {
				...
				if ( Lang13.Bool( this.s_store ) ) {
					this.unEquip( this.s_store, true );
				}
				this.wear_suit = null;
				...
				if ( Lang13.Bool( this.r_store ) ) {
					this.unEquip( this.r_store, true );
				}

				if ( Lang13.Bool( this.l_store ) ) {
					this.unEquip( this.l_store, true );
				}
		...
		public override bool mob_negates_gravity(  ) {
			return Lang13.Bool( this.shoes ) && Lang13.Bool( ((Obj_Item)this.shoes).negates_gravity() );
		}
		...
		public override bool experience_pressure_difference( dynamic pressure_difference = null, int direction = 0 ) {
			...
			_default = base.experience_pressure_difference( (object)(pressure_difference), direction );
			return _default;
		}

		// Function from file: human_movement.dm
		public override bool slip( double s_amount = 0, dynamic w_amount = null, dynamic O = null, dynamic lube = null ) {
			bool _default = false;
			...
			_default = base.slip( s_amount, (object)(w_amount), (object)(O), (object)(lube) );
			return _default;
		}
		...
		public override int Process_Spacemove( dynamic movement_dir = null ) {
			...
			if ( this.back is Obj_Item_Weapon_Tank_Jetpack && this.loc is Tile ) {
				J = this.back;

				if ( ( Lang13.Bool( movement_dir ) || J.stabilization_on ) && ((Obj_Item_Weapon_Tank_Jetpack)J).allow_thrust( 0.01, this ) ) {
					return 1;
				}
			}

			if ( this.wear_suit is Obj_Item_Clothing_Suit_Space_Hardsuit && this.loc is Tile ) {
				C = this.wear_suit;

				if ( Lang13.Bool( C.jetpack ) ) {
					
					if ( ( Lang13.Bool( movement_dir ) || C.jetpack.stabilization_on ) && ((Obj_Item_Weapon_Tank_Jetpack)C.jetpack).allow_thrust( 0.01, this ) ) {
						return 1;
					}
				}
			}
		...
		public override bool abiotic( bool? full_body = null ) {
			...
			if ( full_body == true && ( Lang13.Bool( this.l_hand ) && !Lang13.Bool( this.l_hand.flags & 128 ) || Lang13.Bool( this.r_hand ) && !Lang13.Bool( this.r_hand.flags & 128 ) || Lang13.Bool( this.back ) || Lang13.Bool( this.wear_mask ) || Lang13.Bool( this.head ) || Lang13.Bool( this.shoes ) || Lang13.Bool( this.w_uniform ) || Lang13.Bool( this.wear_suit ) || Lang13.Bool( this.glasses ) || Lang13.Bool( this.ears ) || Lang13.Bool( this.gloves ) ) ) {
				return true;
			}
		...
		// Function from file: human_helpers.dm
		public override double tintcheck(  ) {
			double tinted = 0;
			dynamic HT = null;
			dynamic GT = null;
			dynamic MT = null;
		...
		public override bool attack_larva( Mob_Living_Carbon_Alien_Larva user = null ) {
			...
			if ( base.attack_larva( user ) ) {
				...
				if ( this.stat != 2 ) {
					...
					affecting = this.get_organ( GlobalFuncs.ran_zone( user.zone_sel.selecting ) );
					armor_block = this.run_armor_check( affecting, "melee" );
					this.apply_damage( damage, "brute", affecting, armor_block );
					this.updatehealth();
				}
		...
		public override bool attacked_by( Obj_Item I = null, dynamic user = null, bool? def_zone = null ) {
			...
			target_limb = this.get_organ( GlobalFuncs.check_zone( user.zone_sel.selecting ) );
			affecting = this.get_organ( GlobalFuncs.ran_zone( user.zone_sel.selecting ) );
		...
		// Function from file: human_damage.dm
		public override void take_overall_damage( dynamic brute = null, dynamic burn = null ) {
			ByTable parts = null;
			int update = 0;
			dynamic picked = null;
			...
			while (parts.len != 0 && ( Convert.ToDouble( brute ) > 0 || Convert.ToDouble( burn ) > 0 )) {
				...
				update |= Convert.ToInt32( picked.take_damage( brute_per_part, burn_per_part ) );
		...
		// Function from file: human_damage.dm
		public override void take_organ_damage( dynamic brute = null, dynamic burn = null ) {
			ByTable parts = null;
			dynamic picked = null;
			...
			if ( Lang13.Bool( picked.take_damage( brute, burn ) ) ) {
				this.update_damage_overlays(  );
			}
			this.updatehealth();
			return;
		...
		public override void cuff_resist( dynamic I = null, double? breakouttime = null, bool? cuff_break = null ) {
			...
			if ( this.dna != null && this.dna.check_mutation( "Hulk" ) != 0 ) {
				...
				base.cuff_resist( (object)(I), breakouttime, cuff_break );
			}
			return;
		}
		...
		public override bool ex_act( double? severity = null, dynamic target = null ) {
			...
			foreach (dynamic _c in Lang13.Enumerate( this.organs, typeof(Obj_Item_Organ_Limb) )) {
				...
				switch ((string)( temp.name )) {
					case "head":
						update |= temp.take_damage( b_loss * 0.2, f_loss * 0.2 ) ?1:0;
						break;
					case "chest":
						update |= temp.take_damage( b_loss * 0.4, f_loss * 0.4 ) ?1:0;
						break;
					case "l_arm":
						update |= temp.take_damage( b_loss * 0.05, f_loss * 0.05 ) ?1:0;
						break;
					case "r_arm":
						update |= temp.take_damage( b_loss * 0.05, f_loss * 0.05 ) ?1:0;
						break;
		...
		public override double examine( dynamic user = null ) {
			...
			if ( Lang13.Bool( this.wear_mask ) ) {
				skipface |= ( this.wear_mask.flags_inv ??0) & 8;
			}
			t_He = "It";
			t_his = "its";
			t_him = "it";
			...
			if ( !( skipface != 0 ) && GlobalFuncs.is_thrall( this ) && Map13.GetDistance( user, this ) <= 1 ) {
		...
		public override void handle_blood(  ) {
			...
			if ( this.stat != 2 && Convert.ToDouble( this.bodytemperature ) >= 170 ) {
				...
				if ( blood_volume < 560 && blood_volume != 0 ) {
				...
				if ( 501<=_b&&_b<=10000 ) {
					...
					if ( Convert.ToDouble( this.oxyloss ) < 50 ) {
						this.oxyloss += 10;
					}
					this.oxyloss += 1;
		...
		// Function from file: inventory.dm
		public dynamic equipOutfit( dynamic outfit = null, bool? visualsOnly = null ) {
		...
		public dynamic slimeize( dynamic reproduce = null ) {
			...
			((Ent_Static)new_slime).update_pipe_vision();
			_default = new_slime;
			GlobalFuncs.qdel( this );
			return _default;
		}
		...
		public Mob_Living_Silicon_Robot Robotize( bool? delete_items = null ) {
			...
			O = new Mob_Living_Silicon_Robot( this.loc );
			O.cell = new Obj_Item_Weapon_StockParts_Cell( O );
		...
		public override void UnarmedAttack( dynamic A = null, bool? proximity_flag = null ) {
			...
			((Ent_Static)A).attack_hand( this );
			return;
		}

		// Function from file: click.dm
		public override void LaserEyes( dynamic A = null ) {
			
			if ( this.nutrition > 0 ) {
				base.LaserEyes( (object)(A) );
				this.nutrition = Num13.MaxInt( ((int)( this.nutrition - Rand13.Int( 1, 5 ) )), 0 );
			} else {
				this.WriteMsg( "<span class='danger'>You're out of energy!  You need food!</span>" );
			}
			return;
		}
			...
			if ( confirm == "Yes" ) {
				...
				if ( Lang13.Bool( held_item ) ) {
					...
					if ( damagetype != 0 ) {
						...
						if ( !( ( damagetype | 1 ) != 0 ) && !( ( damagetype | 2 ) != 0 ) && !( ( damagetype | 4 ) != 0 ) && !( ( damagetype | 8 ) != 0 ) ) {
