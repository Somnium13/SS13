...
namespace Somnium.Game {
	class Mob_Camera_God : Mob_Camera {
		...
		public bool? nexus_required = false;
		...
		public void check_death(  ) {
			...
			if ( !( this.alive_followers != 0 ) ) {
				...
				if ( this.god_nexus != null && !GlobalFuncs.qdeleted( this.god_nexus ) ) {
		...
		// Function from file: god.dm
		public void update_health_hud(  ) {
			
			if ( this.god_nexus != null && this.hud_used != null && this.hud_used.deity_health_display != null ) {
				this.hud_used.deity_health_display.maptext = "<div align='center' valign='middle' style='position:relative; top:0px; left:6px'> <font color='lime'>" + this.god_nexus.health + "   </font></div>";
			}
			return;
		}
		...
		public void newprophet(  ) {
			...
			if ( Lang13.Bool( choice ) && Lang13.Bool( choice.current ) && Convert.ToInt32( choice.current.stat ) != 2 ) {
				...
				if ( popestick != null ) {
					...
					if ( !H.put_in_any_hand_if_possible( G, false, true, true ) ) {
