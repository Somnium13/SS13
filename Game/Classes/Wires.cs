...
namespace Somnium.Game {
	class Wires : Game_Data {
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hsrc = null ) {
			...
			if ( Task13.User.Adjacent( this.holder ) && Task13.User is Mob_Living ) {
				L = Task13.User;
					...
					if ( Lang13.Bool( href_list["cut"] ) ) {
						...
						if ( this.IsAttached( colour3 ) ) {
							...
							if ( A.attachable ) {
								...
								this.Attach( colour3, A );
							} else {
								L.WriteMsg( "<span class='warning'>You need a attachable assembly!</span>" );
							}
						}
					}
		...
		// Function from file: wires.dm
		public void Pulse( Obj_Item_Device_Assembly S = null ) {
			dynamic colour = null;
		...
		public dynamic Attach( dynamic colour = null, dynamic S = null ) {
			...
			if ( Lang13.Bool( colour ) && Lang13.Bool( S ) && S.attachable ) {
		...
		// Function from file: wires.dm
		public dynamic GetColour( bool index = false ) {
			dynamic colour = null;
			...
			foreach (dynamic _a in Lang13.Enumerate( this.wires )) {
				...
				if ( Lang13.Bool( this.wires[colour] ) == index ) {
					return colour;
				}
			}
			return null;
		...
		public virtual string GetInteractWindow(  ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( this.wires )) {
				...
				html += "<tr>";
				html += "<td" + this.row_options1 + "><font color='" + colour + "'>" + GlobalFuncs.capitalize( colour ) + "</font></td>";
		...
		// Function from file: wires.dm
		public virtual void Interact( dynamic user = null ) {
			string html = null;
			dynamic A = null;
			Obj_Item I = null;
			Browser popup = null;
			...
			if ( Lang13.Bool( html ) ) {
				...
				if ( user.machine != this.holder ) {
					
					foreach (dynamic _a in Lang13.Enumerate( this.signallers )) {
						A = _a;
						

						if ( this.signallers[A] is Obj_Item ) {
							I = this.signallers[A];
				...
				Interface13.Browse( user, null, "window=wires" );
				return;
			}
		...
		// Function from file: wires.dm
		public bool IsInteractionTool( dynamic I = null ) {
			dynamic A = null;
