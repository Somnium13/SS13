	...
	class Obj_Machinery_Computer_Robotics : Obj_Machinery_Computer {
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hsrc = null ) {
			...
			if ( Lang13.Bool( href_list["temp"] ) ) {
				...
				if ( Task13.User is Mob_Living_Silicon && GlobalFuncs.is_special_character( Task13.User ) != 0 || GlobalFuncs.IsAdminGhost( Task13.User ) ) {
					R3 = Lang13.FindObj( href_list["magbot"] );

					if ( R3 is Mob_Living_Silicon_Robot && !Lang13.Bool( R3.emagged ) && ( R3.connected_ai == Task13.User || GlobalFuncs.IsAdminGhost( Task13.User ) ) && !R3.scrambledcodes && this.can_control( Task13.User, R3 ) ) {
		...
		public override dynamic interact( dynamic user = null, bool? flag1 = null ) {
			...
			if ( this.z > 6 ) {
				user.WriteMsg( "<span class='boldannounce'>Unable to establish a connection</span>: ÿ\"You're too far away from the station!" );
				return null;
			}
			...
			foreach (dynamic _a in Lang13.Enumerate( GlobalVars.mob_list, typeof(Mob_Living_Silicon_Robot) )) {
				...
				if ( user is Mob_Living_Silicon || GlobalFuncs.IsAdminGhost( Task13.User ) ) {
					
					if ( ( user is Mob_Living_Silicon && GlobalFuncs.is_special_character( user ) != 0 || GlobalFuncs.IsAdminGhost( Task13.User ) ) && !R.emagged ) {
