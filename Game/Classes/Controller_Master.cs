...
namespace Somnium.Game {
	class Controller_Master : Controller {
		...
		public double processing_interval = 1;
		...
		public Controller_Master (  ) {
			...
			this.processing_interval = this.calculate_gcd();
			return;
		}
		...
		public override int? process( dynamic seconds = null ) {
			...
			Task13.Schedule( 0, (Task13.Closure)(() => {
				...
				foreach (dynamic _a in Lang13.Enumerate( this.subsystems, typeof(Subsystem) )) {
					...
					timer += this.processing_interval;
					SS.next_fire = timer;
				}
				start_time = null;
					...
					if ( this.processing_interval > 0 ) {
						...
						foreach (dynamic _b in Lang13.Enumerate( this.subsystems, typeof(Subsystem) )) {
							...
							if ( ( SS2.can_fire ?1:0) > 0 ) {
								...
								if ( SS2.next_fire <= Game13.time && SS2.last_fire + SS2.wait * 0.5 <= Game13.time ) {
									ran_subsystems = true;
									timer = Game13.timeofday;
									this.last_type_processed = SS2.type;
									SS2.last_fire = Game13.time;
									SS2.fire();
									SS2.cost = SS2.cost * 0.8 + ( Game13.timeofday - timer ) * 0.2;
						...
						this.cost = this.cost * 0.8 + ( Game13.timeofday - ( start_time ??0) ) * 0.2;
						...
						if ( startingtick < Game13.time || ( start_time ??0) + 1 < Game13.timeofday ) {
							extrasleep += Game13.tick_lag * 2;
						}

						if ( Game13.cpu > 80 ) {
							extrasleep += extrasleep + this.processing_interval;
						}
						Task13.Sleep( ((int)( this.processing_interval + extrasleep )) );
					} else {
						Task13.Sleep( 50 );
					}
				}
		...
		// Function from file: master.dm
		public double calculate_gcd(  ) {
			double GCD = 0;
			int minimumInterval = 0;
			Subsystem SS = null;

			minimumInterval = Num13.MinInt( ((int)( Game13.tick_lag * 2 )), 1 );
			...
			foreach (dynamic _a in Lang13.Enumerate( this.subsystems, typeof(Subsystem) )) {
				...
				if ( SS.wait != 0 ) {
					GCD = GlobalFuncs.Gcd( Num13.Floor( SS.wait * 10 ), GCD );
				}
			}
			GCD = Num13.Floor( GCD );
