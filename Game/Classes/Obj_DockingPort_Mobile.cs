	...
	class Obj_DockingPort_Mobile : Obj_DockingPort {
		...
		public string getStatusText(  ) {
			...
			if ( dockedAt is Obj_DockingPort_Stationary_Transit ) {
				...
				_default += " towards " + ( Lang13.Bool( dst ) ? dst.name : "unknown location" ) + " (" + this.timeLeft( 600 ) + "mins)";
			}
			return _default;
		}
						...
						if ( T02.type != ((dynamic)T02).baseturf ) {
							...
							foreach (dynamic _b in Lang13.Enumerate( T02, typeof(Ent_Dynamic) )) {
								...
								if ( AM is Obj ) {
									...
									if ( O == ((dynamic)T02).lighting_object ) {
										continue;
									}
									O.loc = T1;
									...
									if ( !Lang13.Bool( ((dynamic)M).move_on_shuttle ) ) {
										continue;
									}
									M.loc = T1;
									...
									if ( Lang13.Bool( ((dynamic)M).client ) ) {
										...
										if ( Lang13.Bool( ((dynamic)M).buckled ) ) {
											...
											GlobalFuncs.shake_camera( M, 7, 1 );
										}
									}
		...
		public void enterTransit(  ) {
			...
			if ( S1 != null ) {
				...
				if ( this.dock( S1 ) != 0 ) {
					GlobalFuncs.warning( "" + ( "shuttle \"" + this.id + "\" could not enter transit space. Docked at " + ( Lang13.Bool( S0 ) ? S0.id : "null" ) + ". Transit dock " + ( S1 != null ? S1.id : "null" ) + "." ) + " in " + "code/modules/shuttle/shuttle.dm" + " at line " + 269 + " src: " + this + " usr: " + Task13.User + "." );
				} else {
					this.previous = S0;
				}
			} else {
				GlobalFuncs.warning( "" + ( "shuttle \"" + this.id + "\" could not enter transit space. S0=" + ( Lang13.Bool( S0 ) ? S0.id : "null" ) + " S1=" + ( S1 != null ? S1.id : "null" ) ) + " in " + "code/modules/shuttle/shuttle.dm" + " at line " + 273 + " src: " + this + " usr: " + Task13.User + "." );
			}
			return;
		}
