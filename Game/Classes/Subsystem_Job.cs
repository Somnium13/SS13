	...
	class Subsystem_Job : Subsystem {
		...
		public bool PopcapReached(  ) {
			...
			if ( Lang13.Bool( GlobalVars.config.hard_popcap ) || Lang13.Bool( GlobalVars.config.extreme_popcap ) ) {
				...
				if ( this.initial_players_to_assign - Convert.ToDouble( this.unassigned.len ) >= relevent_cap ) {
					return true;
				}
			}
			return false;
		...
		public void LoadJobs(  ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( this.occupations, typeof(Job) )) {
				...
				regex = "" + J.title + "=(-1|\\d+),(-1|\\d+)";
				results = GlobalFuncs.regex_find( jobstext, regex );
				J.total_positions = String13.ParseNumber( results.f_str( 2 ) );
				J.spawn_positions = String13.ParseNumber( results.f_str( 3 ) );
			}
			return;
		}
		...
		public void setup_officer_positions(  ) {
			...
			if ( ( GlobalVars.config.security_scaling_coeff ??0) > 0 ) {
				...
				if ( ( J.spawn_positions ??0) > 0 ) {
					officer_positions = Num13.MinInt( 12, Num13.MaxInt( ((int)( J.spawn_positions ??0 )), Num13.Floor( Convert.ToDouble( this.unassigned.len / GlobalVars.config.security_scaling_coeff ) ) ) );
		...
		public bool DivideOccupations(  ) {
			...
			this.initial_players_to_assign = Convert.ToInt32( this.unassigned.len );
			this.Debug( "DO, Len: " + this.unassigned.len );

			if ( Lang13.Bool( this.unassigned.len ) == false ) {
				return false;
			}
			...
			if ( Lang13.Bool( GlobalVars.config.minimal_access_threshold ) ) {
				...
				if ( ( GlobalVars.config.minimal_access_threshold ??0) > Convert.ToDouble( this.unassigned.len ) ) {
