...
namespace Somnium.Game {
	class Mob_Living_SimpleAnimal_Bot : Mob_Living_SimpleAnimal {
		...
		public override dynamic Bump( Ent_Dynamic A = null, dynamic yes = null ) {
			...
			if ( ( A is Obj_Machinery_Door_Airlock || A is Obj_Machinery_Door_Window ) && !( this.access_card == null ) ) {
				D = A;
		...
		public override dynamic attackby( dynamic A = null, dynamic user = null, string _params = null, bool? silent = null, bool? replace_spent = null ) {
			...
			if ( A is Obj_Item_Weapon_Screwdriver ) {
				...
				if ( !this.locked ) {
					this.open = !this.open;
					user.WriteMsg( "<span class='notice'>Maintenance panel is now " + ( this.open ? "opened" : "closed" ) + ".</span>" );
				} else {
					user.WriteMsg( "<span class='warning'>Maintenance panel is locked.</span>" );
				}
				...
				if ( ((Obj)this.bot_core).allowed( user ) && !this.open && !( this.emagged != 0 ) ) {
					...
					if ( this.open ) {
						user.WriteMsg( "<span class='danger'>Please close the access panel before locking it.</span>" );
					} else {
				...
				if ( A is Obj_Item_Weapon_Weldingtool && user.a_intent != "harm" ) {
					...
					if ( ((Obj_Item_Weapon_Weldingtool)WT).remove_fuel( 0, user ) ) {
						this.health = Num13.MinInt( Convert.ToInt32( this.maxHealth ), Convert.ToInt32( this.health + 10 ) );
		...
		public override bool adjustHealth( dynamic amount = null ) {
			...
			return base.adjustHealth( (object)(amount) );
		}
		...
		public dynamic hack( dynamic user = null ) {
			...
			if ( user is Mob_Living_Silicon || GlobalFuncs.IsAdminGhost( user ) ) {
				...
				hack += new Txt( "Remote network control radio: <A href='?src=" ).Ref( this ).str( ";operation=remote'>" ).item( ( this.remote_disabled ? "Disconnected" : "Connected" ) ).str( "</A><BR><BR>" ).ToString();
			}
			return hack;
		}

		// Function from file: bot.dm
		public bool topic_denied( Mob user = null ) {
			...
			if ( this.emagged == 2 ) {
				...
				if ( !this.hacked ) {
					return true;
				} else if ( !( user is Mob_Living_Silicon ) && !GlobalFuncs.IsAdminGhost( user ) ) {
					return true;
				}
			}
			return false;
		...
		public void summon_step(  ) {
			...
			if ( this.loc == this.summon_target ) {
				this.bot_reset();
				return;
			} else if ( Convert.ToDouble( this.path.len ) > 0 && Lang13.Bool( this.summon_target ) ) {
				next = this.path[1];
		...
		public void calc_summon_path( Ent_Static avoid = null ) {
			...
			Task13.Schedule( 0, (Task13.Closure)(() => {
				this.path = GlobalFuncs.get_path_to( this.loc, this.summon_target, this, typeof(Tile).GetMethod( "Distance_cardinal" ), false, 150, null, null, this.access_card, avoid );

				if ( !Lang13.Bool( this.path.len ) || this.tries >= 5 ) {
		...
		public virtual void calc_path( Ent_Static avoid = null ) {
			this.check_bot_access();
			this.path = GlobalFuncs.get_path_to( this.loc, this.patrol_target, this, typeof(Tile).GetMethod( "Distance_cardinal" ), false, 120, null, null, this.access_card, avoid );
			return;
		}
		...
		public virtual bool bot_control( string command = null, Mob user = null, dynamic user_turf = null, dynamic user_access = null ) {
			...
			if ( !this.on || this.emagged == 2 || this.remote_disabled ) {
				return true;
			}
			...
			switch ((string)( command )) {
				...
				case "summon":
					...
					if ( Lang13.Bool( user_access.len ) != false ) {
		...
		public void patrol_step(  ) {
			...
			if ( this.loc == this.patrol_target ) {
				...
				return;
			} else if ( Convert.ToDouble( this.path.len ) > 0 && this.patrol_target != null ) {
				next = this.path[1];
				...
				if ( moved ) {
					...
					if ( this.blockcount > 5 ) {
						Task13.Schedule( 2, (Task13.Closure)(() => {
							...
							if ( Lang13.Bool( this.path.len ) == false ) {
								this.find_patrol_target();
								this.tries++;
							} else {
								this.blockcount = 0;
		...
		// Function from file: bot.dm
		public virtual void call_bot( Mob_Living_Silicon_Ai caller = null, dynamic waypoint = null, bool? message = null ) {
			...
			this.path = GlobalFuncs.get_path_to( this, waypoint, this, typeof(Tile).GetMethod( "Distance_cardinal" ), false, 200, null, null, all_access );
		...
		public bool bot_step( dynamic dest = null ) {
			...
			if ( Convert.ToDouble( this.path.len ) > 1 ) {
			...
			} else if ( Lang13.Bool( this.path.len ) == true ) {
		...
		public bool bot_move( dynamic dest = null, int? move_speed = null ) {
			...
			if ( !Lang13.Bool( dest ) || !Lang13.Bool( this.path ) || Lang13.Bool( this.path.len ) == false ) {
				this.path = new ByTable();
				return false;
			}
		...
		public dynamic scan( Type scan_type = null, dynamic old_target = null, int? scan_range = null ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( Map13.FetchInView( this, scan_range ) )) {
				scan = _a;
		...
		public void f_speak( string message = null, string channel = null ) {
			...
			if ( Lang13.Bool( channel ) ) {
				...
				if ( !Lang13.Bool( this.Radio.channels[channel] ) ) {
					this.Radio.channels[channel] = 1;
					this.Radio.talk_into( this, message, channel );
					this.Radio.channels[channel] = 0;
				} else {
					this.Radio.talk_into( this, message, channel );
				}
			} else {
