...
namespace Somnium.Game {
	class Mob_Living_SimpleAnimal_Drone : Mob_Living_SimpleAnimal {
		...
		public void update_inv_internal_storage(  ) {
			...
			if ( Lang13.Bool( this.internal_storage ) && this.client != null && this.hud_used != null ) {
		...
		public void drone_chat( dynamic msg = null ) {
			...
			rendered = "<i><span class='game say'>DRONE CHAT: <span class='name'>" + this.name + "</span>: " + msg + "</span></i>";
		...
		public void alert_drones( string msg = null, bool? dead_can_hear = null ) {
			...
			foreach (dynamic _b in Lang13.Enumerate( GlobalVars.player_list )) {
				...
				if ( M is Mob_Living_SimpleAnimal_Drone && Convert.ToInt32( M.stat ) != 2 ) {
					
					foreach (dynamic _a in Lang13.Enumerate( this.faction )) {
						F = _a;
						

						if ( Lang13.Bool( M.faction.Contains( F ) ) ) {
							send_msg = true;
							break;
						}
					}
				} else if ( dead_can_hear == true && GlobalVars.dead_mob_list.Contains( M ) ) {
					send_msg = true;
				}
