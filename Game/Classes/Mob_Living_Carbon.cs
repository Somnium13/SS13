...
namespace Somnium.Game {
	class Mob_Living_Carbon : Mob_Living {
		...
		public override bool update_inv_handcuffed(  ) {
			...
			Obj_Screen_Inventory R2 = null;
			Obj_Screen_Inventory L2 = null;
			...
			if ( Lang13.Bool( this.handcuffed ) ) {
				...
				return true;
			} else if ( this.hud_used != null ) {
				R2 = this.hud_used.r_hand_hud_object;
				L2 = this.hud_used.l_hand_hud_object;
				R2.overlays = null;
				L2.overlays = null;
			}
			return false;
		...
		public override void update_sight(  ) {
			...
			if ( this.stat == 2 ) {
				...
				if ( !( ( ( this.permanent_sight_flags ?1:0) & 16 ) != 0 ) ) {
					this.sight &= 65519;
				}

				if ( !( ( ( this.permanent_sight_flags ?1:0) & 4 ) != 0 ) ) {
					this.sight &= 65531;
				}

				if ( !( ( ( this.permanent_sight_flags ?1:0) & 8 ) != 0 ) ) {
					this.sight &= 65527;
				}
				...
				this.see_in_dark = ( ( ( this.sight == GlobalVars.SEE_TURFS ?1:0) | 12 ) != 0 ? 8 : 2 );
		...
		public override bool handle_regular_hud_updates(  ) {
			...
			if ( this.damageoverlay != null ) {
				...
				if ( this.damageoverlay.overlays != null ) {
					this.damageoverlay.overlays = new ByTable();
				}

				if ( this.stat == 1 ) {
					
					if ( Convert.ToDouble( this.health ) <= ( GlobalVars.config.health_threshold_crit ??0) ) {
						I = new Image( "icons/mob/screen_full.dmi", null, "passage0" );
						I.blend_mode = GlobalVars.BLEND_OVERLAY;
						...
						this.damageoverlay.overlays.Add( I );
					}
				} else {
					
					if ( Lang13.Bool( this.oxyloss ) ) {
						I2 = new Image( "icons/mob/screen_full.dmi", null, "oxydamageoverlay0" );

						dynamic _b = this.oxyloss; // Was a switch-case, sorry for the mess.
						if ( 10<=_b&&_b<=20 ) {
							I2.icon_state = "oxydamageoverlay1";
						} else if ( 20<=_b&&_b<=25 ) {
							I2.icon_state = "oxydamageoverlay2";
						} else if ( 25<=_b&&_b<=30 ) {
							I2.icon_state = "oxydamageoverlay3";
						} else if ( 30<=_b&&_b<=35 ) {
							I2.icon_state = "oxydamageoverlay4";
						} else if ( 35<=_b&&_b<=40 ) {
							I2.icon_state = "oxydamageoverlay5";
						} else if ( 40<=_b&&_b<=45 ) {
							I2.icon_state = "oxydamageoverlay6";
						} else if ( 45<=_b&&_b<=Double.PositiveInfinity ) {
							I2.icon_state = "oxydamageoverlay7";
						}
						this.damageoverlay.overlays.Add( I2 );
					}
					hurtdamage = this.getBruteLoss() + this.getFireLoss() + this.damageoverlaytemp;
					this.damageoverlaytemp = 0;

					if ( hurtdamage != 0 ) {
						I3 = new Image( "icons/mob/screen_full.dmi", null, "brutedamageoverlay0" );
						I3.blend_mode = GlobalVars.BLEND_ADD;

						dynamic _c = hurtdamage; // Was a switch-case, sorry for the mess.
						if ( 5<=_c&&_c<=15 ) {
							I3.icon_state = "brutedamageoverlay1";
						} else if ( 15<=_c&&_c<=30 ) {
							I3.icon_state = "brutedamageoverlay2";
						} else if ( 30<=_c&&_c<=45 ) {
							I3.icon_state = "brutedamageoverlay3";
						} else if ( 45<=_c&&_c<=70 ) {
							I3.icon_state = "brutedamageoverlay4";
						} else if ( 70<=_c&&_c<=85 ) {
							I3.icon_state = "brutedamageoverlay5";
						} else if ( 85<=_c&&_c<=Double.PositiveInfinity ) {
							I3.icon_state = "brutedamageoverlay6";
						}
						black = new Image( I3.icon, I3.icon_state );
						black.color = "#170000";
						this.damageoverlay.overlays.Add( I3 );
						this.damageoverlay.overlays.Add( black );
					}
				}
			}
		...
		public override void handle_status_effects(  ) {
			...
			if ( this.drowsyness != 0 ) {
				...
				this.eye_blurry = Num13.MaxInt( 2, ((int)( this.eye_blurry )) );
			...
			if ( this.druggy != 0 ) {
				this.druggy = Num13.MaxInt( ((int)( this.druggy - 1 )), 0 );
			}
		...
		public override bool handle_regular_status_updates(  ) {
			...
			if ( base.handle_regular_status_updates() ) {
				...
				if ( Convert.ToDouble( this.getOxyLoss() ) > 50 || Convert.ToDouble( this.health ) <= ( GlobalVars.config.health_threshold_crit ??0) ) {
					this.Paralyse( 3 );
					this.stat = 1;
				}
					...
					if ( Lang13.Bool( ITEM ) && ITEM is Obj_Item_Weapon_Tank && Lang13.Bool( this.wear_mask ) && Lang13.Bool( this.wear_mask.flags & 8 ) ) {
						...
						if ( GlobalFuncs.do_mob( Task13.User, this, 40 ) ) {
							...
							if ( Lang13.Bool( this.v_internal ) ) {
								...
								if ( this.internals != null ) {
									this.internals.icon_state = "internal1";
								}
							}
		...
		// Function from file: carbon.dm
		public override void throw_item( dynamic target = null ) {
			dynamic item = null;
			dynamic G = null;
			...
			if ( !Lang13.Bool( target ) || !( this.loc is Tile ) ) {
				return;
			}
		...
		public override bool flash_eyes( double? intensity = null, bool? override_blindness_check = null, bool? affect_silicon = null ) {
			...
			if ( base.flash_eyes( intensity, override_blindness_check, affect_silicon ) ) {
				
				if ( this.weakeyes ) {
					this.Stun( 2 );
				}

				switch ((double)( damage )) {
					case 1:
					...
					case 2:
						...
						this.eye_stat += Rand13.Int( 2, 4 );
						break;
					default:
						this.WriteMsg( "<span class='warning'>Your eyes itch and burn severely!</span>" );
						this.eye_stat += Rand13.Int( 12, 16 );
						break;
				}

				if ( this.eye_stat > 10 ) {
					this.eye_blind += damage;
					this.eye_blurry += damage * Rand13.Int( 3, 6 );

					if ( this.eye_stat > 20 ) {
						
						if ( Rand13.PercentChance( this.eye_stat - 20 ) ) {
							this.WriteMsg( "<span class='warning'>Your eyes start to burn badly!</span>" );
							this.disabilities |= 8;
						} else if ( Rand13.PercentChance( this.eye_stat - 25 ) ) {
		...
		public override bool ContractDisease( Disease D = null ) {
			...
			if ( !passed && ( D.spread_flags & 64 ) != 0 && !( this.internals != null ) ) {
		...
		public virtual void CheckStamina(  ) {
			...
			if ( Lang13.Bool( this.staminaloss ) ) {
				...
				this.setStaminaLoss( Num13.MaxInt( Convert.ToInt32( this.staminaloss - 2 ), 0 ) );
			}
			return;
		}
		...
		public virtual bool check_breath( dynamic breath = null ) {
			...
			O2_partialpressure = Convert.ToDouble( breath.oxygen / ((GasMixture)breath).total_moles() * breath_pressure );
			Toxins_partialpressure = Convert.ToDouble( breath.toxins / ((GasMixture)breath).total_moles() * breath_pressure );
			CO2_partialpressure = Convert.ToDouble( breath.carbon_dioxide / ((GasMixture)breath).total_moles() * breath_pressure );
			...
			if ( O2_partialpressure < safe_oxy_min ) {
				...
				if ( O2_partialpressure > 0 ) {
					...
					this.failed_last_breath = true;
					oxygen_used = Convert.ToDouble( breath.oxygen * ratio / 6 );
				} else {
				...
				oxygen_used = Convert.ToDouble( breath.oxygen / 6 );
			...
			if ( Toxins_partialpressure > safe_tox_max ) {
				ratio2 = Convert.ToDouble( breath.toxins / safe_tox_max * 10 );

				if ( this.reagents != null ) {
					this.reagents.add_reagent( "plasma", Num13.MaxInt( 1, Num13.MinInt( ((int)( ratio2 )), 10 ) ) );
				}
			...
			if ( breath.trace_gases.len != 0 ) {
				...
				foreach (dynamic _a in Lang13.Enumerate( breath.trace_gases, typeof(Gas_SleepingAgent) )) {
					...
					SA_partialpressure = ( SA.moles ??0) / ((GasMixture)breath).total_moles() * breath_pressure;
		...
		// Function from file: inventory.dm
		public virtual void wear_mask_update( dynamic I = null, bool? unequip = null ) {
		...
		// Function from file: carbon.dm
		public bool vomit( int? lost_nutrition = null, bool? blood = null ) {
		...
		// Function from file: carbon.dm
		public virtual void cuff_resist( dynamic I = null, double? breakouttime = null, bool? cuff_break = null ) {
			...
			if ( I is Obj_Item_Weapon_Restraints ) {
				R = I;
		...
		public void throw_mode_on(  ) {
			...
			this.throw_icon.icon_state = "act_throw_on";
			return;
		}
		...
		public void throw_mode_off(  ) {
			...
			this.throw_icon.icon_state = "act_throw_off";
			return;
		}
		...
		// Function from file: carbon.dm
		public virtual double tintcheck(  ) {
			return 0;
		}
