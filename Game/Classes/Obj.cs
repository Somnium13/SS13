...
namespace Somnium.Game {
	class Obj : Ent_Dynamic {
		...
		public virtual void tesla_act( double power = 0 ) {
			...
			power_bounced = power / 121;
			GlobalFuncs.tesla_zap( this, 5, power_bounced );
		...
		// Function from file: objs.dm
		public virtual void empty_object_contents( bool? burn = null, Ent_Static new_loc = null ) {
			burn = burn ?? false;
		...
		public void updateUsrDialog(  ) {
			...
			if ( this.in_use ) {
				...
				if ( Task13.User is Mob_Living_Silicon_Ai || Task13.User is Mob_Living_Silicon_Robot || GlobalFuncs.IsAdminGhost( Task13.User ) ) {
		...
		public bool check_access_list( ByTable L = null ) {
			...
			if ( !Lang13.Bool( this.req_access.len ) && ( !Lang13.Bool( this.req_one_access ) || !Lang13.Bool( this.req_one_access.len ) ) ) {
				return true;
			}
			...
			if ( Lang13.Bool( this.req_one_access ) && Lang13.Bool( this.req_one_access.len ) ) {
		...
		public virtual bool check_access( dynamic I = null ) {
			...
			if ( !Lang13.Bool( L.len ) && ( !Lang13.Bool( this.req_one_access ) || !Lang13.Bool( this.req_one_access.len ) ) ) {
				return true;
			}
		...
		public void atmosanalyzer_scan( GasMixture air_contents = null, dynamic user = null, dynamic target = null ) {
			...
			if ( total_moles > 0 ) {
				o2_concentration = ( air_contents.oxygen ??0) / total_moles;
				n2_concentration = air_contents.nitrogen / total_moles;
				co2_concentration = ( air_contents.carbon_dioxide ??0) / total_moles;
				plasma_concentration = ( air_contents.toxins ??0) / total_moles;
				...
				user.WriteMsg( "<span class='notice'>Nitrogen: " + Num13.Floor( n2_concentration * 100 ) + " %</span>" );
				user.WriteMsg( "<span class='notice'>Oxygen: " + Num13.Floor( o2_concentration * 100 ) + " %</span>" );
				user.WriteMsg( "<span class='notice'>CO2: " + Num13.Floor( co2_concentration * 100 ) + " %</span>" );
				user.WriteMsg( "<span class='notice'>Plasma: " + Num13.Floor( plasma_concentration * 100 ) + " %</span>" );

				if ( unknown_concentration > 0.01 ) {
					user.WriteMsg( "<span class='danger'>Unknown: " + Num13.Floor( unknown_concentration * 100 ) + " %</span>" );
				}
