...
namespace Somnium.Game {
	class Obj_Item_MechaParts_MechaEquipment_Weapon_Honker : Obj_Item_MechaParts_MechaEquipment_Weapon {
		...
		public override bool f_action( dynamic target = null ) {
			...
			GlobalFuncs.message_admins( new Txt().item( GlobalFuncs.key_name_admin( this.chassis.occupant, this.chassis.occupant.client ) ).str( "(<A HREF='?_src_=holder;adminmoreinfo=" ).Ref( this.chassis.occupant ).str( "'>?</A>) (<A HREF='?_src_=holder;adminplayerobservefollow=" ).Ref( this.chassis.occupant ).str( "'>FLW</A>) used a Mecha Honker in (" ).item( T.x ).str( "," ).item( T.y ).str( "," ).item( T.z ).str( " - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=" ).item( T.x ).str( ";Y=" ).item( T.y ).str( ";Z=" ).item( T.z ).str( "'>JMP</a>)" ).ToString() );
			GlobalFuncs.log_game( "" + this.chassis.occupant.ckey + "(" + this.chassis.occupant + ") used a Mecha Honker in (" + T.x + "," + T.y + "," + T.z + ")" );
			return true;
		}
