...
namespace Somnium.Game {
	class SDQLParser : Game_Data {
		...
		public SDQLParser ( dynamic query_list = null ) {
		...
		public int call_function( int i = 0, ByTable node = null, ByTable arguments = null ) {
			...
			if ( Lang13.Length( this.tokenl( i ) ) != 0 ) {
				...
				if ( this.token( i ) != "(" ) {
					...
					do {
						i = this.expression( i + 1, arguments );
		...
		// Function from file: SDQL_2_parser.dm
		public int assignment( int i = 0, ByTable node = null ) {
			dynamic varname = null;

			node.Add( this.token( i ) );

			if ( this.token( i + 1 ) == "=" ) {
				varname = this.token( i );
				node[varname] = new ByTable();
				i = this.expression( i + 2, node[varname] );
			} else {
		...
		public int parse_error( string error_message = null ) {
			this.error = true;
			Task13.User.WriteMsg( "<span class='danger'>SQDL2 Parsing Error: " + error_message + "</span>" );
