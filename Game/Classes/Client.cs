	...
	class Client : Base_Client {
		...
		public Client ( dynamic TopicData = null ) {
			...
			if ( this.prefs.lastchangelog != GlobalVars.changelog_hash ) {
				...
				if ( GlobalVars.config.aggressive_changelog ) {
					...
					Interface13.WindowSet( this, "rpane.changelogb", "background-color=#eaeaea;font-style=bold" );
				}
			}
		...
		public void only_one(  ) {
			...
			foreach (dynamic _b in Lang13.Enumerate( GlobalVars.player_list, typeof(Mob_Living_Carbon_Human) )) {
				...
				GlobalFuncs.add_objective( H.mind, typeof(Objective_Default_Steal) );
				GlobalFuncs.add_objective( H.mind, typeof(Objective_EscapeObj_Hijack), true );
		...
		public void mod_list( dynamic L = null, dynamic O = null, string original_name = null, dynamic objectvar = null ) {
			...
			if ( Convert.ToDouble( L.len ) > 1000 ) {
		...
		// Function from file: mapping.dm
		[VerbInfo( name: "Count Movable Instances", group: "Debug" )]
		public void count_movable_instances(  ) {
			int count = 0;
			dynamic thing = null;
			...
			Task13.User.WriteMsg( "There are " + count + "/" + 65535 + " instances of non-turfs in the world." );
			return;
		}
		...
		public void enable_debug_verbs(  ) {
			...
			this.verbs.Add( typeof(Client).GetMethod( "fps" ) );
			...
			this.verbs.Add( typeof(Client).GetMethod( "disable_communication" ) );
			this.verbs.Add( typeof(Client).GetMethod( "print_pointers" ) );
			this.verbs.Add( typeof(Client).GetMethod( "count_movable_instances" ) );
			this.verbs.Add( typeof(Client).GetMethod( "cmd_show_at_list" ) );
			this.verbs.Add( typeof(Client).GetMethod( "cmd_show_at_list" ) );
			this.verbs.Add( typeof(Client).GetMethod( "manipulate_organs" ) );
		...
		public void map_template_upload(  ) {
			...
			GlobalVars.map_template_uploads.Or( new File( map ) );
		...
		public void map_template_load(  ) {
			...
			filelist = File13.List( "_maps/templates/" );
			filelist.Or( GlobalVars.map_template_uploads );
			map = Interface13.Input( Task13.User, "Choose a Map Template to place at your CURRENT LOCATION", "Place Map Template", null, filelist, InputType.Null | InputType.Any );

			if ( !Lang13.Bool( map ) ) {
				return;
			}

			if ( GlobalVars.map_template_uploads.Contains( map ) ) {
				mapfile = map;
			} else {
				mapfile = new File( "_maps/templates/" + map );
			}

			if ( mapfile is File ) {
				GlobalVars.maploader.__CallVerb("load map", mapfile, T.x, T.y, T.z );
				GlobalFuncs.message_admins( "<span class='adminnotice'>" + GlobalFuncs.key_name_admin( Task13.User ) + " has placed a map template (" + map + ") at <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=" + T.x + ";Y=" + T.y + ";Z=" + T.z + "'>(JMP)</a></span>" );
			} else {
				Task13.User.WriteMsg( "Bad map file: " + map );
			}
			return;
		...
		public void air_status( Ent_Static target = null ) {
			...
			Task13.User.WriteMsg( "<span class='adminnotice'>@" + target.x + "," + target.y + ": O:" + GM.oxygen + " T:" + GM.toxins + " N:" + GM.nitrogen + " C:" + GM.carbon_dioxide + " w " + GM.temperature + " Kelvin, " + GM.return_pressure() + " kPa " + ( burning ? "ÿBURNING" : null ) + "</span>" );

			foreach (dynamic _a in Lang13.Enumerate( GM.trace_gases, typeof(Gas) )) {
				trace_gas = _a;
				
				Task13.User.WriteMsg( "" + trace_gas.type + ": " + trace_gas.moles );
			}
		...
		public void cmd_debug_mob_lists(  ) {
			...
			if ( _a=="Players" ) {
				Task13.User.WriteMsg( GlobalFuncs.list2text( GlobalVars.player_list, "," ) );
			} else if ( _a=="Admins" ) {
				Task13.User.WriteMsg( GlobalFuncs.list2text( GlobalVars.admins, "," ) );
			} else if ( _a=="Mobs" ) {
				Task13.User.WriteMsg( GlobalFuncs.list2text( GlobalVars.mob_list, "," ) );
			} else if ( _a=="Living Mobs" ) {
				Task13.User.WriteMsg( GlobalFuncs.list2text( GlobalVars.living_mob_list, "," ) );
			} else if ( _a=="Dead Mobs" ) {
				Task13.User.WriteMsg( GlobalFuncs.list2text( GlobalVars.dead_mob_list, "," ) );
			} else if ( _a=="Clients" ) {
				Task13.User.WriteMsg( GlobalFuncs.list2text( GlobalVars.clients, "," ) );
			} else if ( _a=="Joined Clients" ) {
				Task13.User.WriteMsg( GlobalFuncs.list2text( GlobalVars.joined_player_list, "," ) );
			}
			return;
		}
		...
		public void callproc_datum( dynamic A = null ) {
			...
			Task13.User.WriteMsg( "<span class='notice'>" + procname + " returned: " + ( Lang13.Bool( returnval ) ? returnval : "null" ) + "</span>" );
			return;
		}
		...
		public void callproc(  ) {
			...
			switch ((string)( Interface13.Alert( "Proc owned by something?", null, "Yes", "No" ) )) {
				case "Yes":
					...
					if ( _b=="Obj" ) {
						...
						foreach (dynamic _a in Lang13.Enumerate( typeof(Client) )) { // THERE IS A SMALL CHANCE THIS LOOP IS BROKEN! TYPECODE = 16384
							C = _a;
			...
			Task13.User.WriteMsg( "<font color='blue'>" + procname + " returned: " + ( Lang13.Bool( returnval ) ? returnval : "null" ) + "</font>" );
		...
		public void sendmob( dynamic M = null ) {
			...
			if ( Lang13.Bool( A ) && A is Zone ) {
				GlobalFuncs.admin_forcemove( M, Rand13.PickFromTable( GlobalFuncs.get_area_turfs( A ) ) );
		...
		public void Getkey(  ) {
			...
			if ( M2 != null ) {
				GlobalFuncs.admin_forcemove( M2, GlobalFuncs.get_turf( Task13.User ) );
		...
		public void Getmob( dynamic M = null ) {
			...
			GlobalFuncs.admin_forcemove( M, GlobalFuncs.get_turf( Task13.User ) );
		...
		public void jumptokey(  ) {
			...
			GlobalFuncs.admin_forcemove( Task13.User, M2.loc );
		...
		public void jumptomob( dynamic M = null ) {
			...
			if ( this.mob != null ) {
				...
				if ( Lang13.Bool( T ) && T is Tile ) {
					...
					GlobalFuncs.admin_forcemove( A, M.loc );
				} else {
		...
		public void jumptoarea( Zone A = null ) {
			...
			GlobalFuncs.admin_forcemove( Task13.User, T2 );
		...
		public void readmin(  ) {
			...
			ByTable Lines = null;
			dynamic line = null;
			ByTable splitline = null;
			string sql_ckey = null;
			DBQuery query = null;
			string error_extra = null;
			dynamic C = null;
			...
			if ( GlobalVars.config.admin_legacy_system ) {
				...
				foreach (dynamic _b in Lang13.Enumerate( Lines )) {
					...
					splitline = GlobalFuncs.text2list( line, " = " );

					if ( String13.CKey( splitline[1] ) == this.ckey ) {
						
						if ( splitline.len >= 2 ) {
							rank = String13.CKeyPreserveCase( splitline[2] );
						}
						break;
					}
					continue;
				...
				if ( !GlobalVars.dbcon.IsConnected() ) {
					...
					this.WriteMsg( "Warning, mysql database is not connected." );
					return;
				}
				sql_ckey = GlobalFuncs.sanitizeSQL( this.ckey );
				query = GlobalVars.dbcon.NewQuery( "SELECT rank FROM " + GlobalFuncs.format_table_name( "admin" ) + " WHERE ckey = '" + sql_ckey + "'" );
				query.Execute();

				while (query.NextRow()) {
					rank = String13.CKeyPreserveCase( query.item[1] );
				}
			}
			...
			if ( !( D != null ) ) {
				...
				if ( rank_names[rank] == null ) {
					...
					this.WriteMsg( "Error while re-adminning, admin rank (" + rank + ") does not exist." + error_extra );
					GlobalFuncs.warning( "" + ( "Error while re-adminning " + this + ", admin rank (" + rank + ") does not exist." + error_extra ) + " in " + "code/modules/admin/admin_verbs.dm" + " at line " + 600 + " src: " + this + " usr: " + Task13.User + "." );
					return;
				}
				D = new Admins( rank_names[rank], this.ckey );
				C = GlobalVars.directory[this.ckey];
				D.associate( C );
				...
				this.WriteMsg( "You are already an admin." );
		...
		public void reset_ooc(  ) {
			GlobalVars.normal_ooc_colour = Lang13.InitialBroken( GlobalVars.normal_ooc_colour );
			return;
		}
		...
		public string debug_variable( dynamic name = null, string value = null, int level = 0, dynamic DA = null ) {
			...
			if ( value == null ) {
				...
				if ( Convert.ToDouble( L.len ) > 0 && !( name == "underlays" || name == "overlays" || name == "vars" || Convert.ToDouble( L.len ) > 500 ) ) {
					html += "<ul>";
					index = 1;
...
Admin:
	...
	F8 = Invisimin
		...
		// Function from file: interface.dm
		[Verb]
		[VerbInfo( name: "Github", desc: "Visit Github", hidden: true )]
		public void github(  ) {
		...
		public void changes(  ) {
			...
			if ( this.prefs.lastchangelog != GlobalVars.changelog_hash ) {
				...
				this.prefs.save_preferences();
				Interface13.WindowSet( this, "rpane.changelogb", "background-color=none;font-style=;" );
			}
			return;
		}
		...
		public void ooc( dynamic msg = null ) {
			...
			if ( Lang13.Bool( this.prefs.unlock_content ) ) {
				...
				if ( Lang13.Bool( this.prefs.toggles & 16 ) ) {
					keyname = new Txt( "<font color='" ).item( ( Lang13.Bool( this.prefs.ooccolor ) ? this.prefs.ooccolor : GlobalVars.normal_ooc_colour ) ).str( "'><img style='width:9px;height:9px;' class=icon src=" ).Ref( "icons/member_content.dmi" ).str( " iconstate=blag>" ).item( keyname ).str( "</font>" ).ToString();
				}
			}
