...
namespace Somnium.Game {
	class Obj_Machinery_Computer_Security : Obj_Machinery_Computer {
		...
		public Obj_Machinery_Camera current = null;
		...
		public override dynamic attack_hand( dynamic a = null, bool? b = null, bool? c = null ) {
			...
			if ( !( this.stat != 0 ) ) {
				...
				if ( C3 != null ) {
					...
					if ( ( Map13.GetDistance( a, this ) > 1 || a.machine != this || a.eye_blind != 0 || !C3.can_use() ) && !( a is Mob_Living_Silicon_Ai ) ) {
						((Mob)a).unset_machine();
						...
						if ( a is Mob_Living_Silicon_Ai ) {
							A = a;
							((Mob_Camera_AiEye)A.eyeobj).setLoc( GlobalFuncs.get_turf( C3 ) );
		...
		public override bool? check_eye( Mob user = null ) {
			...
			if ( ( Map13.GetDistance( user, this ) > 1 || user.eye_blind != 0 || !( this.current != null ) || !this.current.status ) && !( user is Mob_Living_Silicon ) ) {
				return null;
			}
			viewing = Map13.FetchViewers( null, this );

			if ( user is Mob_Living_Silicon_Robot && !( viewing.Find( user ) != 0 ) ) {
				return null;
			}
