...
namespace Somnium.Game {
	class Mob_Living_Carbon_Monkey : Mob_Living_Carbon {
		...
		public override dynamic update_inv_back(  ) {
			...
			if ( Lang13.Bool( B ) ) {
				B.screen_loc = "CENTER-2:15,SOUTH:5";
		...
		public override dynamic update_inv_head(  ) {
			...
			if ( Lang13.Bool( H ) ) {
				H.screen_loc = "CENTER-4:13,SOUTH:5";
		...
		public override dynamic update_inv_wear_mask(  ) {
			...
			if ( Lang13.Bool( M ) ) {
				M.screen_loc = "CENTER-3:14,SOUTH:5";
		...
		public override bool RestrainedClickOn( dynamic A = null ) {
			...
			if ( base.RestrainedClickOn( (object)(A) ) ) {
				return false;
			}
			...
			if ( ML is Mob_Living_Carbon_Human ) {
				affecting = ((Mob_Living_Carbon_Human)ML).get_organ( GlobalFuncs.ran_zone( dam_zone ) );
			}
			armor = ((Mob_Living)ML).run_armor_check( affecting, "melee" );

			if ( Rand13.PercentChance( 75 ) ) {
				((Mob_Living)ML).apply_damage( Rand13.Int( 1, 3 ), "brute", affecting, armor );
				((Ent_Static)ML).visible_message( "<span class='danger'>" + this.name + " bites " + ML + "!</span>", "<span class='userdanger'>" + this.name + " bites " + ML + "!</span>" );
				...
				foreach (dynamic _a in Lang13.Enumerate( this.viruses, typeof(Disease) )) {
					...
					((Mob)ML).ForceContractDisease( D );
				}
			} else {
				((Ent_Static)ML).visible_message( "<span class='danger'>" + this + " has attempted to bite " + ML + "!</span>" );
			}
			return false;
		}
