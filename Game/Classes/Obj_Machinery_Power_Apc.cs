...
namespace Somnium.Game {
	class Obj_Machinery_Power_Apc : Obj_Machinery_Power {
			...
			if ( _a=="lock" ) {
				...
				if ( this.get_malf_status( malfai ) == 1 ) {
					...
					if ( Lang13.Bool( ((dynamic)malfai).malfhacking ) ) {
						malfai.WriteMsg( "You are already hacking an APC." );
						return true;
					}
					malfai.WriteMsg( "Beginning override of APC systems. This takes some time, and you cannot perform other actions during the process." );
					...
					if ( this != null ) {
						...
						if ( !this.aidisabled ) {
							...
							this.locked = true;
							((dynamic)malfai).malf_picker.processing_time += 10;
							...
							malfai.WriteMsg( "Hack complete. The APC is now under your exclusive control." );
							this.update_icon();
						}
					}
				}
		...
		public override dynamic interact( dynamic user = null, bool? flag1 = null ) {
			...
			if ( this.wiresexposed && !( user is Mob_Living_Silicon_Ai ) ) {
		...
		public override bool emag_act( dynamic user = null ) {
			...
			if ( !( this.emagged != 0 ) && !this.malfhack ) {
		...
		public override dynamic attackby( dynamic A = null, dynamic user = null, string _params = null, bool? silent = null, bool? replace_spent = null ) {
			...
			if ( A is Obj_Item_Weapon_Crowbar && this.opened != 0 ) {
				...
				if ( this.emagged != 0 ) {
					...
					this.locked = !this.locked;
					user.WriteMsg( "<span class='notice'>You " + ( this.locked ? "lock" : "unlock" ) + " the APC interface.</span>" );
					this.update_icon();
				} else {
			...
			} else if ( A is Obj_Item_Weapon_Electronics_Apc && this.opened != 0 && this.has_electronics == 0 && !( ( this.stat & 1 ) != 0 || this.malfhack ) ) {
			...
			} else if ( A is Obj_Item_Wallframe_Apc && this.opened != 0 && this.emagged != 0 ) {
				this.emagged = 0;
				...
				GlobalFuncs.qdel( A );
				this.update_icon();
			} else if ( A is Obj_Item_Wallframe_Apc && this.opened != 0 && ( ( this.stat & 1 ) != 0 || this.malfhack ) ) {
				...
				if ( !( this.opened != 0 ) && this.wiresexposed && this.wires.IsInteractionTool( A ) || user is Mob_Living_Silicon && !( ( this.stat & 1 ) != 0 ) && !this.malfhack ) {
					return this.attack_hand( user );
				}
				base.attackby( (object)(A), (object)(user), _params, silent, replace_spent );

				if ( ( ( this.stat & 1 ) != 0 || this.malfhack ) && !( this.opened != 0 ) && Convert.ToDouble( A.force ) >= 5 && Convert.ToDouble( A.w_class ) >= 3 && Rand13.PercentChance( 20 ) ) {
					this.opened = 2;
			...
			if ( ( update & 1 ) != 0 ) {
				...
				if ( ( this.update_state & 128 ) != 0 ) {
					...
					if ( ( this.update_state & 2 ) != 0 ) {
						...
						if ( ( this.update_state & 16 ) != 0 || this.malfhack ) {
		...
		public void set_broken(  ) {
			...
			if ( this.malfai != null && this.operating ) {
				((dynamic)this.malfai).malf_picker.processing_time = Num13.MaxInt( 0, Num13.MinInt( ((dynamic)this.malfai).malf_picker.processing_time - 10, 1000 ) );
			}
			this.stat |= 1;
			this.operating = false;
		...
		public void ion_act(  ) {
			...
			if ( !this.malfhack && this.z == 1 ) {
				...
				if ( Rand13.PercentChance( 3 ) ) {
					...
					if ( Convert.ToDouble( this.cell.charge ) > 0 ) {
						this.cell.charge = 0;
						((Obj_Item_Weapon_StockParts_Cell)this.cell).corrupt();
						this.malfhack = true;
						this.update_icon();
						smoke = new EffectSystem_SmokeSpread();
						smoke.set_up( 1, this.loc );
						smoke.attach( this );
						smoke.start();
						s = new EffectSystem_SparkSpread();
						s.set_up( 3, 1, this );
						s.start();
						this.visible_message( "<span class='warning'>The " + this.name + " suddenly lets out a blast of smoke and some sparks!</span>", "<span class='italics'>You hear sizzling electronics.</span>" );
					}
				}
			}
			return;
		...
		public bool can_use( Mob user = null, bool? loud = null ) {
			...
			if ( user.has_unlimited_silicon_privilege ) {
				...
				if ( this.aidisabled || this.malfhack && this.malfai is Mob_Living_Silicon_Ai && ( AI is Mob_Living_Silicon_Ai && this.malfai != AI && this.malfai != ((dynamic)AI).parent || robot is Mob_Living_Silicon_Robot && Lang13.Bool( ((dynamic)this.malfai).connected_robots.Contains( robot ) ) ) ) {
					...
					if ( !( loud == true ) ) {
						user.WriteMsg( new Txt( "<span class='danger'>" ).The( this ).item().str( " have AI control disabled!</span>" ).ToString() );
					}
					return false;
				}
			} else if ( !( Map13.GetDistance( this, user ) <= 1 ) || !( this.loc is Tile ) ) {
				return false;
			}
			return true;
		}
		...
		public int get_malf_status( dynamic user = null ) {
			...
			if ( GlobalFuncs.is_special_character( user ) != 0 && user is Mob_Living_Silicon_Ai ) {
				
				if ( this.malfai == ( Lang13.Bool( user.parent ) ? user.parent : user ) ) {
					
					if ( this.occupier == user ) {
						return 3;
		...
		public int check_updates(  ) {
			...
			} else if ( this.emagged != 0 || this.malfai != null ) {
