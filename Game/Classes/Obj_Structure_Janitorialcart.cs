	...
	class Obj_Structure_Janitorialcart : Obj_Structure {
			...
			if ( Lang13.Bool( href_list["sign"] ) ) {
				...
				if ( this.signs != 0 ) {
					...
					if ( Lang13.Bool( Sign ) ) {
						...
						GlobalFuncs.warning( "" + ( "Signs (" + this.signs + ") didn't match contents" ) + " in " + "code/game/objects/structures/janicart.dm" + " at line " + 140 + " src: " + this + " usr: " + Task13.User + "." );
						this.signs = 0;
					}
				}
			}
		...
		public override dynamic attackby( dynamic A = null, dynamic user = null, string _params = null, bool? silent = null, bool? replace_spent = null ) {
			...
			if ( A is Obj_Item_Weapon_Mop ) {
				...
				if ( !( this.mymop != null ) ) {
					((Obj_Item_Weapon_Mop)m).janicart_insert( user, this );
				} else {
				...
				if ( !( this.mybag != null ) ) {
					t = A;
					((Obj_Item_Weapon_Storage_Bag_Trash)t).janicart_insert( user, this );
				} else {
					user.WriteMsg( fail_msg );
				}
				...
				if ( !( this.myreplacer != null ) ) {
					l = A;
					((Obj_Item_Device_Lightreplacer)l).janicart_insert( user, this );
				} else {
					user.WriteMsg( fail_msg );
				}
		...
		// Function from file: janicart.dm
		public void wet_mop( dynamic mop = null, dynamic user = null ) {
