...
namespace Somnium.Game {
	class GameMode : Game_Data {
		...
		public void forge_wizard_objectives( Mind wizard = null ) {
			GlobalFuncs.generate_objectives( wizard, 2 );
			return;
		}
		...
		public void assign_exchange_role( dynamic owner = null ) {
			string faction = null;
			Objective_Default_Steal_Exchange exchange_objective = null;
			Objective_Default_Steal_Exchange_Backstab backstab_objective = null;
			...
			exchange_objective = new Objective_Default_Steal_Exchange();
			...
			if ( Rand13.PercentChance( 20 ) ) {
				backstab_objective = new Objective_Default_Steal_Exchange_Backstab();
		...
		public bool equip_traitor( dynamic traitor_mob = null, bool? safety = null ) {
			...
			if ( !Lang13.Bool( R ) ) {
				...
				traitor_mob.WriteMsg( "The Syndicate have cunningly disguised a Syndicate Uplink as your " + R.name + " " + loc + ". Simply dial the frequency " + GlobalFuncs.format_frequency( freq ) + " to unlock its hidden features." );
				...
				R.hidden_uplink = T2;
				T2.uplink_owner = "" + traitor_mob.key;
				P = R;
				P.lock_code = pda_pass;
				traitor_mob.WriteMsg( "The Syndicate have cunningly disguised a Syndicate Uplink as your " + R.name + " " + loc + ". Simply enter the code \"" + pda_pass + "\" into the ringtone select to unlock its hidden features." );
		...
		public bool auto_declare_completion_traitor(  ) {
			...
			if ( this.traitors.len != 0 ) {
				...
				foreach (dynamic _c in Lang13.Enumerate( this.traitors, typeof(Mind) )) {
					...
					foreach (dynamic _a in Lang13.Enumerate( GlobalVars.world_uplinks, typeof(Obj_Item_Device_Uplink) )) {
						H = _a;
						

						if ( H != null && Lang13.Bool( H.uplink_owner ) && H.uplink_owner == traitor.key ) {
							TC_uses += H.used_TC;
							uplink_true = true;
		...
		public void add_law_zero( dynamic killer = null ) {
			...
			killer.verbs += typeof(Mob_Living_Silicon_Ai).GetMethod( "choose_modules" );
			killer.malf_picker = new ModulePicker();
			return;
		}
		...
		public virtual void forge_traitor_objectives( Mind traitor = null ) {
			bool objective_count = false;
			double? i = null;
			int objective_count2 = 0;
			...
			if ( traitor.current is Mob_Living_Silicon ) {
				...
				if ( Rand13.PercentChance( 10 ) ) {
					GlobalFuncs.add_objective( traitor, typeof(Objective_EscapeObj_Block) );
				}
				i = null;
				i = objective_count ?1:0;

				while (( i ??0) < ( GlobalVars.config.traitor_objectives_amount ??0)) {
					GlobalFuncs.add_objective( traitor, typeof(Objective_Default_Assassinate) );
					i++;
				}
				...
				GlobalFuncs.generate_objectives( traitor, ( GlobalVars.config.traitor_objectives_amount ??0) - objective_count2, GlobalVars.TRUE );
			}
			return;
		}
		...
		public bool remove_shadowling( Mind ling_mind = null ) {
			...
			if ( M is Mob_Living_Silicon ) {
				...
				Task13.Schedule( 30, (Task13.Closure)(() => {
					...
					if ( !Lang13.Bool( M ) || GlobalFuncs.qdeleted( M ) ) {
						return;
					}
			...
			if ( this.syndicates.len != 0 || GlobalVars.ticker != null && GlobalVars.ticker.mode is GameMode_Nuclear ) {
				...
				foreach (dynamic _b in Lang13.Enumerate( this.syndicates, typeof(Mind) )) {
					...
					foreach (dynamic _a in Lang13.Enumerate( GlobalVars.world_uplinks, typeof(Obj_Item_Device_Uplink) )) {
						...
						if ( H != null && Lang13.Bool( H.uplink_owner ) && H.uplink_owner == syndicate.key ) {
							TC_uses += H.used_TC;
							purchases += H.purchase_log;
						}
					}
		...
		// Function from file: nuclear.dm
		public bool equip_syndicate( dynamic synd_mob = null, bool? telecrystals = null ) {
			telecrystals = telecrystals ?? GlobalVars.TRUE;

			
			if ( telecrystals == true ) {
		...
		public bool add_hog_follower( Mind follower_mind = null, string colour = null ) {
			...
			N = Lang13.FindIn( typeof(Obj_Item_Weapon_Nullrod), H );

			if ( Lang13.Bool( N ) ) {
				H.WriteMsg( "<span class='danger'>Your null rod prevented the " + colour + " deity from brainwashing you.</span>" );
				return false;
			}
		...
		public virtual void forge_changeling_objectives( Mind changeling = null, bool? team_mode = null ) {
			...
			bool? escape_objective_possible = null;
			...
			GlobalFuncs.generate_objectives( changeling, 4, escape_objective_possible );
			return;
		}
		...
		public bool auto_declare_completion_blob(  ) {
			...
			if ( GlobalVars.ticker.mode is GameMode_Blob ) {
				...
				if ( Lang13.Bool( ((dynamic)blob_mode).infected_crew.len ) ) {
					text = "<FONT size = 2><B>The blob" + ( Convert.ToDouble( ((dynamic)blob_mode).infected_crew.len ) > 1 ? "s were" : " was" ) + ":</B></FONT>";
		...
		public ByTable get_players_for_role( string role = null ) {
			...
			while (candidates.len < this.recommended_enemies) {
				...
				if ( Convert.ToDouble( drafted.len ) > 0 ) {
		...
		public virtual void send_intercept( int? report = null ) {
			...
			possible_modes[Rand13.Int( 1, Convert.ToInt32( possible_modes.len ) )] = "" + GlobalVars.ticker.mode;
