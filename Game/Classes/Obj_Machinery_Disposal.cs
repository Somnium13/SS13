...
namespace Somnium.Game {
	class Obj_Machinery_Disposal : Obj_Machinery {
		...
		// Function from file: disposal-unit.dm
		public void stuff_mob_in( Ent_Static target = null, dynamic user = null ) {
			
			if ( !( user is Mob_Living_Carbon ) && !Lang13.Bool( user.ventcrawler ) ) {
				return;
			}
			...
			if ( user == target ) {
				((Ent_Static)user).visible_message( "" + user + " starts climbing into " + this + ".", "<span class='notice'>You start climbing into " + this + "...</span>" );
			} else {
			...
			if ( GlobalFuncs.do_mob( user, target, 20 ) ) {
				...
				if ( user == target ) {
					((Ent_Static)user).visible_message( "" + user + " climbs into " + this + ".", "<span class='notice'>You climb into " + this + ".</span>" );
				} else {
