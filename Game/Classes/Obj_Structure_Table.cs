...
namespace Somnium.Game {
	class Obj_Structure_Table : Obj_Structure {
		...
		public dynamic tableclimber = null;
		...
		public override dynamic MouseDrop( dynamic over = null, dynamic src_location = null, dynamic over_location = null, string src_control = null, dynamic over_control = null, string _params = null ) {
			...
			if ( over != Task13.User || !Task13.User.IsAdvancedToolUser() ) {
				return null;
			}
			this.interact( Task13.User );
			return null;
		...
		// Function from file: tables_racks.dm
		public virtual bool climb_table( dynamic user = null ) {
			double? climb_time = null;

			this.add_fingerprint( user );
			((Ent_Static)user).visible_message( "<span class='warning'>" + user + " starts climbing onto " + this + ".</span>", "<span class='notice'>You start climbing onto " + this + "...</span>" );
			climb_time = 20;
			...
			if ( GlobalFuncs.do_mob( user, user, climb_time ) ) {
				...
				if ( this.loc != null ) {
					...
					if ( false ) {
						((Ent_Static)user).visible_message( "<span class='warning'>" + user + " climbs onto " + this + ".</span>", "<span class='notice'>You climb onto " + this + ".</span>" );
		...
		public override bool MouseDrop_T( Ent_Static O = null, dynamic user = null, bool? needs_opened = null, bool? show_message = null, bool? move_them = null ) {
			base.MouseDrop_T( O, (object)(user), needs_opened, show_message, move_them );

			if ( O is Mob && user == O && user is Mob_Living_Carbon_Human ) {
				
				if ( Lang13.Bool( user.canmove ) ) {
		...
		public override dynamic attack_hand( dynamic a = null, bool? b = null, bool? c = null ) {
			...
			if ( Lang13.Bool( this.tableclimber ) && this.tableclimber != a ) {
				((Mob)this.tableclimber).Weaken( 2 );
				((Ent_Static)this.tableclimber).visible_message( "<span class='warning'>" + this.tableclimber.name + " has been knocked off the table", "You're knocked off the table!", "You see " + this.tableclimber.name + " get knocked off the table</span>" );
			}
