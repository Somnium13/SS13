	...
	class Mob_Living_SimpleAnimal_Bot_Medbot : Mob_Living_SimpleAnimal_Bot {
		...
		public override bool handle_automated_action(  ) {
			...
			if ( Lang13.Bool( this.patient ) && Map13.GetDistance( this, this.patient ) <= 1 ) {
				...
				return false;
			} else if ( Lang13.Bool( this.patient ) && Lang13.Bool( this.path.len ) && Map13.GetDistance( this.patient, this.path[this.path.len] ) > 2 ) {
			...
			if ( Lang13.Bool( this.patient ) && Lang13.Bool( this.path.len ) == false && Map13.GetDistance( this, this.patient ) > 1 ) {
				this.path = GlobalFuncs.get_path_to( this.loc, GlobalFuncs.get_turf( this.patient ), this, typeof(Tile).GetMethod( "Distance_cardinal" ), false, 30, null, null, this.access_card );
				this.v_mode = 9;

				if ( !Lang13.Bool( this.path.len ) ) {
					this.path = GlobalFuncs.get_path_to( this.loc, GlobalFuncs.get_turf( this.patient ), this, typeof(Tile).GetMethod( "Distance_cardinal" ), false, 30, true, null, this.access_card );
			...
			if ( Convert.ToDouble( this.path.len ) > 0 && Lang13.Bool( this.patient ) ) {
			...
			if ( Convert.ToDouble( this.path.len ) > 8 && Lang13.Bool( this.patient ) ) {
				this.frustration++;
			}
		...
		public void medicate_patient( dynamic C = null ) {
			...
			if ( !Lang13.Bool( reagent_id ) ) {
				...
				Task13.Schedule( 30, (Task13.Closure)(() => {
					...
					if ( Map13.GetDistance( this, this.patient ) <= 1 && this.on && this.assess_patient( this.patient ) ) {
