...
namespace Somnium.Game {
	class Species : Game_Data {
		...
		// Function from file: species.dm
		public void handle_environment( GasMixture environment = null, Mob_Living_Carbon H = null ) {
			...
			} else if ( Convert.ToDouble( H.bodytemperature ) < 260.41 && !H.dna.mutations.Contains( GlobalVars.mutations_list["Cold Resistance"] ) ) {
			...
			if ( 550<=_c&&_c<=Double.PositiveInfinity ) {
				...
				if ( H.dna.check_mutation( "Cold Resistance" ) != 0 || this.specflags.Contains( "Cold Resistance" ) ) {
					H.clear_alert( "pressure" );
				} else {
					H.adjustBruteLoss( 2 );
		...
		// Function from file: species.dm
		public double handle_too_little_breath( Mob_Living H = null, double? breath_pp = null, double? safe_breath_min = null, double? true_pp = null ) {
		...
		public bool check_breath( GasMixture breath = null, Mob_Living H = null ) {
			...
			gas_breathed = 0;
			O2_pp = breath.get_breath_partial_pressure( breath.oxygen );
			Toxins_pp = breath.get_breath_partial_pressure( breath.toxins );
			CO2_pp = breath.get_breath_partial_pressure( breath.carbon_dioxide );

			if ( Lang13.Bool( this.safe_oxygen_max ) ) {
				
				if ( ( O2_pp ??0) > ( this.safe_oxygen_max ??0) && !this.specflags.Contains( 256 ) ) {
					ratio = ( breath.oxygen ??0) / ( this.safe_oxygen_max ??0) * 10;
					H.adjustOxyLoss( Num13.MaxInt( this.oxy_breath_dam_min ?1:0, Num13.MinInt( ((int)( ratio )), this.oxy_breath_dam_max ) ) );
			...
			if ( Lang13.Bool( this.safe_oxygen_min ) ) {
				...
				if ( ( O2_pp ??0) < ( this.safe_oxygen_min ??0) ) {
					gas_breathed = this.handle_too_little_breath( H, O2_pp, this.safe_oxygen_min, breath.oxygen );
					H.throw_alert( "oxy", typeof(Obj_Screen_Alert_Oxy) );
				} else {
					((dynamic)H).failed_last_breath = 0;
					H.adjustOxyLoss( -5 );
					gas_breathed = ( breath.oxygen ??0) / 6;
					H.clear_alert( "oxy" );
				}
			}
			...
			if ( Lang13.Bool( this.safe_co2_min ) ) {
				...
				if ( ( CO2_pp ??0) < ( this.safe_co2_min ??0) ) {
					gas_breathed = this.handle_too_little_breath( H, CO2_pp, this.safe_co2_min, breath.carbon_dioxide );
					H.throw_alert( "not_enough_co2", typeof(Obj_Screen_Alert_NotEnoughCo2) );
				} else {
					((dynamic)H).failed_last_breath = 0;
					H.adjustOxyLoss( -5 );
					gas_breathed = ( breath.carbon_dioxide ??0) / 6;
			...
			if ( Lang13.Bool( this.safe_toxins_max ) ) {
				...
				if ( ( Toxins_pp ??0) > ( this.safe_toxins_max ??0) && !this.specflags.Contains( 256 ) ) {
					ratio2 = ( breath.toxins ??0) / ( this.safe_toxins_max ??0) * 10;

					if ( H.reagents != null ) {
						H.reagents.add_reagent( "plasma", Num13.MaxInt( this.tox_breath_dam_min ?1:0, Num13.MinInt( ((int)( ratio2 )), this.tox_breath_dam_max ) ) );
					}
			...
			if ( Lang13.Bool( this.safe_toxins_min ) ) {
				...
				if ( ( Toxins_pp ??0) < ( this.safe_toxins_min ??0) && !this.specflags.Contains( 256 ) ) {
					gas_breathed = this.handle_too_little_breath( H, Toxins_pp, this.safe_toxins_min, breath.toxins );
					H.throw_alert( "not_enough_tox", typeof(Obj_Screen_Alert_NotEnoughTox) );
				} else {
					((dynamic)H).failed_last_breath = 0;
					H.adjustOxyLoss( -5 );
					gas_breathed = ( breath.toxins ??0) / 6;
			...
			if ( breath.trace_gases.len != 0 && !this.specflags.Contains( 256 ) ) {
				
				foreach (dynamic _a in Lang13.Enumerate( breath.trace_gases, typeof(Gas_SleepingAgent) )) {
					SA = _a;
					
					SA_pp = breath.get_breath_partial_pressure( SA.moles );
		...
		public bool spec_attack_hand( dynamic M = null, Mob_Living_Carbon_Human H = null ) {
			...
			if ( _a=="help" ) {
				...
				if ( attacker_style != null && attacker_style.harm_act( M, H ) ) {
					...
					damage = Rand13.Int( 0, 9 ) + M.dna.species.punchmod;
					...
					affecting = H.get_organ( GlobalFuncs.ran_zone( M.zone_sel.selecting ) );
				...
				if ( attacker_style != null && attacker_style.disarm_act( M, H ) ) {
					...
					affecting2 = H.get_organ( GlobalFuncs.ran_zone( M.zone_sel.selecting ) );
		...
		public double movement_delay( Mob_Living_Carbon_Human H = null ) {
			...
			dynamic J = null;
			Obj_Item_Weapon_Tank_Jetpack P = null;
			dynamic C = null;
			...
			if ( !( ( H.status_flags & 16 ) != 0 ) ) {
				grav = GlobalFuncs.has_gravity( H );
				hasjetpack = false;

				if ( !grav ) {
					
					if ( H.back is Obj_Item_Weapon_Tank_Jetpack ) {
						J = H.back;
					}

					if ( H.wear_suit is Obj_Item_Clothing_Suit_Space_Hardsuit ) {
						C = H.wear_suit;
						P = C.jetpack;
					}
					...
					if ( Lang13.Bool( J ) ) {
						...
						if ( ((Obj_Item_Weapon_Tank_Jetpack)J).allow_thrust( 0.01, H ) ) {
							hasjetpack = true;
						}
					} else if ( P != null ) {
		...
		// Function from file: species.dm
		public bool handle_hud_icons( Mob_Living_Carbon_Human H = null ) {
				...
				if ( H.stat == 2 ) {
					...
					switch ((int)( H.hal_screwyhud )) {
						...
						default:
							...
							dynamic _a = H.health - H.staminaloss; // Was a switch-case, sorry for the mess.
			...
			if ( H.healthdoll != null ) {
				...
				if ( H.stat == 2 ) {
					...
					foreach (dynamic _c in Lang13.Enumerate( H.organs, typeof(Obj_Item_Organ_Limb) )) {
						L = _c;
						...
						if ( icon_num != 0 ) {
							H.healthdoll.overlays.Add( new Image( "icons/mob/screen_gen.dmi", "" + L.name + icon_num ) );
						}
					}
				}
			}

			dynamic _d = H.nutrition; // Was a switch-case, sorry for the mess.
			if ( 550<=_d&&_d<=Double.PositiveInfinity ) {
				H.throw_alert( "nutrition", typeof(Obj_Screen_Alert_Fat) );
			} else if ( 250<=_d&&_d<=550 ) {
				H.clear_alert( "nutrition" );
			} else if ( 150<=_d&&_d<=250 ) {
		...
		// Function from file: species.dm
		public bool handle_vision( Mob H = null ) {
			int see_temp = 0;
			Obj_Item_Clothing_Glasses G = null;
			...
			if ( H.stat == 2 ) {
				...
				if ( !( ( ( H.permanent_sight_flags ?1:0) & 16 ) != 0 ) ) {
					H.sight &= 65519;
				}

				if ( !( ( ( H.permanent_sight_flags ?1:0) & 4 ) != 0 ) ) {
					H.sight &= 65531;
				}

				if ( !( ( ( H.permanent_sight_flags ?1:0) & 8 ) != 0 ) ) {
					H.sight &= 65527;
				}

				if ( H.remote_view ) {
					H.sight |= GlobalVars.SEE_TURFS;
					H.sight |= GlobalVars.SEE_MOBS;
					H.sight |= GlobalVars.SEE_OBJS;
				}
				H.see_in_dark = ( ( ( H.sight == GlobalVars.SEE_TURFS ?1:0) | 12 ) != 0 ? 8 : this.darksight );
				see_temp = H.see_invisible;
				H.see_invisible = Convert.ToInt32( this.invis_sight );

				if ( Lang13.Bool( ((dynamic)H).glasses ) ) {
					
					if ( ((dynamic)H).glasses is Obj_Item_Clothing_Glasses ) {
						G = ((dynamic)H).glasses;
						H.sight |= G.vision_flags;
						H.see_in_dark = Convert.ToInt32( G.darkness_view );
				...
				if ( Convert.ToDouble( ((dynamic)H).tinttotal ) >= 2 ) {
					
					if ( GlobalVars.tinted_weldhelh ) {
						
						if ( Convert.ToDouble( ((dynamic)H).tinttotal ) >= 3 ) {
							H.eye_blind = Num13.MaxInt( ((int)( H.eye_blind )), 1 );
						}

						if ( H.client != null ) {
							H.client.screen.Add( GlobalVars.global_hud.darkMask );
						}
					}
				}
				...
				if ( H.blind != null ) {
					...
					if ( H.eye_blind != 0 ) {
						H.throw_alert( "blind", typeof(Obj_Screen_Alert_Blind) );
						H.blind.layer = 18;
					} else {
						H.clear_alert( "blind" );
						H.blind.layer = 0;
				...
				if ( Lang13.Bool( H.disabilities & 8 ) && !( ((dynamic)H).glasses is Obj_Item_Clothing_Glasses_Regular ) ) {
					H.client.screen.Add( GlobalVars.global_hud.vimpaired );
				}

				if ( H.eye_blurry != 0 ) {
					H.client.screen.Add( GlobalVars.global_hud.blurry );
				}

				if ( H.druggy != 0 ) {
					H.client.screen.Add( GlobalVars.global_hud.druggy );
					H.throw_alert( "high", typeof(Obj_Screen_Alert_High) );
				} else {
					H.clear_alert( "high" );
				}
		...
		// Function from file: species.dm
		public void handle_chemicals_in_body( Mob_Living H = null ) {
			...
			if ( H.nutrition > 0 && H.stat != 2 && !((dynamic)H).dna.species.need_nutrition ) {
				hunger_rate = 0.1;
		...
		// Function from file: species.dm
		public virtual bool before_equip_job( Job J = null, dynamic H = null, bool? visualsOnly = null ) {
			return false;
		}
