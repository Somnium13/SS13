	...
	class Mob_Living_Silicon_Ai : Mob_Living_Silicon {
		...
		public override bool Life(  ) {
			...
			if ( this.stat == 2 ) {
				...
				} else if ( Lang13.Bool( this.getOxyLoss() ) ) {
					this.adjustOxyLoss( -1 );
				}
				...
				} else if ( !( this.aiRestorePowerRoutine != 0 ) ) {
					this.aiRestorePowerRoutine = 1;
					this.blind_eyes( 1 );
					this.update_sight();
					this.WriteMsg( "You've lost power!" );
					Task13.Schedule( 20, (Task13.Closure)(() => {
						this.start_RestorePowerRoutine();
						return;
					}));
				}
			}
		...
		public override bool death( bool? gibbed = null, bool? toast = null ) {
			...
			foreach (dynamic _b in Lang13.Enumerate( typeof(Game13), typeof(Obj_Machinery_AiStatusDisplay) )) {
				O = _b;
		...
		// Function from file: ai.dm
		public override void replace_identification_name( dynamic oldname = null, dynamic newname = null ) {
			
			if ( this.aiPDA != null ) {
				this.aiPDA.owner = newname;
				this.aiPDA.name = newname + " (" + this.aiPDA.ownjob + ")";
			}
			return;
		}
		...
		public override bool fully_replace_character_name( dynamic oldname = null, dynamic newname = null ) {
			...
			if ( oldname != this.real_name ) {
				...
				if ( this.eyeobj != null ) {
					this.eyeobj.name = "" + newname + " (AI Eye)";
				}
		...
		public void announcement(  ) {
			...
			words = GlobalFuncs.splittext( GlobalFuncs.trim( message ), " " );
		...
		public void holopad_talk( dynamic message = null ) {
			...
			if ( T is Obj_Machinery_Hologram_Holopad && Lang13.Bool( T.masters[this] ) ) {
				this.send_speech( message, 7, T, "robot", this.get_spans() );
		...
		// Function from file: life.dm
		public void start_RestorePowerRoutine(  ) {
			dynamic T = null;
			dynamic AIarea = null;
			...
			if ( Lang13.Bool( AIarea ) && AIarea.master.power_equip ) {
		...
		public bool lacks_power(  ) {
			...
			return !Lang13.Bool( T ) || !Lang13.Bool( A ) || ( !A.master.power_equip || T is Tile_Space ) && !GlobalFuncs.is_type_in_list( this.loc, new ByTable(new object [] { typeof(Obj_Item), typeof(Obj_Mecha) }) );
		}
		...
		// Function from file: ai.dm
		public void add_malf_picker(  ) {
			this.WriteMsg( "In the top right corner of the screen you will find the Malfunctions tab, where you can purchase various abilities, from upgraded surveillance to station ending doomsday devices." );
			this.WriteMsg( "You are also capable of hacking APCs, which grants you more points to spend on your Malfunction powers. The drawback is that a hacked APC will give you away if spotted by the crew. Hacking an APC takes 60 seconds." );
			this.view_core();
			this.verbs.Add( typeof(Mob_Living_Silicon_Ai).GetMethod( "choose_modules" ) );
			this.malf_picker = new ModulePicker();
			return;
		}
		...
		public void light_cameras(  ) {
			...
			foreach (dynamic _b in Lang13.Enumerate( this.eyeobj.visibleCameraChunks, typeof(Camerachunk) )) {
				...
				foreach (dynamic _a in Lang13.Enumerate( CC.cameras, typeof(Obj_Machinery_Camera) )) {
					...
					if ( !C.can_use() || Map13.GetDistance( C, this.eyeobj ) > 7 ) {
						continue;
					}
					visible.Or( C );
				}
		...
		// Function from file: ai.dm
		public void set_waypoint( Ent_Static A = null ) {
		...
		public void override_machine( Obj_Machinery M = null ) {
			...
			if ( M is Obj_Machinery ) {
				...
				if ( !M.can_be_overridden() ) {
					this.WriteMsg( "Can't override this device." );
				}
		...
		public void break_air_alarms(  ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( GlobalVars.machines, typeof(Obj_Machinery_Airalarm) )) {
				AA = _a;
		...
		public void upgrade_turrets(  ) {
			Obj_Machinery_PortaTurret_Ai turret = null;
			...
			foreach (dynamic _a in Lang13.Enumerate( GlobalVars.machines, typeof(Obj_Machinery_PortaTurret_Ai) )) {
				turret = _a;
