...
namespace Somnium.Game {
	class Mob_Living_SimpleAnimal_Slime : Mob_Living_SimpleAnimal {
		...
		public override dynamic adjustFireLoss( dynamic amount = null, bool? updating_health = null ) {
			base.adjustFireLoss( -Math.Abs( Convert.ToDouble( (object)amount ) ), updating_health );
			return null;
		}
		...
		// Function from file: slime.dm
		public override dynamic ObjBump( Ent_Static O = null ) {
			int probab = 0;
		...
		public override void handle_automated_speech( bool? _override = null ) {
			return;
		}
		...
		public void Feedstop( bool? silent = null, bool? living = null ) {
			silent = silent ?? false;
			living = living ?? true;
			...
			if ( this.buckled != null ) {
				...
				if ( !( living == true ) ) {
					this.WriteMsg( "<span class='warning'>" + Rand13.Pick(new object [] { "This subject is incompatible", "This subject does not have life energy", "This subject is empty", "I am not satisified", "I can not feed from this subject", "I do not feel nourished", "This subject is not food" }) + "!</span>" );
				}
		...
		public void handle_feeding(  ) {
			...
			if ( M is Mob_Living_Carbon ) {
				...
				SA = M;
				totaldamage = 0;
				totaldamage += Convert.ToDouble( ((Mob_Living)SA).adjustCloneLoss( Rand13.Int( 2, 4 ) ) );
				totaldamage += Convert.ToDouble( ((Mob_Living)SA).adjustToxLoss( Rand13.Int( 1, 2 ) ) );

				if ( totaldamage <= 0 ) {
					this.Feedstop( false, false );
					return;
				}
			} else {
				this.Feedstop( false, false );
