...
namespace Somnium.Game {
	class Mob_Camera_Blob : Mob_Camera {
		...
		public override bool update_health_hud( dynamic shown_health_amount = null ) {
			...
			if ( this.blob_core != null ) {
				this.hud_used.healths.maptext = "<div align='center' valign='middle' style='position:relative; top:0px; left:6px'><font color='#e36600'>" + Num13.Floor( this.blob_core.health ) + "</font></div>";
			}
			return false;
		}
		...
		public override dynamic Destroy(  ) {
			GlobalVars.overminds.Remove( this );
		...
		public override void create_mob_hud(  ) {
			...
			if ( this.client != null && !( this.hud_used != null ) ) {
				this.hud_used = new Hud_BlobOvermind( this );
			}
			return;
		}
		...
		// Function from file: powers.dm
		[Verb]
		[VerbInfo( name: "Create Blobbernaut (30)", desc: "Create a powerful blobbernaut which is mildly smart and will attack enemies.", group: "Blob" )]
		public void create_blobbernaut(  ) {
			...
			if ( Convert.ToDouble( B.health ) <= Convert.ToDouble( B.maxhealth * 0.8 ) ) {
			...
			((Obj_Effect_Blob)B).take_damage( B.maxhealth * 0.8, "clone", null, false );
			...
			GlobalFuncs.playsound( B.loc, "sound/effects/splat.ogg", 50, 1 );
			...
			if ( candidates.len != 0 ) {
				...
				blobber.WriteMsg( "You can communicate with other blobbernauts and overminds via <b>:b</b>" );
