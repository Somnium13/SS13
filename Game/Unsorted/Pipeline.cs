...
namespace Somnium.Game {
	class Pipeline : Game_Data {
		...
		public bool temperature_interact( Ent_Static target = null, int share_volume = 0, double thermal_conductivity = 0 ) {
			...
			partial_heat_capacity = total_heat_capacity * ( share_volume / ( this.air.volume ??0) );
		...
		public void temporarily_store_air(  ) {
			...
			foreach (dynamic _b in Lang13.Enumerate( this.members, typeof(Obj_Machinery_Atmospherics_Pipe) )) {
				...
				member.air_temporary.copy_from( this.air );
				member_gases = member.air_temporary.gases;

				foreach (dynamic _a in Lang13.Enumerate( member_gases )) {
					id = _a;
					
					member_gases[id][1] *= member.volume / ( this.air.volume ??0);
				}
