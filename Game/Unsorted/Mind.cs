	...
	class Mind : Game_Data {
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hsrc = null ) {
			...
			if ( Lang13.Bool( href_list["role_edit"] ) ) {
				...
				new_obj_type = Interface13.Input( "Select objective type:", "Objective type", def_value, null, new ByTable(new object [] { 
					"assassinate", 
					"maroon", 
					"debrain", 
					"protect", 
				...
				if ( _c=="assassinate" || _c=="protect" || _c=="debrain" || _c=="maroon" ) {
					...
					def_target = null;
					objective_list = new ByTable(new object [] { typeof(Objective_Assassinate), typeof(Objective_Protect), typeof(Objective_Debrain), typeof(Objective_Maroon) });
					...
					if ( new_target == "Free objective" ) {
						...
						((Objective)new_objective).update_explanation_text();
					}
				} else if ( _c=="destroy" ) {
					possible_targets2 = GlobalFuncs.active_ais( true );

					if ( possible_targets2.len != 0 ) {
						new_target2 = Interface13.Input( "Select target:", "Objective target", null, null, possible_targets2, InputType.Null | InputType.Any );
						new_objective = new Objective_Destroy();
						new_objective.target = new_target2.mind;
						new_objective.owner = this;
						((Objective)new_objective).update_explanation_text();
					} else {
						Task13.User.WriteMsg( "No active AIs with minds" );
					}
				} else if ( _c=="prevent" ) {
					...
					new_objective = new Objective_Martyr();
					new_objective.owner = this;
				} else if ( _c=="nuclear" ) {
				...
				} else if ( _c=="download" || _c=="capture" || _c=="absorb" ) {
					def_num = null;
					...
					if ( _b=="download" ) {
						new_objective = new Objective_Download();
				...
				} else if ( _c=="follower block (HOG)" ) {
					new_objective = new Objective_FollowerBlock();
					new_objective.owner = this;
				} else if ( _c=="build (HOG)" ) {
					new_objective = new Objective_Build();
					new_objective.owner = this;
				} else if ( _c=="deicide (HOG)" ) {
					new_objective = new Objective_Deicide();
					new_objective.owner = this;
				} else if ( _c=="follower escape (HOG)" ) {
					new_objective = new Objective_EscapeFollowers();
					new_objective.owner = this;
				} else if ( _c=="sacrifice prophet (HOG)" ) {
					new_objective = new Objective_SacrificeProphet();
					new_objective.owner = this;
				} else if ( _c=="custom" ) {
				...
				dynamic _d = href_list["handofgod"]; // Was a switch-case, sorry for the mess.
				if ( _d=="clear" ) {
				...
				dynamic _f = href_list["revolution"]; // Was a switch-case, sorry for the mess.
				if ( _f=="clear" ) {
					this.remove_rev();
					...
					if ( GlobalVars.ticker.mode.head_revolutionaries.len > 0 ) {
						...
						if ( Lang13.Bool( valid_head ) ) {
							...
							foreach (dynamic _e in Lang13.Enumerate( valid_head.objectives, typeof(Objective_Mutiny) )) {
								O = _e;
				...
				dynamic _k = href_list["gang"]; // Was a switch-case, sorry for the mess.
				if ( _k=="clear" ) {
					this.remove_gang();
				...
				} else if ( _k=="takeequip" ) {
					L3 = ((Mob)this.current).get_contents();

					foreach (dynamic _h in Lang13.Enumerate( L3, typeof(Obj_Item_Weapon_Pen_Gang) )) {
						pen = _h;
						
						GlobalFuncs.qdel( pen );
					}

					foreach (dynamic _i in Lang13.Enumerate( L3, typeof(Obj_Item_Device_Gangtool) )) {
						gangtool = _i;
						
						GlobalFuncs.qdel( gangtool );
					}

					foreach (dynamic _j in Lang13.Enumerate( L3, typeof(Obj_Item_Toy_Crayon_Spraycan_Gang) )) {
						SC = _j;
				...
				dynamic _l = href_list["cult"]; // Was a switch-case, sorry for the mess.
				if ( _l=="clear" ) {
					this.remove_cultist();
					...
					if ( !Lang13.Bool( ((GameMode)GlobalVars.ticker.mode).equip_cultist( this.current ) ) ) {
						Task13.User.WriteMsg( "<span class='danger'>equip_cultist() failed! " + this.current + "'s starting equipment will be incomplete.</span>" );
					}
				}
			} else if ( Lang13.Bool( href_list["wizard"] ) ) {
				
				dynamic _m = href_list["wizard"]; // Was a switch-case, sorry for the mess.
				if ( _m=="clear" ) {
					this.remove_wizard();
				...
				dynamic _n = href_list["changeling"]; // Was a switch-case, sorry for the mess.
				if ( _n=="clear" ) {
				...
				dynamic _p = href_list["nuclear"]; // Was a switch-case, sorry for the mess.
				if ( _p=="clear" ) {
					this.remove_nukeop();
					...
					foreach (dynamic _o in Lang13.Enumerate( GlobalVars.machines, typeof(Obj_Machinery_Nuclearbomb) )) {
						bombue = _o;
				...
				dynamic _q = href_list["traitor"]; // Was a switch-case, sorry for the mess.
				if ( _q=="clear" ) {
					this.remove_traitor();
				...
				dynamic _r = href_list["shadowling"]; // Was a switch-case, sorry for the mess.
				if ( _r=="clear" ) {
				...
				dynamic _s = href_list["abductor"]; // Was a switch-case, sorry for the mess.
				if ( _s=="clear" ) {
				...
				if ( Lang13.Bool( L4.notransform ) ) {
					return null;
				}

				dynamic _v = href_list["monkey"]; // Was a switch-case, sorry for the mess.
				if ( _v=="healthy" ) {
					...
					if ( GlobalFuncs.check_rights( 2 ) ) {
						...
						if ( H2 is Mob_Living_Carbon_Human ) {
							...
							foreach (dynamic _t in Lang13.Enumerate( M.viruses, typeof(Disease) )) {
								D = _t;
					...
					if ( GlobalFuncs.check_rights( 2, false ) ) {
						...
						if ( M3 is Mob_Living_Carbon_Monkey ) {
							...
							foreach (dynamic _u in Lang13.Enumerate( M3.viruses, typeof(Disease) )) {
								D2 = _u;
				...
				dynamic _x = href_list["silicon"]; // Was a switch-case, sorry for the mess.
				if ( _x=="unemag" ) {
					R = this.current;
				...
				} else if ( _x=="unemagcyborgs" ) {
					
					if ( this.current is Mob_Living_Silicon_Ai ) {
						ai = this.current;

						foreach (dynamic _w in Lang13.Enumerate( ai.connected_robots, typeof(Mob_Living_Silicon_Robot) )) {
							R2 = _w;
				...
				dynamic _z = href_list["common"]; // Was a switch-case, sorry for the mess.
				if ( _z=="undress" ) {
					
					foreach (dynamic _y in Lang13.Enumerate( this.current, typeof(Obj_Item) )) {
						W = _y;
						
						((Mob)this.current).unEquip( W, 1 );
					}
				} else if ( _z=="takeuplink" ) {
					this.take_uplink();
					this.memory = null;
					...
					if ( GlobalFuncs.check_rights( 8, false ) ) {
						U = this.find_syndicate_uplink();

						if ( U != null ) {
							crystals = Interface13.Input( "Amount of telecrystals for " + this.key, "Syndicate uplink", U.telecrystals, null, null, InputType.Num | InputType.Null );
				...
				foreach (dynamic _ba in Lang13.Enumerate( this.objectives, typeof(Objective) )) {
					objective4 = _ba;
		...
		public void transfer_actions( dynamic new_character = null ) {
			...
			if ( Lang13.Bool( this.current ) && this.current.actions != null ) {
		...
		// Function from file: mind.dm
		public void make_Nuke( dynamic spawnloc = null, dynamic nuke_code = null, bool? leader = null, int? telecrystals = null ) {
			leader = leader ?? false;
		...
		// Function from file: mind.dm
		public Obj_Item_Device_Uplink find_syndicate_uplink(  ) {
			ByTable L = null;
			Obj_Item I = null;
		...
		public void edit_memory(  ) {
			...
			if ( ( GlobalVars.ticker.mode.head_revolutionaries.Contains( this ) || GlobalVars.ticker.mode.traitors.Contains( this ) || GlobalVars.ticker.mode.syndicates.Contains( this ) ) && this.current is Mob_Living_Carbon_Human ) {
				...
				if ( U != null ) {
					...
					if ( GlobalFuncs.check_rights( 8, false ) ) {
						text += new Txt( ", <a href='?src=" ).Ref( this ).str( ";common=crystals'>" ).item( U.telecrystals ).str( "</a> TC" ).ToString();
					} else {
