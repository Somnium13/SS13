...
namespace Somnium.Game {
	class SurgeryStep_AddLimb : SurgeryStep {
		...
		public override int preop( dynamic user = null, Mob target = null, string target_zone = null, dynamic tool = null, Surgery surgery = null ) {
			...
			if ( Lang13.Bool( this.L ) ) {
				((Ent_Static)user).visible_message( "" + user + " begins to augment " + target + "'s " + GlobalFuncs.parse_zone( user.zone_selected ) + ".", "<span class ='notice'>You begin to augment " + target + "'s " + GlobalFuncs.parse_zone( user.zone_selected ) + "...</span>" );
			} else {
