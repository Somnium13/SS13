	...
	class Obj_Machinery_Computer_Rdconsole : Obj_Machinery_Computer {
		...
		public override dynamic interact( dynamic user = null, bool? flag1 = null ) {
			...
			else if (this.screen == 121)
			{
				dat += new Txt( "<A href='?src=" ).Ref( this ).str( ";menu=1.0'>Main Menu</A>" ).ToString();
					...
					foreach (dynamic _c in Lang13.Enumerate( this.files.known_tech )) {
						v2 = _c;
						
						T2 = this.files.known_tech[v2];
						dat += "" + T2.name + " ";
			...
			else if (this.screen == 1.5)
			{
				dat += new Txt( "<A href='?src=" ).Ref( this ).str( ";menu=1.0'>Main Menu</A>" ).ToString();
					...
					foreach (dynamic _e in Lang13.Enumerate( this.files.known_designs )) {
						v3 = _e;
						
						D = this.files.known_designs[v3];
						dat += "" + D.name + " ";
			...
			else if (this.screen == 2.2)
			{
				dat += new Txt( "<A href='?src=" ).Ref( this ).str( ";menu=1.0'>Main Menu</A><div class='statusDisplay'>" ).ToString();
					...
					foreach (dynamic _g in Lang13.Enumerate( temp_tech )) {
						...
						foreach (dynamic _f in Lang13.Enumerate( this.files.known_tech )) {
							v4 = _f;
							
							F = this.files.known_tech[v4];
			...
			else if (this.screen == 3.15)
			{
				dat += new Txt( "<A href='?src=" ).Ref( this ).str( ";menu=1.0'>Main Menu</A>" ).ToString();
					...
					foreach (dynamic _j in Lang13.Enumerate( this.files.known_designs )) {
						v5 = _j;
						
						D2 = this.files.known_designs[v5];
			...
			else if (this.screen == 4.15)
			{
				dat += new Txt( "<A href='?src=" ).Ref( this ).str( ";menu=1.0'>Main Menu</A>" ).ToString();
					...
					foreach (dynamic _o in Lang13.Enumerate( this.files.known_designs )) {
						v6 = _o;
						
						D4 = this.files.known_designs[v6];
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hsrc = null ) {
			...
			if ( Lang13.Bool( href_list["category"] ) ) {
				...
				this.t_disk.stored = this.files.known_tech[href_list["copy_tech_ID"]];
				this.screen = 1.2;
				...
				D = this.files.known_designs[href_list["copy_design_ID"]];

				if ( D != null ) {
					autolathe_friendly = true;

					foreach (dynamic _a in Lang13.Enumerate( D.materials )) {
						x = _a;
						

						if ( !new ByTable(new object [] { "$metal", "$glass" }).Contains( x ) ) {
							autolathe_friendly = false;
							D.category.Remove( "Imported" );
						}
					}

					if ( ( D.build_type & 14 ) != 0 ) {
						D.build_type = ( autolathe_friendly ? D.build_type | 4 : D.build_type );
						D.category.Or( "Imported" );
					}
							...
							if ( this.linked_destroy != null ) {
								...
								if ( !this.linked_destroy.hacked ) {
									...
									if ( Convert.ToDouble( ((dynamic)this.linked_destroy).loaded_item.reliability ) >= 99 - Convert.ToDouble( ((dynamic)this.linked_destroy).decon_mod * 3 ) || ((dynamic)this.linked_destroy).loaded_item.crit_fail ) {
										...
										foreach (dynamic _b in Lang13.Enumerate( temp_tech )) {
											...
											if ( Rand13.PercentChance( Convert.ToInt32( ((dynamic)this.linked_destroy).loaded_item.reliability ) ) ) {
												this.files.UpdateTech( T, temp_tech[T] );
											}
										}
									...
									if ( this.linked_lathe != null ) {
										...
										foreach (dynamic _c in Lang13.Enumerate( ((dynamic)this.linked_destroy).loaded_item.materials )) {
											material = _c;
								...
								foreach (dynamic _e in Lang13.Enumerate( this.linked_destroy.contents, typeof(Obj) )) {
									I = _e;
									

									foreach (dynamic _d in Lang13.Enumerate( I.contents )) {
										M = _d;
				...
				if ( !this.sync ) {
					...
					Task13.Schedule( 30, (Task13.Closure)(() => {
						...
						if ( this != null ) {
							...
							foreach (dynamic _j in Lang13.Enumerate( GlobalVars.machines, typeof(Obj_Machinery_RND_Server) )) {
								S2 = _j;
								...
								if ( S2.id_with_upload.Contains( this.id ) || S2 is Obj_Machinery_RND_Server_Centcom ) {
									...
									foreach (dynamic _f in Lang13.Enumerate( this.files.known_tech )) {
										...
										T2 = this.files.known_tech[v];
										S2.files.AddTech2Known( T2 );
									}

									foreach (dynamic _g in Lang13.Enumerate( this.files.known_designs )) {
										v2 = _g;
								...
								if ( S2.id_with_download.Contains( this.id ) && !( S2 is Obj_Machinery_RND_Server_Centcom ) || S2.hacked ) {
									...
									foreach (dynamic _h in Lang13.Enumerate( S2.files.known_tech )) {
										v3 = _h;
										
										T3 = S2.files.known_tech[v3];
										this.files.AddTech2Known( T3 );
									}

									foreach (dynamic _i in Lang13.Enumerate( S2.files.known_designs )) {
										v4 = _i;
										
										D3 = S2.files.known_designs[v4];
				...
				if ( this.linked_lathe != null ) {
					being_built = this.files.known_designs[href_list["build"]];
					...
					if ( being_built != null ) {
						...
						foreach (dynamic _k in Lang13.Enumerate( being_built.materials )) {
							M2 = _k;
						...
						if ( g2g ) {
							...
							foreach (dynamic _l in Lang13.Enumerate( being_built.materials )) {
								MAT = _l;
							...
							if ( !((MaterialContainer)((dynamic)this.linked_lathe).materials).has_materials( efficient_mats, amount ) ) {
								...
								foreach (dynamic _m in Lang13.Enumerate( being_built.reagents )) {
									R = _m;
							...
							if ( enough_materials ) {
								...
								foreach (dynamic _n in Lang13.Enumerate( being_built.reagents )) {
									R2 = _n;
				...
				if ( this.linked_imprinter != null ) {
					being_built2 = this.files.known_designs[href_list["imprint"]];
					...
					if ( being_built2 != null ) {
						...
						if ( g2g2 ) {
							...
							foreach (dynamic _q in Lang13.Enumerate( being_built2.materials )) {
								M4 = _q;
								...
								dynamic _p = M4; // Was a switch-case, sorry for the mess.
								if ( _p=="$glass" ) {
				...
				dynamic _r = href_list["lathe_ejectsheet"]; // Was a switch-case, sorry for the mess.
				if ( _r=="metal" ) {
					MAT2 = "$metal";
				} else if ( _r=="glass" ) {
					MAT2 = "$glass";
				...
				dynamic _s = href_list["imprinter_ejectsheet"]; // Was a switch-case, sorry for the mess.
				if ( _s=="glass" ) {
				...
				dynamic _t = href_list["disconnect"]; // Was a switch-case, sorry for the mess.
				if ( _t=="destroy" ) {
				...
				foreach (dynamic _u in Lang13.Enumerate( this.files.known_designs )) {
					v5 = _u;
					
					D4 = this.files.known_designs[v5];

					if ( !( ( D4.build_type & ( compare ??0) ) != 0 ) ) {
						continue;
					}

					if ( String13.FindIgnoreCase( D4.name, href_list["to_search"], 1, 0 ) != 0 ) {
						this.matching_designs.Add( D4 );
					}
				}
			}
			this.updateUsrDialog();
		...
		public void SyncRDevices(  ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( Map13.FetchInViewExcludeThis( this, 3 ), typeof(Obj_Machinery_RND) )) {
				...
				if ( D.linked_console != null || D.disabled || Lang13.Bool( D.panel_open ) ) {
					continue;
				}
