...
namespace Somnium.Game {
	class Obj_Item_Weapon_Locator : Obj_Item_Weapon {
		...
		protected override void __FieldInit() {
			...
			this.throw_speed = 4;
			this.throw_range = 20;
			this.starting_materials = new ByTable().Set( "$iron", 400 );
			this.w_type = 5;
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hclient = null, HtmlInterface currui = null ) {
			...
			if ( !Lang13.Bool( current_location ) || Convert.ToInt32( current_location.z ) == 2 ) {
				GlobalFuncs.to_chat( Task13.User, "The " + this + " is malfunctioning." );
				return null;
			}
				...
				if ( Lang13.Bool( href_list["refresh"] ) ) {
					...
					if ( Lang13.Bool( sr ) ) {
						...
						foreach (dynamic _b in Lang13.Enumerate( typeof(Game13), typeof(Obj_Item_Weapon_Implant_Tracking) )) {
							...
							if ( !W2.implanted || !( W2.loc is Organ_External || W2.loc is Mob ) ) {
								continue;
							} else {
								M = W2.loc;
							...
							if ( tr2.z == sr.z && Lang13.Bool( tr2 ) ) {
								...
								if ( Convert.ToDouble( direct2 ) < 20 ) {
									...
									this.temp += "" + W2.id + "-" + GlobalFuncs.dir2text( Map13.GetDistance( sr, tr2 ) ) + "-" + direct2 + "<BR>";
								}
							}
						}
						this.temp += new Txt( "<B>You are at [" ).item( sr.x - GlobalVars.WORLD_X_OFFSET[sr.z] ).str( "," ).item( sr.y - GlobalVars.WORLD_Y_OFFSET[sr.z] ).str( "," ).item( sr.z ).str( "]</B> in orbital coordinates.<BR><BR><A href='byond://?src=" ).Ref( this ).str( ";refresh=1'>Refresh</A><BR>" ).ToString();
					} else {
					...
					this.frequency += String13.ParseNumber( href_list["freq"] );
