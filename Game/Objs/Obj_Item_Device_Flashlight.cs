...
namespace Somnium.Game {
	class Obj_Item_Device_Flashlight : Obj_Item_Device {
		...
		public override bool attack( dynamic M = null, dynamic user = null, bool? def_zone = null ) {
			...
			if ( this.on && user.zone_selected == "eyes" ) {
				...
				if ( M == user ) {
					...
					if ( ((Mob_Living)M).flash_eyes( null, null, null, true ) ) {
					...
					if ( C is Mob_Living_Carbon ) {
						...
						if ( Convert.ToInt32( C.stat ) == 2 || Lang13.Bool( C.disabilities & 1 ) ) {
							user.WriteMsg( "<span class='warning'>" + C + " pupils don't react to the light!</span>" );
						} else if ( ((Dna)C.dna).check_mutation( "X Ray Vision" ) != 0 ) {
							user.WriteMsg( "<span class='danger'>" + C + " pupils give an eerie glow!</span>" );
						} else if ( ((Mob_Living)C).flash_eyes( null, null, null, true ) ) {
							user.WriteMsg( "<span class='notice'>" + C + "'s pupils narrow.</span>" );
						}
					}
				}
			} else {
