	...
	class Obj_Item_Weapon_Toy_XmasCracker : Obj_Item_Weapon_Toy {
		...
		public override bool? attack( dynamic M = null, dynamic user = null, string def_zone = null, bool? eat_override = null ) {
			...
			if ( !this.cracked && M is Mob_Living_Carbon_Human && Lang13.Bool( M.stat ) == false && !Lang13.Bool( ((Mob)M).get_active_hand() ) ) {
				((Ent_Static)M).visible_message( new Txt( "<span class='notice'>" ).item( user ).str( " and " ).item( M ).str( " pop " ).a( this ).item().str( "! *pop*</span>" ).ToString(), new Txt( "<span class='notice'>You pull " ).a( this ).item().str( " with " ).item( M ).str( "! *pop*</span>" ).ToString(), "<span class='notice'>You hear a *pop*.</span>" );
