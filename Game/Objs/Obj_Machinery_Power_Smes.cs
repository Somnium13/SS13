	...
	class Obj_Machinery_Power_Smes : Obj_Machinery_Power {
		...
		public Obj_Machinery_Power_Smes ( dynamic loc = null ) : base( (object)(loc) ) {
			...
			foreach (dynamic _a in Lang13.IterateRange( 1, 5 )) {
				...
				this.component_parts.Add( new Obj_Item_Weapon_StockParts_Cell_High_Empty( null ) );
			}
			...
			Task13.Schedule( 5, (Task13.Closure)(() => {
				...
				foreach (dynamic _c in Lang13.Enumerate( GlobalVars.cardinal )) {
					...
					foreach (dynamic _b in Lang13.Enumerate( T, typeof(Obj_Machinery_Power_Terminal) )) {
						term = _b;
		...
		public override double emp_act( int severity = 0 ) {
			...
			this.output_level = Rand13.Int( 0, Convert.ToInt32( this.output_level_max ) );
			this.input_level = Rand13.Int( 0, Convert.ToInt32( this.input_level_max ) );
		...
		public override int? ui_act( string action = null, ByTable _params = null, Tgui ui = null, UiState state = null ) {
			...
			switch ((string)( action )) {
				...
				case "input":
					...
					if ( target == "input" ) {
						target = Interface13.Input( "New input target (0-" + this.input_level_max + "):", this.name, this.input_level, null, null, InputType.Num | InputType.Null );
						...
						target = this.input_level_max;
						_default = GlobalVars.TRUE;
					} else if ( Lang13.Bool( adjust ) ) {
						target = this.input_level + ( adjust ??0);
						_default = GlobalVars.TRUE;
					...
					if ( Lang13.Bool( _default ) ) {
						this.input_level = Num13.MaxInt( 0, Num13.MinInt( Convert.ToInt32( target ), Convert.ToInt32( this.input_level_max ) ) );
						this.log_smes( Task13.User.ckey );
					}
					break;
				case "output":
					target2 = _params["target"];
					adjust2 = String13.ParseNumber( _params["adjust"] );

					if ( target2 == "input" ) {
						target2 = Interface13.Input( "New output target (0-" + this.output_level_max + "):", this.name, this.output_level, null, null, InputType.Num | InputType.Null );

						if ( !( target2 == null ) && !Lang13.Bool( base.ui_act( action, _params, ui, state ) ) ) {
							_default = GlobalVars.TRUE;
						}
					} else if ( target2 == "min" ) {
						target2 = 0;
						...
						target2 = this.output_level_max;
						_default = GlobalVars.TRUE;
					} else if ( Lang13.Bool( adjust2 ) ) {
						target2 = this.output_level + adjust2;
						_default = GlobalVars.TRUE;
					} else if ( String13.ParseNumber( target2 ) != null ) {
						target2 = String13.ParseNumber( target2 );
						_default = GlobalVars.TRUE;
					}

					if ( Lang13.Bool( _default ) ) {
						this.output_level = Num13.MaxInt( 0, Num13.MinInt( Convert.ToInt32( target2 ), Convert.ToInt32( this.output_level_max ) ) );
						this.log_smes( Task13.User.ckey );
					}
					break;
			}
		...
		public override int? process( dynamic seconds = null ) {
			...
			if ( this.terminal != null && this.input_attempt != 0 ) {
				...
				if ( this.inputting != 0 ) {
					...
					if ( Convert.ToDouble( this.input_available ) > 0 && Convert.ToDouble( this.input_available ) >= this.input_level ) {
						load = Num13.MinInt( ((int)( ( this.capacity - this.charge ) / 0.05 )), ((int)( this.input_level )) );
						this.charge += load * 0.05;
						this.add_load( load );
					} else {
						this.inputting = 0;
					}
				} else if ( this.input_attempt != 0 && Convert.ToDouble( this.input_available ) > 0 && Convert.ToDouble( this.input_available ) >= this.input_level ) {
					this.inputting = 1;
				}
			} else {
				this.inputting = 0;
		...
		public void log_smes( string user = null ) {
			...
			this.investigate_log( "input/output; " + ( this.input_level > Convert.ToDouble( this.output_level ) ? "<font color='green'>" : "<font color='red'>" ) + this.input_level + "/" + this.output_level + "</font> | Charge: " + this.charge + " | Output-mode: " + ( this.output_attempt != 0 ? "<font color='green'>on</font>" : "<font color='red'>off</font>" ) + " | Input-mode: " + ( this.input_attempt != 0 ? "<font color='green'>auto</font>" : "<font color='red'>off</font>" ) + " by " + user, "singulo" );
			return;
		}
		...
		public override dynamic attackby( dynamic A = null, dynamic user = null, string _params = null, bool? silent = null, bool? replace_spent = null ) {
			...
			if ( A is Obj_Item_Weapon_Wirecutters && this.terminal != null && Lang13.Bool( this.panel_open ) ) {
