...
namespace Somnium.Game {
	class Obj_Machinery_EmbeddedController_Radio_AdvancedAirlockController : Obj_Machinery_EmbeddedController_Radio {

		public Obj_Machinery_EmbeddedController_Radio_AdvancedAirlockController ( dynamic loc = null, int ndir = 0, bool? building = null ) : base( (object)(loc), ndir, building ) {
		...
		public override bool linkWith( Mob user = null, Base_Data buffer = null, ByTable context = null ) {
			...
			if ( S is Obj_Machinery_AirlockSensor ) {
				...
				if ( context["slot"] == "int" ) {
					this.tag_interior_sensor = ((dynamic)S).id_tag;
					((dynamic)buffer).master_tag = this.id_tag;
					return true;
				}

				if ( context["slot"] == "ext" ) {
					this.tag_exterior_sensor = ((dynamic)S).id_tag;
					((dynamic)buffer).master_tag = this.id_tag;
					return true;
				}
		...
		public override bool? isLinkedWith( Base_Data O = null ) {
			...
			if ( S is Obj_Machinery_AirlockSensor ) {
				...
				if ( this.tag_interior_sensor == ((dynamic)S).id_tag ) {
					return true;
				}

				if ( this.tag_exterior_sensor == ((dynamic)S).id_tag ) {
					return true;
				}
		...
		public override bool unlinkFrom( Mob user = null, Base_Data buffer = null ) {
			...
			if ( S is Obj_Machinery_AirlockSensor ) {
				...
				if ( this.tag_chamber_sensor == ((dynamic)S).id_tag ) {
					...
					return true;
				} else if ( this.tag_interior_sensor == ((dynamic)S).id_tag ) {
					this.tag_interior_sensor = null;
					return true;
				} else if ( this.tag_exterior_sensor == ((dynamic)S).id_tag ) {
					this.tag_exterior_sensor = null;
					return true;
				}
			}
		...
		public override bool canLink( Base_Data O = null, ByTable context = null ) {
			...
			if ( O is Obj_Machinery_AirlockSensor ) {
				Interface13.Stat( null, new ByTable(new object [] { "int", "ext", "chamber" }).Contains( context["slot"] ) );
		...
		public override void ui_interact( dynamic user = null, string ui_key = null, Nanoui ui = null, bool? force_open = null ) {
			...
			data = new ByTable()
				...
				.Set( "external_pressure", Num13.Floor( Convert.ToDouble( this.program.memory["external_sensor_pressure"] ) ) )
				.Set( "internal_pressure", Num13.Floor( Convert.ToDouble( this.program.memory["internal_sensor_pressure"] ) ) )
				.Set( "processing", this.program.memory["processing"] )
				.Set( "purge", this.program.memory["purge"] )
				.Set( "secure", this.program.memory["secure"] )
			;
			ui = GlobalVars.nanomanager.try_update_ui( user, this, ui_key, ui, data );

			if ( !( ui != null ) ) {
				ui = new Nanoui( user, this, ui_key, "advanced_airlock_console.tmpl", this.name, 470, 290 );
		...
		<b>Sensors:</b>
		<ul>
		<li>" ).item( this.format_tag( "Chamber", "tag_chamber_sensor" ) ).str( "</li>\n		<li>" ).item( this.format_tag( "Interior", "tag_interior_sensor" ) ).str( "</li>\n		<li>" ).item( this.format_tag( "Exterior", "tag_exterior_sensor" ) ).str( "</li>\n		</ul>" ).ToString();
		}
