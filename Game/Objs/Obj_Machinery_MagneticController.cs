	...
	class Obj_Machinery_MagneticController : Obj_Machinery {
		...
		public Obj_Machinery_MagneticController ( dynamic loc = null ) : base( (object)(loc) ) {
			...
			Task13.Schedule( 45, (Task13.Closure)(() => {
				...
				if ( GlobalVars.radio_controller != null ) {
					this.radio_connection = GlobalVars.radio_controller.add_object( this, this.frequency, GlobalVars.RADIO_MAGNETS );
				}
				return;
			}));
		...
		public void MagnetMove(  ) {
			...
			while (this.moving && this.rpath.len >= 1) {
				...
				((dynamic)signal).frequency = this.frequency;
				((dynamic)signal).data["code"] = this.code;
				...
				((dynamic)signal).data["command"] = nextmove;
				this.pathpos++;
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hclient = null, HtmlInterface currui = null ) {
			...
			if ( Lang13.Bool( href_list["radio-op"] ) ) {
				...
				if ( _a=="togglepower" ) {
					((dynamic)signal).data["command"] = "toggle-power";
				} else if ( _a=="minuselec" ) {
					((dynamic)signal).data["command"] = "sub-elec";
				} else if ( _a=="pluselec" ) {
					((dynamic)signal).data["command"] = "add-elec";
				} else if ( _a=="minusmag" ) {
					((dynamic)signal).data["command"] = "sub-mag";
				} else if ( _a=="plusmag" ) {
					((dynamic)signal).data["command"] = "add-mag";
				}
				this.radio_connection.post_signal( this, signal, GlobalVars.RADIO_MAGNETS );
		...
		public override dynamic attack_hand( dynamic a = null, dynamic b = null, dynamic c = null ) {
			...
			dat += new Txt( "<br>Speed: <a href='?src=" ).Ref( this ).str( ";operation=minusspeed'>-</a> " ).item( this.speed ).str( " <a href='?src=" ).Ref( this ).str( ";operation=plusspeed'>+</a><br>\n			Path: {<a href='?src=" ).Ref( this ).str( ";operation=setpath'>" ).item( this.path ).str( "</a>}<br>\n			Moving: <a href='?src=" ).Ref( this ).str( ";operation=togglemoving'>" ).item( ( this.moving ? "Enabled" : "Disabled" ) ).str( "</a>" ).ToString();
