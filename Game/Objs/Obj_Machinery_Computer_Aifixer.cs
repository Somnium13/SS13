...
namespace Somnium.Game {
	class Obj_Machinery_Computer_Aifixer : Obj_Machinery_Computer {
		...
		public Ent_Static occupier = null;
		...
		public override bool transfer_ai( dynamic interaction = null, dynamic user = null, Ent_Static AI = null, dynamic card = null ) {
			...
			if ( interaction == 2 ) {
				...
				((dynamic)AI).WriteMsg( "You have been uploaded to a stationary terminal. Sadly, there is no remote access from here." );
				...
				card.AI = null;
				this.update_icon();
			} else if ( this.occupier != null && !this.active ) {
				((dynamic)this.occupier).WriteMsg( "You have been downloaded to a mobile storage device. Still no remote access." );
		...
		public override bool update_icon( dynamic new_state = null, dynamic new_icon = null, int? new_px = null, int? new_py = null ) {
			...
			if ( ( this.stat & 3 ) != 0 ) {
				...
				if ( this.occupier != null ) {
					...
					dynamic _a = ((dynamic)this.occupier).stat; // Was a switch-case, sorry for the mess.
					if ( _a==0 ) {
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hsrc = null ) {
			...
			if ( Lang13.Bool( href_list["fix"] ) ) {
				...
				while (Convert.ToDouble( ((dynamic)this.occupier).health ) < 100) {
					((dynamic)this.occupier).adjustOxyLoss( -1, 0 );
					((dynamic)this.occupier).adjustFireLoss( -1, 0 );
					((dynamic)this.occupier).adjustToxLoss( -1, 0 );
					((dynamic)this.occupier).adjustBruteLoss( -1, 0 );
					((dynamic)this.occupier).updatehealth();

					if ( Convert.ToDouble( ((dynamic)this.occupier).health ) >= 0 && Convert.ToInt32( ((dynamic)this.occupier).stat ) == 2 ) {
						((dynamic)this.occupier).stat = 0;
						((dynamic)this.occupier).adjust_blindness( -1 );
						((dynamic)this.occupier).lying = 0;
		...
		public override dynamic interact( dynamic user = null, bool? flag1 = null ) {
			...
			if ( this.occupier != null ) {
				...
				dat += "<b>System integrity:</b> " + ( ((dynamic)this.occupier).health + 100 ) / 2 + "%<br>";

				if ( Lang13.Bool( ((dynamic)this.occupier).laws.zeroth ) ) {
					laws += "<b>0:</b> " + ((dynamic)this.occupier).laws.zeroth + "<BR>";
				}
				index = null;
				index = 1;

				while (( index ??0) <= ((dynamic)this.occupier).laws.ion.len) {
					law = ((dynamic)this.occupier).laws.ion[index];
				...
				while (( index2 ??0) <= ((dynamic)this.occupier).laws.inherent.len) {
					law2 = ((dynamic)this.occupier).laws.inherent[index2];
				...
				while (( index3 ??0) <= ((dynamic)this.occupier).laws.supplied.len) {
					law3 = ((dynamic)this.occupier).laws.supplied[index3];
				...
				if ( Convert.ToInt32( ((dynamic)this.occupier).stat ) == 2 ) {
		...
		public override dynamic attackby( dynamic A = null, dynamic user = null, string _params = null, bool? silent = null, bool? replace_spent = null ) {
			...
			if ( this.occupier != null && A is Obj_Item_Weapon_Screwdriver ) {
