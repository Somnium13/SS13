...
namespace Somnium.Game {
	class Obj_Item_Organ_Internal_Shadowtumor : Obj_Item_Organ_Internal {
		...
		public override int? process( dynamic seconds = null ) {
			Ent_Static T = null;
			dynamic light_count = null;

			
			if ( this.loc is Tile ) {
				T = this.loc;
				light_count = ((dynamic)T).get_lumcount();

				if ( Convert.ToDouble( light_count ) > 4 && this.health > 0 ) {
					this.health--;
				} else if ( Convert.ToDouble( light_count ) < 2 && this.health < 3 ) {
					this.health++;
				}
