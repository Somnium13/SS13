	...
	class Obj_Machinery_Recharger : Obj_Machinery {
		...
		public override int? process( dynamic seconds = null ) {
			...
			if ( Lang13.Bool( this.charging ) ) {
				...
				if ( this.charging is Obj_Item_AmmoBox_Magazine_Recharge ) {
					R = this.charging;

					if ( R.stored_ammo.len < ( R.max_ammo ??0) ) {
						R.stored_ammo.Add( Lang13.Call( R.ammo_type, R ) );
						this.f_use_power( this.recharge_coeff * 200 );
						using_power = true;
					}
				}
			}
			this.update_icon( using_power );
			return null;
		}
		...
		public override dynamic attackby( dynamic A = null, dynamic user = null, string _params = null, bool? silent = null, bool? replace_spent = null ) {
			...
			if ( A is Obj_Item_Weapon_Gun_Energy || A is Obj_Item_Weapon_Melee_Baton || A is Obj_Item_AmmoBox_Magazine_Recharge ) {
				
				if ( Lang13.Bool( this.anchored ) ) {
					
					if ( Lang13.Bool( this.charging ) || Lang13.Bool( this.panel_open ) ) {
						return null;
					}
