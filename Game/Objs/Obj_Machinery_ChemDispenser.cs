	...
	class Obj_Machinery_ChemDispenser : Obj_Machinery {
		...
		public override int? ui_act( string action = null, ByTable _params = null, Tgui ui = null, UiState state = null ) {
			...
			switch ((string)( action )) {
				case "amount":
					target = String13.ParseNumber( _params["target"] );

					if ( this.beaker.possible_transfer_amounts.Contains( target ) ) {
						this.amount = target;
						_default = GlobalVars.TRUE;
					}
					break;
				case "dispense":
					reagent = _params["reagent"];

					if ( Lang13.Bool( this.beaker ) && Lang13.Bool( this.dispensable_reagents.Find( reagent ) ) ) {
						R = this.beaker.reagents;
						free = R.maximum_volume - R.total_volume;
						actual = Num13.MinInt( ((int)( this.amount ??0 )), this.energy * 10, Convert.ToInt32( free ) );
						R.add_reagent( reagent, actual );
						this.energy = Num13.MaxInt( ((int)( this.energy - ( actual ??0) / 10 )), 0 );
						_default = GlobalVars.TRUE;
					}
					break;
				case "remove":
					...
					if ( this.beaker.possible_transfer_amounts.Contains( Lang13.Bool( this.beaker ) && Lang13.Bool( amount ) ) ) {
						((Reagents)this.beaker.reagents).remove_all( amount );
						_default = GlobalVars.TRUE;
					}
					break;
				case "eject":
