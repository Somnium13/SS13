...
namespace Somnium.Game {
	class Obj_Mecha : Obj {
		...
		public bool thrusters_active = false;
		public int? defence_mode = 0;
		public int defence_mode_deflect_chance = 35;
		public int? leg_overload_mode = 0;
		public int leg_overload_coeff = 2;
		...
		public int occupant_sight_flags = 0;
		...
		public override dynamic Topic( string href = null, ByTable href_list = null, dynamic hsrc = null ) {
			...
			if ( Lang13.Bool( href_list["rmictoggle"] ) ) {
				this.radio.broadcasting = !this.radio.broadcasting;
				GlobalFuncs.send_byjax( this.occupant, "exosuit.browser", "rmicstate", ( this.radio.broadcasting ? "Engaged" : "Disengaged" ) );
			}

			if ( Lang13.Bool( href_list["rspktoggle"] ) ) {
				this.radio.listening = !this.radio.listening;
				GlobalFuncs.send_byjax( this.occupant, "exosuit.browser", "rspkstate", ( this.radio.listening ? "Engaged" : "Disengaged" ) );
			}

			if ( Lang13.Bool( href_list["rfreq"] ) ) {
				new_frequency = ( this.radio.frequency ??0) + Convert.ToDouble( filter.getNum( "rfreq" ) );

				if ( !Lang13.Bool( this.radio.freerange ) || ( this.radio.frequency ??0) < 1200 || ( this.radio.frequency ??0) > 1600 ) {
			...
			if ( Lang13.Bool( href_list["dna_lock"] ) ) {
				...
				if ( this.occupant != null && !( this.occupant is Mob_Living_Carbon ) ) {
					((dynamic)this.occupant).WriteMsg( "<span class='danger'> You do not have any DNA!</span>" );
					return null;
				}
				this.dna_lock = ((dynamic)this.occupant).dna.unique_enzymes;
		...
		public override bool update_remote_sight( Mob user = null ) {
			...
			if ( this.occupant_sight_flags != 0 ) {
				
				if ( user == this.occupant ) {
					user.sight |= this.occupant_sight_flags;
				}
			}
			return false;
		}
		...
		public override void Exited( Ent_Dynamic Obj = null, Ent_Static newloc = null ) {
			...
			if ( this.occupant != null && this.occupant == Obj ) {
		...
		public override bool MouseDrop_T( Ent_Static dropping = null, Mob user = null ) {
			...
			if ( !user.canUseTopic( this ) || user != dropping ) {
				return false;
			}
			...
			if ( Lang13.Bool( this.dna_lock ) ) {
				...
				if ( user.has_dna() != null ) {
					...
					if ( ((dynamic)C).dna.unique_enzymes == this.dna_lock ) {
						passed = true;
					}
				}
		...
		public override bool transfer_ai( dynamic interaction = null, dynamic user = null, Ent_Static AI = null, dynamic card = null ) {
			...
			if ( _a==1 ) {
				...
				if ( !( AI != null ) || !( this.occupant is Mob_Living_Silicon_Ai ) ) {
					user.WriteMsg( "<span class='warning'>No AI detected in the " + this.name + " onboard computer.</span>" );
					return false;
				}

				if ( Lang13.Bool( ((dynamic)AI).mind.special_role ) ) {
					user.WriteMsg( "<span class='boldannounce'>ACCESS DENIED.</span>" );
					return false;
				}
				((dynamic)AI).aiRestorePowerRoutine = 0;
				((dynamic)AI).control_disabled = 1;
				((dynamic)AI).radio_enabled = 0;
				AI.loc = card;
				card.AI = AI;
				this.occupant = null;
				((dynamic)AI).controlled_mech = null;
				((dynamic)AI).remote_control = null;
				this.icon_state = Lang13.Initial( this, "icon_state" ) + "-open";
				((dynamic)AI).WriteMsg( "You have been downloaded to a mobile storage device. Wireless connection offline." );
				...
				if ( this.occupant != null ) {
					((dynamic)AI).WriteMsg( "<span class='warning'>Pilot detected! Forced ejection initiated!" );
					((dynamic)this.occupant).WriteMsg( "<span class='danger'>You have been forcibly ejected!</span>" );
					this.go_out( true );
				}
				...
				if ( !( AI != null ) ) {
					...
					return false;
				} else if ( Lang13.Bool( ((dynamic)AI).stat ) || !Lang13.Bool( ((dynamic)AI).client ) ) {
					user.WriteMsg( "<span class='warning'>" + AI.name + " is currently unresponsive, and cannot be uploaded.</span>" );
					return false;
				} else if ( this.occupant != null || Lang13.Bool( this.dna_lock ) ) {
					user.WriteMsg( "<span class='warning'>Access denied. " + this.name + " is " + ( this.occupant != null ? "currently occupied" : "secured with a DNA lock" ) + "." );
					return false;
				}
			...
			if ( this.phasing && this.get_charge() >= this.phasing_energy_drain && !this.throwing ) {
				Task13.Schedule( 0, (Task13.Closure)(() => {
					...
					if ( this.can_move ) {
						...
						if ( Lang13.Bool( this.phase_state ) ) {
							Icon13.Flick( this.phase_state, this );
						}
				...
				if ( Obstacle is Obj ) {
					...
					if ( !Lang13.Bool( ((dynamic)O).anchored ) ) {
						Map13.Step( (Ent_Dynamic)(Obstacle), this.dir );
					}
		...
		public override int Process_Spacemove( dynamic movement_dir = null ) {
			...
			_default = base.Process_Spacemove( (object)(movement_dir) );

			if ( _default != 0 ) {
				return 1;
			}

			if ( this.thrusters_active && Lang13.Bool( movement_dir ) && this.use_power( this.step_energy_drain ) ) {
				return 1;
			}
			...
			if ( Lang13.Bool( backup ) ) {
				...
				if ( backup is Ent_Dynamic && Lang13.Bool( movement_dir ) && !Lang13.Bool( backup.anchored ) ) {
					...
					if ( ((Ent_Dynamic)backup).newtonian_move( Num13.Rotate( movement_dir, 180 ) ) ) {
						...
						if ( this.occupant != null ) {
							((dynamic)this.occupant).WriteMsg( "<span class='info'>You push off of " + backup + " to propel yourself.</span>" );
						}
					}
				}
				return 1;
		...
		public override string Hear( string message = null, dynamic speaker = null, int message_langs = 0, dynamic raw_message = null, dynamic radio_freq = null, ByTable spans = null ) {
			...
			if ( speaker == this.occupant ) {
				...
				Task13.Schedule( 0, (Task13.Closure)(() => {
					GlobalFuncs.flick_overlay( new Image( "icons/mob/talk.dmi", this, "machine" + GlobalFuncs.say_test( raw_message ), 5 ), speech_bubble_recipients, 30 );
					return;
				}));
			}
			return null;
			...
			if ( this.internal_damage != 0 ) {
				...
				if ( ( this.internal_damage & 1 ) != 0 ) {
					...
					if ( this.internal_tank != null ) {
						...
						if ( int_tank_air.return_pressure() > this.internal_tank.maximum_pressure && !( ( this.internal_damage & 8 ) != 0 ) ) {
			...
			if ( this.occupant != null ) {
				...
				if ( Lang13.Bool( this.cell ) ) {
					...
					if ( 0.75<=_a&&_a<=Double.PositiveInfinity ) {
						((dynamic)this.occupant).clear_alert( "charge" );
				...
				if ( 30<=_b&&_b<=45 ) {
					...
					((dynamic)this.occupant).clear_alert( "mech damage" );
				}
		...
		// Function from file: mecha_topic.dm
		public string output_maintenance_dialog( dynamic id_card = null, dynamic user = null ) {
			...
			Interface13.Browse( user, _default, "window=exosuit_maint_console" );
			GlobalFuncs.onclose( user, "exosuit_maint_console" );
			return _default;
		}

		// Function from file: mecha_topic.dm
		public string output_access_dialog( dynamic id_card = null, bool user = false ) {
			string _default = null;
			...
			foreach (dynamic _a in Lang13.Enumerate( this.operation_req_access )) {
				...
				_default += new Txt().item( GlobalFuncs.get_access_desc( a ) ).str( " - <a href='?src=" ).Ref( this ).str( ";del_req_access=" ).item( a ).str( ";user=" ).Ref( user ).str( ";id_card=" ).Ref( id_card ).str( "'>Delete</a><br>" ).ToString();
			}
			_default += "<hr><h1>Following keycodes were detected on portable device:</h1>";
			...
			foreach (dynamic _b in Lang13.Enumerate( id_card.access )) {
				...
				_default += new Txt().item( a_name ).str( " - <a href='?src=" ).Ref( this ).str( ";add_req_access=" ).item( a2 ).str( ";user=" ).Ref( user ).str( ";id_card=" ).Ref( id_card ).str( "'>Add</a><br>" ).ToString();
			}
			_default += new Txt( "<hr><a href='?src=" ).Ref( this ).str( ";finish_req_access=1;user=" ).Ref( user ).str( "'>Finish</a> <span class='danger'>(Warning! The ID upload panel will be locked. It can be unlocked only through Exosuit Interface.)</span>" ).ToString();
			_default += "</body></html>";
			Interface13.Browse( user, _default, "window=exosuit_add_access" );
		...
		public string get_log_html(  ) {
			...
			_default = "<html><head><title>" + this.name + " Log</title></head><body style='font: 13px 'Courier', monospace;'>";

			foreach (dynamic _a in Lang13.Enumerate( this.log, typeof(ByTable) )) {
				entry = _a;
				
				_default += "<div style='font-weight: bold;'>" + entry["time"] + " " + String13.FormatTime( Convert.ToDouble( entry["date"] ), "MMM DD" ) + " " + entry["year"] + "</div>\n						<div style='margin-left:15px; margin-bottom:10px;'>" + entry["message"] + "</div>\n						";
			}
			_default += "</body></html>";
			return _default;
		}
		...
		public virtual string get_equipment_list(  ) {
			...
			_default = "<b>Equipment:</b><div style=\"margin-left: 15px;\">";

			foreach (dynamic _a in Lang13.Enumerate( this.equipment, typeof(Obj_Item_MechaParts_MechaEquipment) )) {
				MT = _a;
				
				_default += new Txt( "<div id='" ).Ref( MT ).str( "'>" ).item( MT.get_equip_info() ).str( "</div>" ).ToString();
			}
			_default += "</div>";
			return _default;
		}
		...
		// Function from file: mecha_topic.dm
		public string get_equipment_menu(  ) {
			string _default = null;
			...
			if ( this.equipment.len != 0 ) {
				_default += "<div class='wr'>\n						<div class='header'>Equipment</div>\n						<div class='links'>";

				foreach (dynamic _a in Lang13.Enumerate( this.equipment, typeof(Obj_Item_MechaParts_MechaEquipment) )) {
					W = _a;
					
					_default += new Txt().item( W.name ).str( " <a href='?src=" ).Ref( W ).str( ";detach=1'>Detach</a><br>" ).ToString();
				}
				_default += "<b>Available equipment slots:</b> " + ( this.max_equip - this.equipment.len );
				_default += "</div></div>";
			}
			return _default;
		}
		...
		public virtual string get_commands(  ) {
			...
			_default = new Txt( @"<div class='wr'>
						<div class='header'>Electronics</div>
						<div class='links'>
						<b>Radio settings:</b><br>
						Microphone: <a href='?src=" ).Ref( this ).str( ";rmictoggle=1'><span id=\"rmicstate\">" ).item( ( this.radio.broadcasting ? "Engaged" : "Disengaged" ) ).str( "</span></a><br>\n						Speaker: <a href='?src=" ).Ref( this ).str( ";rspktoggle=1'><span id=\"rspkstate\">" ).item( ( this.radio.listening ? "Engaged" : "Disengaged" ) ).str( "</span></a><br>\n						Frequency:\n						<a href='?src=" ).Ref( this ).str( ";rfreq=-10'>-</a>\n						<a href='?src=" ).Ref( this ).str( ";rfreq=-2'>-</a>\n						<span id=\"rfreq\">" ).item( GlobalFuncs.format_frequency( this.radio.frequency ) ).str( "</span>\n						<a href='?src=" ).Ref( this ).str( ";rfreq=2'>+</a>\n						<a href='?src=" ).Ref( this ).str( @";rfreq=10'>+</a><br>
						</div>
						</div>
						<div class='wr'>
		...
		public virtual string get_stats_part(  ) {
			...
			_default = "" + this.report_internal_damage() + "\n						" + ( integrity < 30 ? "<span class='userdanger'>DAMAGE LEVEL CRITICAL</span><br>" : null ) + "\n						<b>Integrity: </b> " + integrity + "%<br>\n						<b>Powercell charge: </b>" + ( cell_charge == null ? "No powercell installed" : "" + ((Obj_Item_Weapon_StockParts_Cell)this.cell).percent() + "%" ) + "<br>\n						<b>Air source: </b>" + ( this.use_internal_tank ? "Internal Airtank" : "Environment" ) + "<br>\n						<b>Airtank pressure: </b>" + tank_pressure + "kPa<br>\n						<b>Airtank temperature: </b>" + tank_temperature + "&deg;K|" + ( tank_temperature - 273.41 ) + "&deg;C<br>\n						<b>Cabin pressure: </b>" + ( cabin_pressure > 325 ? ((dynamic)( "<span class='danger'>" + cabin_pressure + "</span>" )) : ((dynamic)( cabin_pressure )) ) + "kPa<br>\n						<b>Cabin temperature: </b> " + this.return_temperature() + "&deg;K|" + ( this.return_temperature() - 273.41 ) + "&deg;C<br>\n						" + ( Lang13.Bool( this.dna_lock ) ? new Txt( "<b>DNA-locked:</b><br> <span style='font-size:10px;letter-spacing:-1px;'>" ).item( this.dna_lock ).str( "</span> [<a href='?src=" ).Ref( this ).str( ";reset_dna=1'>Reset</a>]<br>" ).ToString() : "" ) + "<br>\n						" + ( Lang13.Bool( this.thrusters_action.owner ) ? "<b>Thrusters: </b> " + ( this.thrusters_active ? "Enabled" : "Disabled" ) + "<br>" : "" ) + "\n						" + ( Lang13.Bool( this.defense_action.owner ) ? "<b>Defence Mode: </b> " + ( Lang13.Bool( this.defence_mode ) ? "Enabled" : "Disabled" ) + "<br>" : "" ) + "\n						" + ( Lang13.Bool( this.overload_action.owner ) ? "<b>Leg Actuators Overload: </b> " + ( Lang13.Bool( this.leg_overload_mode ) ? "Enabled" : "Disabled" ) + "<br>" : "" ) + "\n						" + ( Lang13.Bool( this.smoke_action.owner ) ? "<b>Smoke: </b> " + this.smoke + "<br>" : "" ) + "\n						" + ( Lang13.Bool( this.zoom_action.owner ) ? "<b>Zoom: </b> " + ( this.zoom_mode ? "Enabled" : "Disabled" ) + "<br>" : "" ) + "\n						" + ( Lang13.Bool( this.switch_damtype_action.owner ) ? "<b>Damtype: </b> " + this.damtype + "<br>" : "" ) + "\n						" + ( Lang13.Bool( this.phasing_action.owner ) ? "<b>Phase Modulator: </b> " + ( this.phasing ? "Enabled" : "Disabled" ) + "<br>" : "" ) + "\n					";
			return _default;
		}

		// Function from file: mecha_topic.dm
		public string report_internal_damage(  ) {
			string _default = null;
			...
			foreach (dynamic _a in Lang13.Enumerate( dam_reports )) {
				...
				if ( ( this.internal_damage & ((int)( intdamflag ??0 )) ) != 0 ) {
					_default += dam_reports[tflag];
					_default += "<br />";
				}
			}

			if ( this.return_pressure() > 325 ) {
				_default += "<span class='userdanger'>DANGEROUSLY HIGH CABIN PRESSURE</span><br />";
			}
			return _default;
		}
		...
		// Function from file: mecha.dm
		public virtual void RemoveActions( Ent_Static user = null, bool? human_occupant = null ) {
		...
		// Function from file: mecha.dm
		public virtual void GrantActions( Ent_Static user = null, bool? human_occupant = null ) {
			human_occupant = human_occupant ?? false;
		...
		public void occupant_message( string message = null ) {
			...
			if ( Lang13.Bool( message ) ) {
				...
				if ( this.occupant != null && Lang13.Bool( ((dynamic)this.occupant).client ) ) {
					((dynamic)this.occupant).WriteMsg( new Txt().icon( this ).str( " " ).item( message ).ToString() );
				}
			}
			return;
		}
		...
		// Function from file: mecha.dm
		public bool operation_allowed( Mob M = null ) {
		...
		public virtual void go_out( bool? forced = null, Ent_Static newloc = null ) {
			...
			if ( Lang13.Bool( ((dynamic)mob_container).forceMove( newloc ) ) ) {
				...
				if ( mob_container is Obj_Item_Device_Mmi ) {
					...
					if ( Lang13.Bool( ((dynamic)mmi).brainmob ) ) {
						L.loc = mmi;
						((dynamic)L).reset_perspective();
					}
					((dynamic)mmi).mecha = null;
					((dynamic)mmi).update_icon();
					((dynamic)L).canmove = 0;
				}
				this.icon_state = Lang13.Initial( this, "icon_state" ) + "-open";
			...
			if ( L != null && Lang13.Bool( ((dynamic)L).client ) ) {
				((dynamic)L).client.view = Convert.ToInt32( Game13.view );
		...
		public virtual bool mmi_moved_inside( dynamic mmi_as_oc = null, dynamic user = null ) {
			...
			if ( Map13.FetchInRange( null, 1 ).Contains( Lang13.Bool( mmi_as_oc ) && Lang13.Bool( user ) ) ) {
				...
				brainmob.loc = this;
				((dynamic)brainmob).reset_perspective( this );
				...
				if ( !( this.internal_damage != 0 ) ) {
					((dynamic)this.occupant).WriteMsg( new Sound( "sound/mecha/nominal.ogg", null, null, null, 50 ) );
				}
		...
		// Function from file: mecha.dm
		public void aimob_exit_mech( Mob_Living_SimpleAnimal_Hostile_Syndicate_MechaPilot pilot_mob = null ) {
			
			if ( this.occupant == pilot_mob ) {
				this.occupant = null;
			}

			if ( pilot_mob.mecha == this ) {
				pilot_mob.mecha = null;
			}
			this.icon_state = "" + Lang13.Initial( this, "icon_state" ) + "-open";
			pilot_mob.forceMove( GlobalFuncs.get_turf( this ) );
			this.RemoveActions( pilot_mob );
			return;
		}

		// Function from file: mecha.dm
		public void aimob_enter_mech( Mob_Living_SimpleAnimal_Hostile_Syndicate_MechaPilot pilot_mob = null ) {
			
			if ( pilot_mob != null && pilot_mob.Adjacent( this ) ) {
				
				if ( this.occupant != null ) {
					return;
				}
				this.icon_state = Lang13.Initial( this, "icon_state" );
				this.occupant = pilot_mob;
				pilot_mob.mecha = this;
				pilot_mob.loc = this;
				this.GrantActions( pilot_mob );
			}
			return;
		}

		// Function from file: mecha.dm
		public void ai_enter_mech( Ent_Static AI = null, dynamic interaction = null ) {
			((dynamic)AI).aiRestorePowerRoutine = 0;
			AI.loc = this;
			this.occupant = AI;
			this.icon_state = Lang13.Initial( this, "icon_state" );
			...
			((dynamic)AI).controlled_mech = this;
			((dynamic)AI).remote_control = this;
			((dynamic)AI).canmove = 1;
			((dynamic)AI).can_shunt = 0;
			((dynamic)AI).WriteMsg( "" + ( interaction == 3 ? "<span class='announce'>Takeover of " + this.name + " complete! You are now permanently loaded onto the onboard computer. Do not attempt to leave the station sector!</span>" : "<span class='notice'>You have been uploaded to a mech's onboard computer." ) );
			((dynamic)AI).WriteMsg( "<span class='boldnotice'>Use Middle-Mouse to activate mech functions and equipment. Click normally for AI interactions.</span>" );
		...
		public void setInternalDamage( dynamic int_dam_flag = null ) {
			...
			((dynamic)this.occupant).WriteMsg( new Sound( "sound/machines/warning-buzzer.ogg", null, false ) );
			this.diag_hud_set_mechstat();
			return;
		}
		...
		// Function from file: mecha.dm
		public bool domove( int? direction = null ) {
			...
			if ( Lang13.Bool( this.defence_mode ) ) {
		...
		// Function from file: mecha.dm
		public void click_action( dynamic target = null, Mob user = null ) {
			int dir_to_target = 0;
		...
		// Function from file: mecha.dm
		public bool can_use( Ent_Static user = null ) {
			...
			if ( user != null && user is Mob ) {
				...
				if ( !Lang13.Bool( ((dynamic)user).incapacitated() ) ) {
					return true;
				}
			}
			return false;
		...
		public GasMixture add_cabin(  ) {
			...
			this.cabin_air.assert_gases( "o2", "n2" );
			this.cabin_air.gases["o2"][1] = ( this.cabin_air.volume ??0) * 0.01 / Convert.ToDouble( this.cabin_air.temperature * 8.31 );
			this.cabin_air.gases["n2"][1] = ( this.cabin_air.volume ??0) * 0.79 / Convert.ToDouble( this.cabin_air.temperature * 8.31 );
			return this.cabin_air;
		}
