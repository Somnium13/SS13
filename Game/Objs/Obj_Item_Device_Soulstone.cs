			...
			switch ((string)( choice )) {
				case "FORCE":
					...
					if ( T.client != null ) {
						...
						user.WriteMsg( "<span class='userdanger'>Capture failed!</span>: The soul has already fled its mortal frame. You attempt to bring it back..." );
				...
				case "VICTIM":
					...
					if ( this.imprinted != "empty" ) {
						...
						if ( GlobalFuncs.iscultist( user ) ) {
							new Obj_Item_SummoningOrb( GlobalFuncs.get_turf( T2 ) );
						}
				...
				case "SHADE":
					...
					if ( Convert.ToInt32( T3.stat ) == 2 ) {
						...
						if ( user != T3 ) {
							user.WriteMsg( "<span class='info'><b>Capture successful!</b>:</span> " + T3.real_name + "'s has been recaptured and stored within the soul stone." );
						}
					}
					break;
				case "CONSTRUCT":
					...
					if ( Lang13.Bool( A ) ) {
						...
						switch ((string)( construct_class )) {
							...
							case "Artificer":
								...
								if ( GlobalFuncs.iscultist( user ) || GlobalFuncs.iswizard( user ) ) {
									GlobalFuncs.makeNewConstruct( typeof(Mob_Living_SimpleAnimal_Hostile_Construct_Builder), A, user, false, T4.loc );
								} else {
									GlobalFuncs.makeNewConstruct( typeof(Mob_Living_SimpleAnimal_Hostile_Construct_Builder_Noncult), A, user, false, T4.loc );
								}
								break;
						}
						GlobalFuncs.qdel( T4 );
