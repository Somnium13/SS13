...
namespace Somnium.Game {
	class Obj_Machinery_Photocopier : Obj_Machinery {
		...
		public override bool MouseDrop_T( Ent_Static dropping = null, Mob user = null ) {
			...
			if ( !( dropping is Mob ) || Lang13.Bool( ((dynamic)dropping).anchored ) || Lang13.Bool( ((dynamic)dropping).buckled ) || !this.Adjacent( user ) || !this.Adjacent( dropping ) || !user.canUseTopic( this, true ) || dropping == this.ass || this.copier_blocked() ) {
				return false;
			}
			...
			if ( dropping == user ) {
				...
				user.visible_message( "<span class='warning'>" + user + " starts putting " + dropping + " onto the photocopier!</span>", "<span class='notice'>You start putting " + dropping + " onto the photocopier...</span>" );
			}

			if ( GlobalFuncs.do_after( user, 20, null, this ) ) {
				
				if ( !( dropping != null ) || Lang13.Bool( dropping.gc_destroyed ) || Lang13.Bool( this.gc_destroyed ) || !this.Adjacent( dropping ) ) {
					return false;
				}
				...
				if ( dropping == user ) {
					...
					user.visible_message( "<span class='warning'>" + user + " puts " + dropping + " onto the photocopier!</span>", "<span class='notice'>You put " + dropping + " onto the photocopier.</span>" );
				}
				dropping.loc = GlobalFuncs.get_turf( this );
				this.ass = dropping;
