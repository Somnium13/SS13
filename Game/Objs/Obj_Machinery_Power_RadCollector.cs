	...
	class Obj_Machinery_Power_RadCollector : Obj_Machinery_Power {
		...
		public void receive_pulse( double? pulse_strength = null ) {
			...
			if ( Lang13.Bool( this.P ) && this.active ) {
				power_produced = ( Lang13.Bool( this.P.air_contents.gases["plasma"] ) ? Lang13.Bool( this.P.air_contents.gases["plasma"][1] ) : false ) ?1:0;
				power_produced *= ( pulse_strength ??0) * 20;
		...
		public override int? process( dynamic seconds = null ) {
			...
			if ( Lang13.Bool( this.P ) ) {
				...
				if ( !Lang13.Bool( this.P.air_contents.gases["plasma"] ) ) {
					this.investigate_log( "<font color='red'>out of fuel</font>.", "singulo" );
					this.eject();
				} else {
					this.P.air_contents.gases["plasma"][1] -= ( this.drainratio ?1:0) * 0.001;
					this.P.air_contents.garbage_collect();
				}
			}
			return null;
		}
