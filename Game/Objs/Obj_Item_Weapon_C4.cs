	...
	class Obj_Item_Weapon_C4 : Obj_Item_Weapon {
		...
		public void explode(  ) {
			...
			if ( Lang13.Bool( this.target ) ) {
				...
				if ( !Lang13.Bool( GlobalFuncs.qdeleted( this.target ) ) ) {
		...
		public override bool afterattack( dynamic target = null, dynamic user = null, bool? proximity_flag = null, string click_parameters = null ) {
			...
			if ( target is Obj_Item_Weapon_Storage && !( target is Obj_Item_Weapon_Storage_Secure || target is Obj_Item_Weapon_Storage_Lockbox ) ) {
				return false;
			}

			if ( target is Obj_Item_Weapon_Storage_Secure || target is Obj_Item_Weapon_Storage_Lockbox ) {
				S = target;

				if ( !Lang13.Bool( S.locked ) ) {
					return false;
				}
			}
