// FILE AUTOGENERATED BY SOMNIUM13.

using System;
using Somnium.Engine.ByImpl;

namespace Somnium.Game {
	class Obj_Structure_Closet_SecureCloset_Miner : Obj_Structure_Closet_SecureCloset {

		protected override void __FieldInit() {
			base.__FieldInit();

			this.icon_closed = "miningsec";
			this.icon_locked = "miningsec1";
			this.icon_opened = "miningsecopen";
			this.icon_broken = "miningsecbroken";
			this.icon_off = "miningsecoff";
			this.req_access = new ByTable(new object [] { 48 });
			this.icon_state = "miningsec1";
		}

		// Function from file: mine_items.dm
		public Obj_Structure_Closet_SecureCloset_Miner ( dynamic loc = null ) : base( (object)(loc) ) {
			// Warning: Super call was HERE! If anything above HERE is needed by the super call, it might break!;
			Task13.Sleep( 2 );

			if ( Rand13.PercentChance( 50 ) ) {
				new Obj_Item_Weapon_Storage_Backpack_Industrial( this );
			} else {
				new Obj_Item_Weapon_Storage_Backpack_SatchelEng( this );
			}
			new Obj_Item_Device_Radio_Headset_HeadsetMining(  );
			new Obj_Item_Clothing_Under_Rank_Miner( this );
			new Obj_Item_Clothing_Gloves_Black( this );
			new Obj_Item_Clothing_Shoes_Black( this );
			new Obj_Item_Device_MiningScanner( this );
			new Obj_Item_Weapon_Storage_Bag_Ore( this );
			new Obj_Item_Device_Flashlight_Lantern( this );
			new Obj_Item_Weapon_Pickaxe_Shovel( this );
			new Obj_Item_Weapon_Pickaxe( this );
			new Obj_Item_Clothing_Glasses_Meson( this );
			new Obj_Item_Device_Gps_Mining( this );
			new Obj_Item_Weapon_Storage_Belt_Mining( this );
			return;
		}

	}

}