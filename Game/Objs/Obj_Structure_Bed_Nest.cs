...
namespace Somnium.Game {
	class Obj_Structure_Bed_Nest : Obj_Structure_Bed {
		...
		public override bool user_buckle_mob( Ent_Static M = null, Mob user = null ) {
			...
			if ( !( M is Mob ) || Map13.GetDistance( this, user ) > 1 || M.loc != this.loc || user.restrained() || user.stat != 0 || Lang13.Bool( ((dynamic)M).buckled ) || user is Mob_Living_Silicon_Pai ) {
				return false;
			}
			...
			if ( !Lang13.Bool( user.getorgan( typeof(Obj_Item_Organ_Internal_Alien_Plasmavessel) ) ) ) {
				return false;
			}
			this.unbuckle_mob();
