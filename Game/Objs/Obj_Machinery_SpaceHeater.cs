	...
	class Obj_Machinery_SpaceHeater : Obj_Machinery {
		...
		public override dynamic attackby( dynamic A = null, dynamic user = null, string _params = null, bool? silent = null, bool? replace_spent = null ) {
			...
			if ( A is Obj_Item_Weapon_StockParts_Cell ) {
				...
				((Ent_Static)user).visible_message( new Txt().The( user ).item().str( " " ).item( ( Lang13.Bool( this.panel_open ) ? "opens" : "closes" ) ).str( " the hatch on " ).the( this ).item().str( "." ).ToString(), new Txt( "<span class='notice'>You " ).item( ( Lang13.Bool( this.panel_open ) ? "open" : "close" ) ).str( " the hatch on " ).the( this ).item().str( ".</span>" ).ToString() );
				this.update_icon();
		...
		public override double examine( dynamic user = null ) {
			...
			user.WriteMsg( new Txt().The( this ).item().str( " is " ).item( ( Lang13.Bool( this.on ) ? "on" : "off" ) ).str( ", and the hatch is " ).item( ( Lang13.Bool( this.panel_open ) ? "open" : "closed" ) ).str( "." ).ToString() );
