	...
	class ModulePicker : Game_Data {
		...
		public void use( Mob user = null ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( this.possible_modules, typeof(AIModule_Large) )) {
				...
				dat += new Txt( "<A href='byond://?src=" ).Ref( this ).str( ";" ).item( module.mod_pick_name ).str( "=1'>" ).item( module.module_name ).str( "</A> (" ).item( module.cost ).str( ")<BR>" ).ToString();
			}

			foreach (dynamic _b in Lang13.Enumerate( this.possible_modules, typeof(AIModule_Small) )) {
				module2 = _b;
				
				dat += new Txt( "<A href='byond://?src=" ).Ref( this ).str( ";" ).item( module2.mod_pick_name ).str( "=1'>" ).item( module2.module_name ).str( "</A> (" ).item( module2.cost ).str( ")<BR>" ).ToString();
			}
			dat += "<HR>";
