...
namespace Somnium.Game {
	class Objective_Hijack : Objective {
		...
		protected override void __FieldInit() {
			...
			this.explanation_text = "Hijack the emergency shuttle by escaping without any organic life-forms, other than yourself.";
		}
		...
		public override dynamic check_completion(  ) {
			...
			if ( !Lang13.Bool( ((dynamic)this.owner).current ) || Lang13.Bool( ((dynamic)this.owner).current.stat ) ) {
				return 0;
			}
			...
			protected_mobs = new ByTable(new object [] { typeof(Mob_Living_Silicon_Ai), typeof(Mob_Living_Silicon_Pai) });
			protected_mobs.Add( new ByTable(new object [] { typeof(Mob_Living_Silicon_Robot), typeof(Mob_Living_Silicon_Robot_Mommi) }) );
