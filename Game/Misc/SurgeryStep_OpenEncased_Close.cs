...
namespace Somnium.Game {
	class SurgeryStep_OpenEncased_Close : SurgeryStep_OpenEncased {
		...
		public override bool end_step( dynamic user = null, dynamic target = null, string target_zone = null, Obj_Item tool = null, dynamic surgery = null ) {
			...
			affected = ((Mob_Living_Carbon_Human)target).get_organ( target_zone );
			msg = new Txt( "<span class='notice'>" ).item( user ).str( " bends " ).item( target ).str( "'s " ).item( affected.encased ).str( " back into place with " ).the( tool ).item().str( ".</span>" ).ToString();
			self_msg = new Txt( "<span class='notice'>You bend " ).item( target ).str( "'s " ).item( affected.encased ).str( " back into place with " ).the( tool ).item().str( ".</span>" ).ToString();
			((Ent_Static)user).visible_message( msg, self_msg );
			affected.open = 2.5;
			return false;
		}
		...
		public override bool begin_step( dynamic user = null, dynamic target = null, string target_zone = null, Obj_Item tool = null, dynamic surgery = null ) {
			...
			affected = ((Mob_Living_Carbon_Human)target).get_organ( target_zone );
			msg = new Txt().item( user ).str( " starts bending " ).item( target ).str( "'s " ).item( affected.encased ).str( " back into place with " ).the( tool ).item().str( "." ).ToString();
			self_msg = new Txt( "You start bending " ).item( target ).str( "'s " ).item( affected.encased ).str( " back into place with " ).the( tool ).item().str( "." ).ToString();
			((Ent_Static)user).visible_message( msg, self_msg );
