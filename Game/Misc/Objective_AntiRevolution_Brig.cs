...
namespace Somnium.Game {
	class Objective_AntiRevolution_Brig : Objective_AntiRevolution {

		public bool already_completed = false;

		public Objective_AntiRevolution_Brig ( string text = null ) : base( text ) {
		...
		public override dynamic check_completion(  ) {
			...
			if ( Lang13.Bool( this.target ) && Lang13.Bool( this.target.current ) ) {
				...
				if ( ((Mind)this.target).is_brigged( 6000 ) ) {
		...
		public override dynamic find_target_by_role( dynamic role = null, bool? role_type = null ) {
			...
			if ( Lang13.Bool( this.target ) && Lang13.Bool( this.target.current ) ) {
				this.explanation_text = "Brig " + this.target.current.real_name + ", the " + ( !( role_type == true ) ? this.target.assigned_role : this.target.special_role ) + " for 20 minutes to set an example.";
			} else {
		...
		public override dynamic find_target(  ) {
			...
			if ( Lang13.Bool( this.target ) && Lang13.Bool( this.target.current ) ) {
				this.explanation_text = "Brig " + this.target.current.real_name + ", the " + this.target.assigned_role + " for 20 minutes to set an example.";
			} else {
				this.explanation_text = "Free Objective";
			}
			return this.target;
