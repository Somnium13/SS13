...
namespace Somnium.Game {
	class Disease_Wizarditis : Disease {
		...
		public Disease_Wizarditis ( bool? process = null, Disease_Advance D = null ) : base( process, D ) {
		...
		public void teleport(  ) {
			...
			foreach (dynamic _a in Lang13.Enumerate( Map13.FetchInRangeExcludeThis( this.affected_mob, 80 ) )) {
				AR = _a;
				

				if ( theareas.Find( AR ) != 0 || AR.type == typeof(Zone) ) {
					continue;
				}
				theareas.Add( AR );
			}
			...
			if ( this.affected_mob is Mob_Living_Carbon_Human ) {
				...
				if ( Rand13.PercentChance( chance ??0 ) ) {
					...
					if ( !( H.head is Obj_Item_Clothing_Head_Wizard ) ) {
						...
						if ( Lang13.Bool( H.head ) ) {
							((Mob)H).drop_from_inventory( H.head );
						}
						H.head = new Obj_Item_Clothing_Head_Wizard( H );
						H.head.layer = 20;
					}
					return;
				}
				...
				if ( Rand13.PercentChance( chance ??0 ) ) {
					...
					if ( !( H.wear_suit is Obj_Item_Clothing_Suit_Wizrobe ) ) {
						...
						if ( Lang13.Bool( H.wear_suit ) ) {
							((Mob)H).drop_from_inventory( H.wear_suit );
						}
						H.wear_suit = new Obj_Item_Clothing_Suit_Wizrobe( H );
				...
				if ( Rand13.PercentChance( chance ??0 ) ) {
					...
					if ( !( H.shoes is Obj_Item_Clothing_Shoes_Sandal ) ) {
						...
						if ( Lang13.Bool( H.shoes ) ) {
							((Mob)H).drop_from_inventory( H.shoes );
						}
						H.shoes = new Obj_Item_Clothing_Shoes_Sandal( H );
						H.shoes.layer = 20;
					}
					return;
				}
				...
				if ( Rand13.PercentChance( chance ??0 ) ) {
					...
					if ( !( H2.r_hand is Obj_Item_Weapon_Staff ) ) {
						...
						if ( Lang13.Bool( H2.r_hand ) || Lang13.Bool( H2.drop_item( H2.r_hand ) ) ) {
		...
		public override bool stage_act(  ) {
			...
			switch ((int?)( this.stage )) {
				case 2:
					...
					if ( Rand13.PercentChance( 1 ) && Rand13.PercentChance( 50 ) ) {
						GlobalFuncs.to_chat( this.affected_mob, "<span class='warning'>You feel " + Rand13.Pick(new object [] { "that you don't have enough mana.", "that the winds of magic are gone.", "an urge to summon familiar." }) + "</span>" );
					}
					break;
				case 3:
					...
					if ( Rand13.PercentChance( 1 ) && Rand13.PercentChance( 50 ) ) {
						GlobalFuncs.to_chat( this.affected_mob, "<span class='warning'>You feel " + Rand13.Pick(new object [] { "the magic bubbling in your veins", "that this location gives you a +1 to INT", "an urge to summon familiar." }) + ".</span>" );
					}
					break;
				case 4:
					...
					if ( Rand13.PercentChance( 1 ) && Rand13.PercentChance( 50 ) ) {
						GlobalFuncs.to_chat( this.affected_mob, "<span class='warning'>You feel " + Rand13.Pick(new object [] { "the tidal wave of raw power building inside", "that this location gives you a +2 to INT and +1 to WIS", "an urge to teleport" }) + ".</span>" );
